...
SCREENSHOT = Path("/tmp/tracevane-chat-composer-upload-failure-acceptance.png")
...
def write_temp_file(name: str, content: str) -> Path:
    root = Path(tempfile.mkdtemp(prefix="tracevane-chat-upload-failure-"))
    path = root / name
...
def upload_response(session_key: str, payload: dict[str, object], suffix: str) -> dict[str, object]:
    ...
    relative_path = f".tracevane/chat-uploads/mock-{int(time.time() * 1000)}-{suffix}-{file_name}"
