...
test('history page API windows messages and exposes search/date helpers', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'openclaw-studio-history-page-'));
  try {
    ...
    fs.writeFileSync(path.join(root, 'agents', 'main', 'sessions', 'sessions.json'), JSON.stringify({
      [sessionKey]: {
        sessionId: 'session-1',
    ...
    fs.writeFileSync(runShadowFile, JSON.stringify({
      sessions: {
        [sessionKey]: [
          {
            sessionKey,
    ...
    const page1 = await context.services.chat.getHistory(sessionKey, { limit: 2 });
    assert.deepEqual(page1.messages.map((message) => message.id), ['m3', 'm4']);
    ...
    assert.deepEqual(page2.messages.map((message) => message.id), ['m1', 'm2']);
    ...
    assert.deepEqual(filteredDay.messages.map((message) => message.id), ['m1', 'm2']);
    ...
    assert.deepEqual(search.messages.map((message) => message.id), ['m2']);
    ...
    const resourceLedger = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'resource',
      limit: 10,
    });
    assert.deepEqual(resourceLedger.messages.map((message) => message.id), ['m3']);
    ...
    assert.deepEqual(codeLedger.messages.map((message) => message.id), ['m3', 'm4']);
    assert.equal(codeLedger.day, null);

    const codeLedgerMarch20 = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'code',
      day: '2026-03-20',
    ...
    assert.deepEqual(codeLedgerMarch20.messages.map((message) => message.id), []);
    assert.equal(codeLedgerMarch20.day, '2026-03-20');
    assert.equal(codeLedgerMarch20.matches.length, 0);

    const codeLedgerMarch21 = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'code',
      day: '2026-03-21',
    ...
    assert.deepEqual(codeLedgerMarch21.messages.map((message) => message.id), ['m3', 'm4']);
    assert.equal(codeLedgerMarch21.day, '2026-03-21');
    assert.deepEqual(codeLedgerMarch21.matches.map((match) => match.messageId), ['m3', 'm4']);
    assert.deepEqual(codeLedgerMarch21.matches.map((match) => match.day), ['2026-03-21', '2026-03-21']);

    const codeLedgerPage1 = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'code',
      limit: 1,
    });
    assert.deepEqual(codeLedgerPage1.messages.map((message) => message.id), ['m4']);
    ...
    const codeLedgerPage2 = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'code',
    ...
    assert.deepEqual(codeLedgerPage2.messages.map((message) => message.id), ['m3']);
    assert.equal(codeLedgerPage2.pageInfo.hasMoreAfter, true);
    assert.equal(typeof codeLedgerPage2.pageInfo.afterCursor, 'string');

    const codeLedgerPage3 = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      role: 'all',
      content: 'code',
    ...
    assert.deepEqual(codeLedgerPage3.messages.map((message) => message.id), ['m4']);
    assert.equal(codeLedgerPage3.roleFilter, 'all');
    assert.equal(codeLedgerPage3.contentFilter, 'code');

    const legacyLedgerBeforeCursor = Buffer.from(JSON.stringify({
    ...
    const legacyLedgerPage = await context.services.chat.searchHistory(sessionKey, {
      query: 'ledger',
      before: legacyLedgerBeforeCursor,
      limit: 1,
    });
    assert.deepEqual(legacyLedgerPage.messages.map((message) => message.id), ['m3']);
    ...
    assert.deepEqual(dates.days.map((entry) => `${entry.day}:${entry.count}`), [
      '2026-03-21:2',
      '2026-03-20:2',
    ]);
...
test('history page resyncs local transcript after rewrite and truncation without duplicating assistant steps', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'openclaw-studio-history-resync-'));
  try {
    const workspace = path.join(root, 'workspace');
    ...
    const writeTranscript = (lines) => {
      fs.writeFileSync(transcriptFile, lines.map((line) => JSON.stringify(line)).join('\n'));
    };
    ...
    const initial = await context.services.chat.getHistory(sessionKey, { limit: 10 });
    assert.deepEqual(initial.messages.map((message) => `${message.id}:${message.text}`), [
    ...
    const rewritten = await context.services.chat.getHistory(sessionKey, { limit: 10 });
    assert.deepEqual(rewritten.messages.map((message) => `${message.id}:${message.text}`), [
      'user-1:first question',
      'assistant-1-rewritten:first answer',
      'assistant-2-rewritten:second answer revised',
    ]);
    assert.equal(
      rewritten.messages.filter((message) => message.text === 'first answer').length,
      1,
    );
    ...
    const truncated = await context.services.chat.getHistory(sessionKey, { limit: 10 });
    assert.deepEqual(truncated.messages.map((message) => `${message.id}:${message.text}`), [
      'user-1:first question',
      'assistant-2-compacted:compacted final answer',
    ]);
...
test('history page collapses inject user with shadow-restored transcript user even when transcript lacks runId', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'openclaw-studio-history-user-dedupe-'));
  let gateway = null;
  try {
    const workspace = path.join(root, 'workspace');
    const sessionsStorePath = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
    const transcriptFile = path.join(root, 'transcripts', 'session-user-dedupe.jsonl');
    ...
    fs.writeFileSync(sessionsStorePath, JSON.stringify({
      [sessionKey]: {
    ...
    const history = await context.services.chat.getHistory(sessionKey, { limit: 20 });
    const userMessages = history.messages.filter((message) => message.role === 'user');

    assert.equal(userMessages.length, 1);
    assert.equal(userMessages[0]?.text, 'history dedupe should restore the pure user text');
    assert.equal(userMessages[0]?.source, 'history');
    assert.equal(userMessages[0]?.runId, ack.runId);
    assert.equal(
      history.messages.some((message) => message.role === 'user' && message.source === 'inject'),
      false,
    );
  } finally {
    await gateway?.close();
