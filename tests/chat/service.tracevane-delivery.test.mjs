...
test('chat upload returns canonical resource ref and signed preview resource', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-upload-resource-'));
  const workspace = path.join(root, 'workspace');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
...
test('chat resource resolver resolves workspace and upload refs without exposing raw paths', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-resource-resolver-'));
  const workspace = path.join(root, 'workspace');
  ...
  try {
    ...
    assert.equal(payload.resources[0]?.resource?.source, 'tracevane_resource');
...
test('chat service restores user shadow blocks with inline refs and keeps only unreferenced attachments as cards', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-user-shadow-'));
  const workspace = path.join(root, 'workspace');
  const uploadsDir = path.join(workspace, 'uploads');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
  const sessionStoreFile = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
  const shadowFile = path.join(root, 'tracevane', 'chat-message-shadows.json');
  const imageRelativePath = 'uploads/123-diagram.png';
  const fileRelativePath = 'uploads/456-report.pdf';
  const transportText = '@uploads/123-diagram.png @uploads/456-report.pdf\n---\n请参考 [@diagram.png](uploads:123-diagram.png "tracevane:inline-image")。';
  ...
  try {
    ...
    assert.equal(message.text, '请参考 [@diagram.png](uploads:123-diagram.png "tracevane:inline-image")。');
...
test('chat service maps transcript tracevane_delivery tool results into assistant blocks/resources', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-delivery-'));
  const workspace = path.join(root, 'workspace');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
  const sessionStoreFile = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
...
test('chat service compiles assistant markdown tracevane links into enhanced markdown plus assistant_markdown resources', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-markdown-'));
  const workspace = path.join(root, 'workspace');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
  const sessionStoreFile = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
...
test('real transcript shape keeps only the compiled tracevane_delivery assistant message', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-delivery-roundtrip-'));
  const workspace = path.join(root, 'workspace');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
  const sessionStoreFile = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
  ...
  try {
    ...
    assert.equal(message.resources?.every((item) => item.source === 'tracevane_delivery'), true);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('gateway history shape with payload serialized in toolResult text still compiles into assistant delivery', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tracevane-service-delivery-gateway-shape-'));
  const workspace = path.join(root, 'workspace');
  const sessionKey = 'agent:main:webchat:direct:tracevane-test';
  const sessionFile = path.join(root, 'agents', 'main', 'sessions', 'tracevane-test.jsonl');
  const sessionStoreFile = path.join(root, 'agents', 'main', 'sessions', 'sessions.json');
