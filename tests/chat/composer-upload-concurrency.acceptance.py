...
SCREENSHOT = Path("/tmp/tracevane-chat-composer-upload-concurrency-acceptance.png")
...
def write_temp_file(name: str, content: str) -> Path:
    root = Path(tempfile.mkdtemp(prefix="tracevane-chat-upload-concurrency-"))
    path = root / name
...
def upload_response(session_key: str, payload: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "resource": {
            ...
            "resourceRef": resource_ref,
            "source": "user_upload",
            "url": preview_url or f"/api/files/download?rootId=project-root&path={relative_path}",
            "downloadUrl": preview_url or f"/api/files/download?rootId=project-root&path={relative_path}&download=true",
        },
    }
...
def read_events(page) -> list[dict[str, object]]:
    return page.evaluate("() => window.__tracevaneReadEvents || []")
