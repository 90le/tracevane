...
const tracevanePluginPath = path.join(
  rootDir,
  "index.ts",
);
...
const tracevanePluginSource = fs.readFileSync(tracevanePluginPath, "utf8");
...
test("terminal workspace page composes integrated shell sections and binds state modules", () => {
  ...
  assert.match(appVue, /<TracevaneShellTopbar[\s\S]*v-if="!isChatSurface && !isFilesSurface && !isTerminalSurface"/);
  assert.doesNotMatch(workspaceCss, /\.main-content\.terminal-surface-route \.tracevane-shell-topbar/);
...
test("terminal workspace side activity exposes real search and source control panels", () => {
  ...
  assert.match(workspacePage, /TERMINAL_THEME_STORAGE_KEY = 'tracevane\.terminal\.theme'/);
...
test("terminal resource explorer opens IDE-style editable file previews", () => {
  ...
  assert.match(workspacePage, /TERMINAL_FILE_PREVIEW_STORAGE_KEY = 'tracevane\.terminal\.filePreviewTabs'/);
  ...
  assert.match(terminalFilePreviewPane, /readFilePreviewModePreference/);
  assert.match(terminalFilePreviewPane, /persistPreviewModePreference\(kind, state\.previewMode\)/);
  assert.match(terminalFilePreviewPane, /resolveDefaultPreviewMode\(tab\)/);
  assert.match(terminalFilePreviewPane, /normalizePreviewModeForKind/);
  ...
  assert.match(terminalMarkdownPreview, /const terminalMarkdownProcessor = unified\(\)/);
  assert.match(terminalMarkdownPreview, /TERMINAL_MARKDOWN_RENDER_CACHE_LIMIT/);
  assert.match(terminalMarkdownPreview, /readTerminalMarkdownRenderCache/);
  assert.match(terminalMarkdownPreview, /writeTerminalMarkdownRenderCache/);
  ...
  assert.match(terminalMarkdownPreview, /@click="handlePreviewClick"/);
  assert.match(terminalMarkdownPreview, /@change="handleEditableChange"/);
  ...
  assert.match(terminalMarkdownPreview, /function handleEditableChange\(event: Event\): void/);
  assert.match(terminalMarkdownPreview, /function handlePreviewClick\(event: MouseEvent\): void/);
  assert.match(terminalMarkdownPreview, /function handleMarkdownDrop\(event: DragEvent\): void/);
  assert.match(terminalMarkdownPreview, /function handleMarkdownPaste\(event: ClipboardEvent\): void/);
  assert.match(terminalMarkdownPreview, /function insertMarkdownUploadedFiles\(files: File\[\]\): Promise<void>/);
  assert.match(terminalMarkdownPreview, /function applyMarkdownMediaAction\(button: HTMLButtonElement\): void/);
  assert.match(terminalMarkdownPreview, /function openMarkdownMediaLightbox/);
  assert.match(terminalMarkdownPreview, /terminal-doc-lightbox/);
  assert.match(terminalMarkdownPreview, /terminal-doc-media-block/);
  assert.match(terminalMarkdownPreview, /data-terminal-media-action/);
  assert.match(terminalMarkdownPreview, /serializeEditableMarkdownMediaElement/);
  assert.match(terminalMarkdownPreview, /function copyPreviewCodeBlock\(button: HTMLButtonElement\): Promise<void>/);
  ...
  assert.match(terminalMarkdownPreview, /node\.classList\.contains\('terminal-doc-codeblock'\)/);
  ...
  assert.match(terminalMarkdownPreview, /renderTerminalCodeBlockHtml/);
  assert.match(terminalMarkdownPreview, /class="terminal-doc-codeblock"/);
  assert.match(terminalMarkdownPreview, /data-terminal-code-copy/);
  assert.match(terminalMarkdownPreview, /contenteditable="false"/);
  ...
  assert.match(workspaceCss, /\.terminal-doc-preview\s*\{[\s\S]*scrollbar-gutter:\s*stable;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document > :where\(p, ul, ol, blockquote, dl, details\)\s*\{[\s\S]*max-width:\s*1180px;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document h1\s*\{[\s\S]*font-size:\s*2\.42rem;/);
  ...
  assert.match(workspaceCss, /\.terminal-doc-preview__document li::marker\s*\{[\s\S]*color:\s*var\(--doc-brand\);/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.task-list-item\s*\{[\s\S]*display:\s*flex;/);
  assert.match(workspaceCss, /\.terminal-doc-preview--editable \.terminal-doc-preview__document \.task-list-item:hover\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-task-checkbox\s*\{[\s\S]*cursor:\s*pointer;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document kbd\s*\{[\s\S]*font:\s*700 0\.82em var\(--terminal-font-family\);/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document pre\s*\{[\s\S]*max-height:\s*min\(64vh,\s*760px\);/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-codeblock\s*,[\s\S]*\.terminal-doc-preview__document \.code-block-wrapper\s*\{[\s\S]*background:\s*var\(--doc-code-bg\);/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-codeblock__bar\s*\{[\s\S]*justify-content:\s*space-between;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-codeblock__copy\s*\{[\s\S]*min-height:\s*28px;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-codeblock__copy\[data-copy-state="copied"\]\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document caption\s*\{[\s\S]*text-align:\s*left;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document thead th\s*\{[\s\S]*position:\s*sticky;/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document details\s*\{[\s\S]*border:\s*1px solid var\(--doc-line\);/);
  assert.match(workspaceCss, /\.terminal-doc-preview__document details\[open\] > summary\s*\{/);
  ...
  assert.match(workspaceCss, /\.terminal-doc-preview__document \.terminal-doc-media-block\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-media-block__tools\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-media-block__tools button\.active\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-lightbox\s*\{/);
  assert.match(workspaceCss, /\.terminal-doc-lightbox__panel\s*\{/);
  ...
  assert.match(terminalFilePreviewPane, /imagePanByTab/);
  assert.match(terminalFilePreviewPane, /class="terminal-file-preview__image-toolbar"/);
  assert.match(terminalFilePreviewPane, /class="terminal-file-preview__image-stage"/);
  assert.match(terminalFilePreviewPane, /'terminal-file-preview__image-stage--pannable': activeImagePannable/);
  assert.match(terminalFilePreviewPane, /'terminal-file-preview__image-stage--dragging': imagePanDragging/);
  assert.match(terminalFilePreviewPane, /@pointerdown="startImagePan"/);
  assert.match(terminalFilePreviewPane, /@pointermove="moveImagePan"/);
  assert.match(terminalFilePreviewPane, /@wheel\.ctrl\.prevent="zoomActiveImageFromWheel"/);
  assert.match(terminalFilePreviewPane, /class="terminal-file-preview__image-canvas"/);
  assert.match(terminalFilePreviewPane, /class="terminal-file-preview__image-img"/);
  assert.match(terminalFilePreviewPane, /draggable="false"/);
  assert.match(terminalFilePreviewPane, /activeImageZoomPercent/);
  assert.match(terminalFilePreviewPane, /activeImagePan/);
  assert.match(terminalFilePreviewPane, /activeImageStyle/);
  assert.match(terminalFilePreviewPane, /translate3d\(\$\{activeImagePan\.value\.x\}px, \$\{activeImagePan\.value\.y\}px, 0\) scale\(\$\{activeImageZoom\.value\}\)/);
  ...
  assert.match(terminalFilePreviewPane, /querySelector\('\.terminal-file-preview__image-canvas'\)/);
  assert.match(terminalFilePreviewPane, /function startImagePan\(event: PointerEvent\): void/);
  assert.match(terminalFilePreviewPane, /function moveImagePan\(event: PointerEvent\): void/);
  assert.match(terminalFilePreviewPane, /function endImagePan\(event: PointerEvent\): void/);
  assert.match(terminalFilePreviewPane, /function setImagePan\(tabId: string, x: number, y: number\): void/);
  assert.match(terminalFilePreviewPane, /function clampImageZoom\(value: number\): number/);
  assert.match(terminalFilePreviewPane, /function normalizeImagePanOffset\(value: number\): number/);
  ...
  assert.match(workspaceCss, /\.terminal-file-preview__image-stage\s*\{[\s\S]*overflow:\s*hidden;[\s\S]*overscroll-behavior:\s*contain;[\s\S]*padding:\s*42px 18px 18px;/);
  assert.match(workspaceCss, /\.terminal-file-preview__image-canvas\s*\{[\s\S]*display:\s*grid;[\s\S]*place-items:\s*center;[\s\S]*width:\s*100%;[\s\S]*height:\s*100%;/);
  assert.match(workspaceCss, /\.terminal-file-preview__image-stage--pannable\s*\{[\s\S]*cursor:\s*grab;[\s\S]*touch-action:\s*none;/);
  assert.match(workspaceCss, /\.terminal-file-preview__image-stage--dragging\s*\{[\s\S]*cursor:\s*grabbing;[\s\S]*user-select:\s*none;/);
  ...
  assert.match(workspaceCss, /\.terminal-file-preview__image-img\s*\{[\s\S]*max-width:\s*100%;[\s\S]*max-height:\s*100%;[\s\S]*user-select:\s*none;[\s\S]*will-change:\s*transform;[\s\S]*transition:\s*transform 120ms ease;/);
  assert.match(workspaceCss, /\.terminal-file-preview__image-stage--dragging \.terminal-file-preview__image-img\s*\{[\s\S]*transition:\s*none;/);
  assert.doesNotMatch(workspaceCss, /\.terminal-file-preview__image-img\s*\{[^}]*pointer-events:\s*none;/);
...
test("terminal console keeps replay cursor ephemeral so refreshed xterm replays backlog", () => {
  ...
  assert.match(terminalConsole, /gatewayClient\.notify\(TRACEVANE_TERMINAL_GATEWAY_METHODS\.input/);
  ...
  assert.match(terminalConsole, /getTracevaneTerminalDirectWebSocketUrl/);
  ...
  assert.match(terminalConsole, /TRACEVANE_TERMINAL_GATEWAY_METHODS\.clear/);
  ...
  assert.match(tracevanePluginSource, /TRACEVANE_TERMINAL_GATEWAY_METHODS\.clear/);
  assert.match(apiRuntimeConfig, /terminalDirectWebSocketPort: null/);
  assert.match(webRuntimeConfig, /function getTracevaneTerminalDirectWebSocketUrl/);
...
test("terminal gateway targeted output preserves order through service batching", () => {
  ...
  assert.doesNotMatch(
    tracevanePluginSource,
    /broadcastToConnIds\(TRACEVANE_TERMINAL_GATEWAY_EVENT, event, connIds, \{\s*dropIfSlow: true,\s*\}\)/,
  );
  assert.match(
    tracevanePluginSource,
