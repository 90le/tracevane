...
test("terminal workspace exposes shared inspector content through the integrated menu and mobile sheet", () => {
  ...
  assert.match(inspectorContent, /function closeOtherInspectorMenus\(openMenu: TerminalInspectorMenuKey\): void/);
  ...
  assert.match(inspectorContent, /window\.addEventListener\('resize', closeInspectorMenus\)/);
...
test("terminal resource explorer opens IDE-style editable file previews", () => {
  ...
  assert.match(terminalFilePreviewPane, /AsyncMarkdownBlock/);
  assert.match(terminalFilePreviewPane, /import\('\.\.\/chat\/MarkdownBlock\.vue'\)/);
  ...
  assert.match(terminalFilePreviewPane, /type TerminalFilePreviewMode = 'edit' \| 'preview'/);
  ...
  assert.match(terminalFilePreviewPane, /state\.previewMode = state\.previewMode === 'preview' \? 'edit' : 'preview'/);
  assert.match(terminalFilePreviewPane, /<AsyncMarkdownBlock[\s\S]*:source="activeDraft"[\s\S]*:force-eager-render="true"/);
