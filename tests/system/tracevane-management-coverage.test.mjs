...
const baselineFile = path.join(
  ...
  "superpowers",
  "inventories",
  "tracevane-management-coverage.json",
);
const coverageScriptFile = path.join(
  root,
  "scripts",
  "tracevane-management-coverage.mjs",
);
...
function runCoverageScript() {
  const stdout = execFileSync(
    process.execPath,
    [path.join(root, "scripts", "tracevane-management-coverage.mjs")],
    {
      cwd: root,
      encoding: "utf8",
    },
...
test("tracevane management coverage script matches the committed baseline inventory", () => {
...
test("tracevane management coverage script loads manifest exports instead of scanning ts source text", () => {
...
test("tracevane management coverage baseline includes the skills workspace recipe regression", () => {
  const parsed = runCoverageScript();
  ...
  assert.deepEqual(skills, {
    ...
    matchedFiles: [
      "tests/system/tracevane-web-skills-lifecycle-contract.test.mjs",
      "tests/system/tracevane-web-skills-workspace-recipe.test.mjs",
    ],
  });
});
