...
test('chat portal overlays are backed by unscoped style blocks so teleported nodes keep their surfaces', () => {
  ...
  assert.match(conversationPane, /<style>[\s\S]*\.chat-session-menu-popover\s*\{/);
  assert.match(conversationPane, /<style>[\s\S]*\.chat-rendering-settings-dialog\s*\{/);
  assert.match(chatShellPage, /<style>[\s\S]*\.chat-mobile-drawer-mask\s*\{/);
  assert.match(chatShellPage, /<style>[\s\S]*\.chat-inspector-sheet\s*\{/);
  assert.match(sessionFilterBar, /<style>[\s\S]*\.chat-shell-session-filter-popover\s*\{/);
  assert.match(newChatAgentPicker, /<style>[\s\S]*\.chat-agent-picker-mask\s*\{/);
});
