...
test('channels router exposes dedicated deep-edit routes instead of one shared workspace stack', () => {
  ...
  assert.match(routerSource, /path:\s*["']:type\/settings["']/);
  assert.match(routerSource, /path:\s*["']:type\/accounts\/:accountId["']/);
  assert.match(routerSource, /path:\s*["']:type\/accounts\/:accountId\/access["']/);
  assert.match(routerSource, /path:\s*["']:type\/accounts\/:accountId\/pairing["']/);
  assert.match(routerSource, /path:\s*["']:type\/bindings["']/);
});
