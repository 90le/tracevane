...
test("app shell navigation metadata matches current product domains", () => {
  ...
  assert.match(navigation, /export type NavGroup = "总览" \| "工作" \| "连接" \| "平台" \| "证据"/);
  assert.match(navigation, /NAV_GROUP_ORDER: NavGroup\[\] = \["总览", "工作", "连接", "平台", "证据"\]/);
  ...
  assert.match(navigation, /label: "OpenClaw"/);
  assert.match(navigation, /group: "平台"/);
  assert.match(navigation, /label: "工作区文件"/);
