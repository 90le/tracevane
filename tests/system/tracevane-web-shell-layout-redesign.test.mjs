...
test("shell redesign keeps one sidebar navigation model and no global context panel", () => {
  assert.match(appVue, /TracevaneShellTopbar/);
  ...
  assert.match(
    appVue,
    /<TracevaneCommandPalette\s+v-model:open="commandPaletteOpen"\s*\/>/,
  );

  assert.doesNotMatch(appVue, /TracevaneContextPanel|TracevaneShellContextRail/);
