...
test('cron view wires through overview recipe seam', () => {
  assert.match(
    cronView,
    /import\s+\{\s*buildCronOverviewRecipe\s*\}\s+from\s+'\.\.\/features\/cron\/cron-overview-recipe'/,
  );
  assert.match(cronView, /const\s+overviewRecipe\s*=\s*buildCronOverviewRecipe\(/);
  assert.match(cronView, /<CronControlPage\s+:overview-recipe="overviewRecipe"\s*\/>/);
});
...
test('cron control page consumes recipe content for stage copy', () => {
  ...
  assert.match(cronControlPage, /DEFAULT_CRON_OVERVIEW_RECIPE/);
  assert.match(
    cronControlPage,
    /const\s+overviewRecipe\s*=\s*computed\(\(\)\s*=>\s*props\.overviewRecipe\s*\?\?\s*DEFAULT_CRON_OVERVIEW_RECIPE\)/,
  );
  ...
  assert.match(cronControlPage, /overviewRecipe\.value\.workspaceTabs\.overview/);
...
test('cron overview recipe exports typed recipe builder', () => {
  assert.match(cronOverviewRecipe, /export interface CronOverviewRecipe/);
  assert.match(cronOverviewRecipe, /export function buildCronOverviewRecipe\(/);
});

test('cron api and service keep seam builders out of transport layers', () => {
  assert.doesNotMatch(cronApi, /buildCronOverviewRecipe/);
  assert.doesNotMatch(cronService, /buildCronOverviewRecipe/);
});
