...
const sidebarRailPath = path.join(rootDir, 'apps/web-vue/src/components/TracevaneSidebarRail.vue');
...
test('app shell uses reka dialog primitives for the mobile sidebar instead of a hand-rolled mask div', () => {
  ...
  assert.match(appVue, /TracevaneSidebarRail/);
...
test('sidebar rail no longer duplicates topbar theme and locale controls', () => {
  ...
  assert.match(appVue, /<TracevaneShellTopbar[\s\S]*:theme-switch-label/);
  assert.match(appVue, /<TracevaneShellTopbar[\s\S]*:locale-switch-label/);
});
