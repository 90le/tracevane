...
test("native Channel Connectors daemon owns Feishu long-connection ingress", () => {
  ...
  assert.match(daemonSource, /feishu_ws_lock_held_by_pid_/);
    assert.match(daemonSource, /return DEFAULT_FEISHU_PING_TIMEOUT_SECONDS/);
  ...
  assert.doesNotMatch(daemonSource, /group\.lifecycleDispatcherCallbacks === 0/);
  assert.doesNotMatch(daemonSource, /group\.lifecycleReceivedMessages > 0/);
  ...
  assert.doesNotMatch(daemonSource, /feishu_ping_timeout_seconds/);
