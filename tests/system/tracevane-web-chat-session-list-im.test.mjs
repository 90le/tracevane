...
test('folder scope keeps agent/source filter options from descendant sessions and archived mode stays isolated', () => {
  ...
  const activeSessions = ref([
    createSession('root-session', 'alpha', 'tracevane'),
    createSession('child-session', 'beta', 'external'),
    createSession('loose-root-session', 'gamma', 'tracevane'),
  ]);
  ...
  const observedSessions = ref([
    createSession('observed-session', 'omega', 'tracevane', {
  ...
  assert.deepEqual(filters.availableSourceOptions.value.map((option) => option.id), ['external', 'tracevane', 'system']);
  ...
  assert.deepEqual([...new Set(filters.availableSourceOptions.value.map((option) => option.id))], ['external', 'tracevane']);
...
test('session filtering avoids full searchable text construction until a query is present', () => {
  const activeSessions = ref([
    createSession('plain-session', 'alpha', 'tracevane'),
  ]);
  let titleCalls = 0;
  let previewCalls = 0;
...
test('session selection summary stays cold until selection mode opens', () => {
  const visibleSessions = ref([
    createSession('selection-a', 'alpha', 'tracevane'),
    createSession('selection-b', 'beta', 'tracevane'),
  ]);
