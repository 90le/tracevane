...
const workbenchCss = fs.readFileSync(
  path.join(rootDir, "apps/web-vue/src/shared/styles/tracevane-workbench.css"),
  "utf8",
);
...
test("agents workspace uses a persistent object list and top task bar instead of the old ledger landing page", () => {
  ...
  assert.match(agentsWorkspaceLayout, /class="agents-task-bar tracevane-workbench-task-bar"/);
  assert.doesNotMatch(agentsWorkspaceLayout, /agents-task-rail|tracevane-workbench-task-rail/);
...
test("agents workspace chrome inherits DuoYuan tokens instead of local modal and status colors", () => {
  ...
  assert.match(workbenchCss, /\.tracevane-workbench-task-nav-button\s*\{[\s\S]*background:\s*transparent;/);
  assert.match(workbenchCss, /\.tracevane-workbench-task-nav-button\.active\s*\{[\s\S]*background:\s*color-mix\(in srgb, var\(--acc\) 12%, var\(--surface-base\)\);/);
