...
test("Workspace document view registry is quarantined as a future adapter boundary", () => {
  ...
  assert.match(registry, /JSON 结构检查/);
  assert.match(registry, /CSV\/TSV 表格检查/);
  assert.match(registry, /图片只读检查/);
  assert.match(registry, /PDF 只读检查/);
  assert.match(registry, /文本源码检查/);
  assert.match(registry, /二进制\/未知文件占位检查/);
  ...
  assert.doesNotMatch(registry, /label: "JSON 结构化预览"/);
  assert.doesNotMatch(registry, /label: "CSV\/TSV 表格预览"/);
  assert.doesNotMatch(registry, /label: "图片预览"/);
  assert.doesNotMatch(registry, /label: "PDF 预览"/);
...
test("Workspace terminal commands treat terminal as a first-class IDE panel", () => {
  ...
  assert.match(panelCommands, /terminal\.panel\.diagnosticEvidence/);
  ...
  assert.doesNotMatch(panelCommands, /terminal\.panel\.ai\.diagnose/);
...
test("Workspace Git commands separate evidence from AI generation", () => {
  ...
  assert.match(gitCommands, /Git：复制提交证据包/);
  assert.match(gitCommands, /git\.panel\.commitReviewEntry/);
  assert.match(gitCommands, /id: "git\.panel\.commitMessageAdvice"[\s\S]{0,220}group: "证据"/);
  assert.match(gitCommands, /git\.panel\.changeReviewSummary/);
  assert.doesNotMatch(gitCommands, /git\.panel\.ai\.commitMessage/);
  assert.doesNotMatch(gitCommands, /git\.panel\.explainStatus/);
