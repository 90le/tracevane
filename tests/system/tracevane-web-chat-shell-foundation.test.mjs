...
test("chat shell dark theme uses DuoYuan solid dark surfaces instead of gray glass slabs", () => {
  ...
  assert.match(
    chatShellWorkspaceCss,
    /\.chat-shell-layout\s*\{[\s\S]*border-radius:\s*var\(--tracevane-workspace-radius,\s*12px\);/,
  );
  assert.match(
    chatShellWorkspaceCss,
...
test("chat shell defers the root-route history window and loads date buckets on demand", () => {
  ...
  assert.match(
    chatShellPage,
    /watch\(\s*\[routeSessionKey, \(\) => props\.shellMode, tracevaneManagedSessions, observedSessions, sessionsLoading, bootstrapLoading\],/,
  );
  assert.match(
    chatShellPage,
...
test("chat shell coalesces high-frequency tool partial output while keeping terminal tool events immediate", () => {
  ...
  assert.match(chatShellPage, /const CHAT_DEBUG_TRACE_STORAGE_KEY = 'tracevane\.chat\.debug-stream-trace';/);
  assert.match(chatShellPage, /window\.__TRACEVANE_CHAT_TRACE__/);
...
test("chat shell bootstraps the first session rail quickly and hydrates lower-priority agents later", () => {
  ...
  assert.match(chatShellPage, /const CHAT_SHELL_WARM_CACHE_STORAGE_KEY = 'tracevane\.chat\.shell-warm-cache';/);
