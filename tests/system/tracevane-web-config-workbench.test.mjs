...
test('dev config watcher does not force a full page reload after ajax saves', () => {
  assert.match(viteConfig, /syncTracevaneDevConfig\('openclaw\.json changed'\)/);
...
test('config workbench tracks unsaved domains and protects refresh', () => {
  ...
  assert.match(cssRuleBlock(configWorkspaceCss, '.config-save-dock'), /border-radius:\s*var\(--tracevane-workspace-radius,\s*12px\);/);
