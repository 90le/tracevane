...
const commandPaletteSource = read("apps/web-vue/src/components/TracevaneCommandPalette.vue");
const commandPaletteCss = read("apps/web-vue/src/components/tracevane-command-palette.css");
const topbarSource = read("apps/web-vue/src/components/TracevaneShellTopbar.vue");

test("standard mobile shell pages expose the global navigation control", () => {
  assert.match(topbarSource, /class="topbar-mobile-nav"/);
  assert.doesNotMatch(topbarSource, /tracevane-shell-topbar__command/);
  assert.doesNotMatch(topbarSource, /open-command-palette|commandLabel|Ctrl K/);
  assert.doesNotMatch(styleCss, /\.tracevane-shell-topbar__command/);
  assert.match(
    styleCss,
  ...
  assert.match(
    styleCss,
    /@media\s*\(max-width:\s*980px\)\s*\{[\s\S]*?\.tracevane-shell-topbar\s*\{[\s\S]*?grid-template-columns:\s*34px minmax\(0,\s*1fr\) auto;/,
  );
  assert.match(
    styleCss,
    /@media\s*\(max-width:\s*920px\)\s*\{[\s\S]*?\.tracevane-shell-topbar__controls\s*\{[\s\S]*?grid-column:\s*1\s*\/\s*-1;/,
  );
});

test("tool rail command button opens a non-navigation command palette", () => {
  assert.match(appVue, /TracevaneCommandPalette/);
  ...
  assert.match(commandPaletteSource, /import '\.\/tracevane-command-palette\.css';/);
  ...
  assert.doesNotMatch(styleCss, /\.tracevane-command-palette/);
  assert.equal(appVue.includes('<TracevaneCommandPalette\n        v-model:open="commandPaletteOpen"\n        :nav-groups="navGroups"'), false);
});
...
test("shell popovers use solid theme-aware modal surfaces", () => {
  ...
  assert.match(
    commandPaletteCss,
    /\.tracevane-command-palette__panel\s*\{[\s\S]*background:\s*var\(--modal-panel-bg\);/,
  );
  assert.match(
    commandPaletteCss,
    /\.tracevane-command-palette__item:focus-visible\s*\{[\s\S]*box-shadow:\s*0 0 0 3px var\(--mono-ring\);/,
  ...
  assert.doesNotMatch(styleCss, /\.tracevane-context-panel\s*\{/);
