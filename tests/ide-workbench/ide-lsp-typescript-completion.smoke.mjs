...
async function requestGatewayEvent(payload) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      socket.close();
      reject(new Error('LSP WebSocket completion timeout'));
    }, 10_000);
...
async function run() {
  ...
  const prefix = `tracevane-ide-lsp-ts-completion-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  const tsPath = `${smokeDir}/completion.ts`;
  ...
  const payload = { type: 'completion', rootId, path: tsPath, language: 'typescript', content: tsContent, line: 4, column: 21 };
  const completion = await api('/api/lsp/completion', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
  if (completion.provider !== 'typescript' || !completion.items?.some((item) => item.label === 'tracevaneAnswer' && item.kind === 'function')) {
    throw new Error(`direct TypeScript completion mismatch: ${JSON.stringify(completion)}`);
  }

  const gatewayCompletion = await requestGatewayEvent(payload);
  if (gatewayCompletion.provider !== 'typescript' || !gatewayCompletion.items?.some((item) => item.label === 'tracevaneAnswer')) {
    throw new Error(`LSP WebSocket TypeScript completion mismatch: ${JSON.stringify(gatewayCompletion)}`);
  }
  ...
  try {
    ...
    await page.locator('[data-ide-output-event]', { hasText: 'TypeScript/JavaScript hover/definition/completion' }).waitFor({ state: 'visible', timeout: 30_000 });
  } catch (error) {
