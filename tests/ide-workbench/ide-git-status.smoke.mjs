...
async function run() {
  ...
  const gitFilePath = normalizePortablePath(`${repoParentRelativePath}/tests/ide-workbench/git-status-smoke-${suffix}.txt`);
  ...
  if (!status.changes?.some((change) => change.path === expectedRepoPath && change.kind === 'untracked')) {
    throw new Error(`Git status did not include smoke file ${expectedRepoPath}: ${JSON.stringify(status.changes?.slice(0, 20))}`);
  }
  ...
  try {
    ...
    await page.locator(`${tabSelector(gitFilePath)} [data-ide-editor-tab-git-decoration]`).first().waitFor({ state: 'visible', timeout: 30_000 });
