...
async function run() {
  ...
  const gitFilePath = normalizePortablePath(`${repoParentRelativePath}/tests/ide-workbench/git-diff-smoke-${suffix}.txt`);
  ...
  await createFile(rootId, gitFilePath, `tracevane git diff smoke ${suffix}\n`);
  ...
  try {
    ...
    const expectedContent = `tracevane git diff smoke ${suffix}\n`;
