...
const SMOKE_PORT = Number(process.env.TRACEVANE_WEB_PORT || 5193);
...
async function startDevServerIfNeeded() {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) throw new Error(`dev server exited ${child.exitCode}: ${logs.join('').slice(-4000)}`);
    try {
  ...
  throw new Error(`Timed out waiting for debug API: ${lastError instanceof Error ? lastError.message : String(lastError)}\nlogs=${logs.join('').slice(-4000)}`);
}
...
async function run() {
  ...
  if (!rootId || !root.absolutePath) throw new Error('No file root is available for IDE debug watch/evaluate smoke');
  const smokeDir = normalizePortablePath(path.join(relativePathFromRoot(root.absolutePath, process.cwd()), '.tmp', `ide-debug-watch-evaluate-${Date.now().toString(36)}`));
  const filePath = `${smokeDir}/debug-watch-evaluate-target.js`;
  await ensureDirectory(rootId, smokeDir);
  await createFile(rootId, filePath, ['const smoke = process.argv.slice(2);', 'console.log(smoke.join(","));', ''].join('\n'));

  const status = await waitForApi('/api/debug/status');
  for (const feature of ['launch-profiles', 'launch-config-validation', 'launch-args-env-guard', 'debug-control-commands', 'debug-scopes', 'debug-console-evaluate-proof', 'debug-watch-expressions-proof']) {
    if (!status.features?.includes(feature)) throw new Error(`Debug status missing ${feature}: ${JSON.stringify(status.features)}`);
  }
  const nodeLite = status.supportedProfiles?.find((profile) => profile.id === 'node-lite');
  if (!nodeLite?.requiresProgram || !nodeLite.allowArgs || !nodeLite.allowEnv || !nodeLite.programExtensions?.includes('.js')) {
    throw new Error(`Node Lite launch profile metadata is incomplete: ${JSON.stringify(nodeLite)}`);
  }
  ...
  const badEnv = await apiRaw('/api/debug/sessions', {
    method: 'POST',
    body: JSON.stringify({ rootId, cwd: smokeDir, profileId: 'node-lite', program: filePath, env: { 'BAD-KEY': '1' } }),
  });
  if (badEnv.ok || !badEnv.text.includes('Invalid debug launch env key')) {
    throw new Error(`Node Lite launch unexpectedly accepted invalid env: ${badEnv.text}`);
  }
  ...
  if (apiPayload.session?.profileId !== 'node-lite' || apiPayload.session?.launchArgs?.length !== 2 || apiPayload.session?.launchEnvKeys?.[0] !== 'TRACEVANE_SMOKE') {
    throw new Error(`Debug launch profile API did not preserve sanitized config: ${JSON.stringify(apiPayload)}`);
  }
  const evaluatePayload = await api('/api/debug/sessions/evaluate', {
    method: 'POST',
    body: JSON.stringify({ sessionId: apiPayload.session.id, expression: 'session.state', mode: 'evaluate' }),
  });
  if (evaluatePayload.result?.value !== 'stopped' || evaluatePayload.result?.mode !== 'evaluate') {
    throw new Error(`Debug evaluate API returned unexpected result: ${JSON.stringify(evaluatePayload)}`);
  }
  const watchPayload = await api('/api/debug/sessions/evaluate', {
    method: 'POST',
    body: JSON.stringify({ sessionId: apiPayload.session.id, expression: 'cwd', mode: 'watch' }),
  });
  if (watchPayload.result?.value !== smokeDir || watchPayload.result?.mode !== 'watch') {
    throw new Error(`Debug watch API returned unexpected result: ${JSON.stringify(watchPayload)}`);
  }
  const badEvaluate = await apiRaw('/api/debug/sessions/evaluate', {
    method: 'POST',
    body: JSON.stringify({ sessionId: apiPayload.session.id, expression: '', mode: 'evaluate' }),
  });
  if (badEvaluate.ok || !badEvaluate.text.includes('expression is required')) {
    throw new Error(`Debug evaluate unexpectedly accepted empty expression: ${badEvaluate.text}`);
  }
  ...
  try {
    ...
    await page.locator('[data-ide-debug-profile-select]').selectOption('node-lite');
    ...
    await page.locator('[data-ide-debug-scope][data-ide-debug-scope-name="Local"]').first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-scope][data-ide-debug-scope-name="Launch"]').first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-scope][data-ide-debug-scope-name="Adapter"]').first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-scope-variable][data-ide-debug-scope-variable-name="envKeys"]', { hasText: 'TRACEVANE_SMOKE' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-variable][data-ide-debug-variable-name="envKeys"]', { hasText: 'TRACEVANE_SMOKE' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-event]', { hasText: 'Debug scopes received' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-input]').fill('session.state');
    await page.locator('[data-ide-debug-console-evaluate]').click();
    await page.locator('[data-ide-debug-evaluation-result]', { hasText: 'stopped' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-event]', { hasText: 'Debug evaluate proof: session.state => stopped' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-input]').fill('cwd');
    await page.locator('[data-ide-debug-console-watch]').click();
    await page.locator('[data-ide-debug-watch-result]', { hasText: smokeDir }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-input]').fill('program');
    await page.locator('[data-ide-debug-console-evaluate]').click();
    await page.locator('[data-ide-debug-evaluation-result]', { hasText: filePath }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-stop]').click();
    await page.locator('[data-ide-debug-session][data-ide-debug-session-state="terminated"][data-ide-debug-session-profile="node-lite"]').first().waitFor({ state: 'visible', timeout: 30_000 });
  } catch (error) {
