...
async function run() {
  ...
  try {
    ...
    const closeUiElapsedMs = Date.now() - closeStartedAt;
    if (closeUiElapsedMs >= 1_000) {
      throw new Error(`Terminal pane UI waited for backend close before removing pane: ${closeUiElapsedMs}ms`);
    }
