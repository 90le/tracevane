...
const SMOKE_PORT = Number(process.env.TRACEVANE_WEB_PORT || 5190);
...
async function run() {
  ...
  if (!rootId || !root.absolutePath) throw new Error('No file root is available for IDE debug launch profile smoke');
  const smokeDir = normalizePortablePath(path.join(relativePathFromRoot(root.absolutePath, process.cwd()), '.tmp', `ide-debug-launch-profile-${Date.now().toString(36)}`));
  const filePath = `${smokeDir}/launch-profile-target.js`;
  ...
  try {
    ...
    await page.locator('[data-ide-debug-console-event]', { hasText: 'args=2; envKeys=1' }).first().waitFor({ state: 'visible', timeout: 30_000 });
  } catch (error) {
