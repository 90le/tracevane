...
async function run() {
  ...
  const prefix = `tracevane-ide-lsp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  const jsonPath = `${smokeDir}/invalid.json`;
  ...
  const directPayload = { type: 'diagnose', rootId, path: jsonPath, language: 'json', content: '{"x": }' };
  const direct = await api('/api/lsp/diagnostics', {
    method: 'POST',
    body: JSON.stringify(directPayload),
  });
  if (!direct.diagnostics?.length) throw new Error('direct LSP diagnostics endpoint returned no diagnostics');
  const gateway = await requestGatewayDiagnostics(directPayload);
  if (!gateway.diagnostics?.length) throw new Error('LSP WebSocket gateway returned no diagnostics');
  ...
  try {
    ...
    await page.locator(`[data-ide-problem-row][data-ide-problem-path="${cssAttr(jsonPath)}"]`).waitFor({ state: 'visible', timeout: 30_000 });
    const problemText = await page.locator(`[data-ide-problem-row][data-ide-problem-path="${cssAttr(jsonPath)}"]`).first().innerText();
    if (!problemText.includes('JSON_PARSE') || !problemText.includes('lsp')) throw new Error(`LSP problem row mismatch: ${problemText}`);

    await page.locator(`[data-ide-problem-row][data-ide-problem-path="${cssAttr(jsonPath)}"]`).first().click();
    await page.locator(tabSelector(jsonPath)).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator(`[data-ide-monaco-editor-panel][data-ide-editor-file-path="${cssAttr(jsonPath)}"]`).waitFor({ state: 'visible', timeout: 30_000 });
    ...
    await page.locator('[data-ide-output-event]', { hasText: 'JSON diagnostics active' }).waitFor({ state: 'visible', timeout: 30_000 });
  } catch (error) {
