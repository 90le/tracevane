...
async function requestGatewayEvent(payload) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      socket.close();
      reject(new Error('LSP WebSocket references timeout'));
    }, 10_000);
    ...
    socket.on('message', (data) => {
      ...
      if (event.type === 'diagnostics' || event.type === 'hover' || event.type === 'definition' || event.type === 'references') {
...
async function run() {
  ...
  const prefix = `tracevane-ide-lsp-ts-references-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  const tsPath = `${smokeDir}/references.ts`;
  ...
  const referencesPayload = { type: 'references', rootId, path: tsPath, language: 'typescript', content: tsContent, line: 1, column: 10 };
  const references = await api('/api/lsp/references', {
    method: 'POST',
    body: JSON.stringify(referencesPayload),
  });
  const referenceLines = new Set((references.locations ?? []).filter((location) => location.path === tsPath).map((location) => location.startLine));
  if (references.provider !== 'typescript' || !referenceLines.has(1) || !referenceLines.has(4) || references.locations.length < 2) {
    throw new Error(`direct TypeScript references mismatch: ${JSON.stringify(references)}`);
  }

  const gatewayReferences = await requestGatewayEvent(referencesPayload);
  const gatewayReferenceLines = new Set((gatewayReferences.locations ?? []).filter((location) => location.path === tsPath).map((location) => location.startLine));
  if (gatewayReferences.provider !== 'typescript' || !gatewayReferenceLines.has(1) || !gatewayReferenceLines.has(4)) {
    throw new Error(`LSP WebSocket TypeScript references mismatch: ${JSON.stringify(gatewayReferences)}`);
  }
  ...
  try {
    ...
    await page.locator('[data-ide-output-event]', { hasText: 'TypeScript/JavaScript hover/definition/completion/references' }).waitFor({ state: 'visible', timeout: 30_000 });
  } catch (error) {
