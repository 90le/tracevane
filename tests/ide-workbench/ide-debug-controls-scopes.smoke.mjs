...
const SMOKE_PORT = Number(process.env.TRACEVANE_WEB_PORT || 5192);
...
async function run() {
  ...
  if (!rootId || !root.absolutePath) throw new Error('No file root is available for IDE debug controls smoke');
  const smokeDir = normalizePortablePath(path.join(relativePathFromRoot(root.absolutePath, process.cwd()), '.tmp', `ide-debug-controls-scopes-${Date.now().toString(36)}`));
  const filePath = `${smokeDir}/debug-controls-target.js`;
  ...
  for (const feature of ['launch-profiles', 'launch-config-validation', 'launch-args-env-guard', 'debug-control-commands', 'debug-scopes']) {
  ...
  try {
    ...
    await page.locator('[data-ide-debug-control-step-over]').click();
    await page.locator('[data-ide-debug-console-event]', { hasText: 'Debug control stepOver' }).first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-control-continue]').click();
    await page.locator('[data-ide-debug-session][data-ide-debug-session-state="running"][data-ide-debug-session-profile="node-lite"]').first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-control-pause]').click();
    await page.locator('[data-ide-debug-session][data-ide-debug-session-state="stopped"][data-ide-debug-session-profile="node-lite"]').first().waitFor({ state: 'visible', timeout: 30_000 });
    await page.locator('[data-ide-debug-console-event]', { hasText: 'Debug control pause' }).first().waitFor({ state: 'visible', timeout: 30_000 });
