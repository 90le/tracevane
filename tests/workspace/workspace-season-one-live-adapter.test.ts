...
const live = createWorkspaceSeasonOneLiveModel({
  ...
  openFiles: ["docs/DESIGN.md", "apps/web/src/features/workspace/WorkspacePage.tsx"],
  gitChanges: 3,
  evidenceItems: 2,
...
assert.match(live.mission.resourceSummary, /2 open files · 3 Git changes · 2 evidence items/);
assert.equal(live.resources[0]?.label, "docs/DESIGN.md");
assert.ok(live.resources.some((item) => item.id === "git-live" && item.state === "review"));
assert.ok(live.resources.some((item) => item.id === "evidence-live" && item.state === "ready"));
...
assert.ok(blocked.resources.some((item) => item.id === "evidence-live" && item.state === "blocked"));
