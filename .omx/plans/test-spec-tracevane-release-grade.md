# Tracevane release-grade test spec
...
- Tracevane Gateway must expose native OpenAI Responses, native Anthropic Messages, and OpenAI Chat Completions compatibility contracts.
- Gateway daemon lifecycle must not depend on the Tracevane frontend process or OpenClaw single-port mount staying alive.
- Official provider route restore must preserve official auth backup when present.
- Switching to Tracevane Gateway must write local route settings only after the chosen path is confirmed.
...
- Mutating Tracevane Gateway operations are single-flight or otherwise guarded from duplicate execution.
...
- Tracevane Gateway status loads.
