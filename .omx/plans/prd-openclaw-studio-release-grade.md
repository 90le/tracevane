...
This goal covers frontend, backend, feature behavior, performance, compatibility, Studio Gateway routing, model and provider configuration, health checks, logs, setup recovery, CSS ownership, old feature cleanup, and release evidence.
...
- Make first install, reinstall/update, repair, health check, model validation, Studio Gateway route selection, and official provider restore work as one guided workflow.
...
- Never auto-route a CLI/tool through Studio Gateway unless the selected provider and protocol checks are fresh and passing.
- Keep official provider auth and Studio Gateway auth separately recoverable.
...
### M1. Studio Gateway release path

- First-use install, reinstall, repair, health check, smoke validation, and Studio Gateway route switching are one guided workflow.
...
- Required checks include relevant system tests, typecheck, web build, live health, and targeted Studio Gateway smoke where safe.
- Completion requires machine-readable audit evidence and explicit residual-risk review.

## Current Priority

Start with Studio Gateway because it blocks safe customer use:
...
- Studio Gateway now replaces the legacy page direction. New work must build provider, app connection, runtime, and diagnostics surfaces as fresh product surfaces.

## Remaining High-Risk Work

- Build the new Studio Gateway management surface after the legacy page and API are retired.
...
- A normal user can install, repair, validate, and choose Studio Gateway or official provider routes without understanding internal service names first.
