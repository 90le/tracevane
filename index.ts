...
const tracevanePlugin = {
  ...
  name: "Tracevane",
  description:
    "Tracevane 本地 AI Agent 控制工作台，聚焦 Gateway、Channel Connectors、CLI Agent 和运行态自愈。",
  kind: "ui" as const,
  configSchema: {
    type: "object",
    properties: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const registerGatewayMethod =
      typeof api.registerGatewayMethod === "function"
    ...
    api.on?.(
      ...
      (_event, ctx) => {
        ...
        return buildTracevaneBeforePromptBuildResult({
    ...
    api.on?.(
      ...
      (event, ctx) => {
        ...
        if (result?.block) {
          api.logger.warn(
            `tracevane: blocked tool call in Tracevane Agent Chat (tool=${event.toolName}): ${result.blockReason}`,
          );
        }
        return result;
      },
    ...
    api.on?.(
      "reply_dispatch",
      (event, ctx) => {
        return maybeHandleTracevaneReplyDispatch(
...

    {
      ...
      if (isTracevaneGatewayEnabled(config)) {
        if (!registerHttpRoute || !registerGatewayMethod) {
          api.logger.warn(
            "tracevane: gateway exposure requested but host plugin API lacks route registration; skipping gateway runtime",
          );
        } else {
          ...
          api.logger.info(
            `tracevane: gateway HTTP route registered at ${config.transport.gateway.basePath}`,
          );
          ...
          registerGatewayMethod(
            ...
            (opts) => {
              try {
                ...
                const payload = ctx.services.terminal.sendGatewayInput(params, {
    ...
    api.registerService({
      ...
      start: async () => {
        ...
        if (!isTracevaneStandaloneEnabled(config)) {
          api.logger.info(
            "tracevane: standalone exposure disabled; skipping standalone HTTP server",
          );
          return;
        }
    ...
    api.logger.info(
      "tracevane: management foundation, delivery tool, and Tracevane chat hooks registered",
    );
  },
};
