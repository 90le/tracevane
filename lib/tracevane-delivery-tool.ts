...
export function createTracevaneDeliveryTool() {
  return {
    ...
    description: [
      ...
      'Prefer assistant Markdown rich replies first for ordinary Tracevane returns, using explicit refs like files:<rootId>:<path> or tracevane-file: plus tracevane: display hints.',
      'Prefer files:<rootId>:<path> refs for newly created workspace/upload files when a File Manager / Workspace root id is available; use tracevane-file: mainly for explicit local-file compatibility, and keep workspace:/uploads: only as legacy OpenClaw/WebChat compatibility refs.',
