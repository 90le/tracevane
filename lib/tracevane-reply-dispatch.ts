...
type TracevaneReplyDispatchEvent = {
...
type TracevaneReplyDispatchContext = {
...
type TracevaneReplyDispatchResult = {
  handled: true;
  queuedFinal: boolean;
  counts: ReplyDispatchCounts;
};

type TracevaneReplyResolver = (
  ctx: Record<string, unknown>,
  opts?: {
    runId?: string;
    abortSignal?: AbortSignal;
...
let tracevaneReplyRuntimePromise: Promise<{
  getReplyFromConfig: TracevaneReplyResolver;
}> | null = null;
...
function shouldQueueTracevaneFinalReply(payload: ReplyPayload): boolean {
...
async function loadTracevaneReplyResolver(): Promise<TracevaneReplyResolver> {
  tracevaneReplyRuntimePromise ??= import('openclaw/plugin-sdk/reply-runtime') as Promise<{
    getReplyFromConfig: TracevaneReplyResolver;
  }>;
  const runtime = await tracevaneReplyRuntimePromise;
  return runtime.getReplyFromConfig;
}

function shouldHandleTracevaneReplyDispatch(event: TracevaneReplyDispatchEvent): boolean {
...
export function buildTracevaneReplyDispatchConfigOverride(
  ...
  const deny = Array.from(new Set([
    ...readDenyList(config),
    ...TRACEVANE_PRIVATE_CHAT_BLOCKED_TOOL_NAMES,
  ]));
  return {
    tools: {
      deny,
...
export async function maybeHandleTracevaneReplyDispatch(
  event: TracevaneReplyDispatchEvent,
  ctx: TracevaneReplyDispatchContext,
  replyResolver?: TracevaneReplyResolver,
): Promise<TracevaneReplyDispatchResult | undefined> {
  if (!shouldHandleTracevaneReplyDispatch(event)) {
    return undefined;
  }

  const resolvedReplyResolver = replyResolver ?? await loadTracevaneReplyResolver();
  const configOverride = buildTracevaneReplyDispatchConfigOverride(ctx.cfg);
  ...
  for (const payload of finalReplies) {
    ...
    if (!shouldQueueTracevaneFinalReply(payload)) {
      continue;
    }
  ...
  ctx.recordProcessed('completed', {
    reason: 'tracevane_reply_dispatch_restricted',
  });
