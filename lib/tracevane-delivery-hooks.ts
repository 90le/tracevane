...
export const TRACEVANE_DELIVERY_PROMPT_GUIDANCE = [
  ...
  'Prefer explicit Tracevane resource refs from the shared Files API: files:<rootId>:<path> for any resource that can be expressed by File Manager / Workspace roots; workspace: and uploads: are legacy compatibility refs only.',
  'For newly created workspace or upload files, prefer files:<rootId>:<path> when a Files root id is available; use tracevane-file: mainly for explicit local files that cannot be expressed through Files API roots, and use workspace:/uploads: only for old OpenClaw/WebChat compatibility.',
  ...
  'Primary Markdown examples: [Diagram](files:project-root:diagram.png "tracevane:break-image"), [Demo](files:project-root:uploads/demo.mp4 "tracevane:break-video"), [Package](tracevane-file:./report.pdf "tracevane:card"), [Attachment](files:project-root:report.pdf "tracevane:inline-chip").',
  'Primary raw HTML examples: <img src="files:project-root:diagram.png" title="tracevane:break-image" alt="Diagram">, <video src="files:project-root:uploads/demo.mp4" title="tracevane:break-video"></video>, <a href="tracevane-file:./report.pdf" title="tracevane:card">Package</a>, <img src="files:project-root:thumb.png" data-tracevane-display="inline-image" alt="Thumb">.',
  ...
  'Do not prefer bare ./graph.png style paths when files:<rootId>:<path>, tracevane-file:, or a legacy workspace:/uploads: compatibility ref can express the intent more clearly.',
