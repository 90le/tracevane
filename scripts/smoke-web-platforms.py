#!/usr/bin/env python3
"""Browser smoke for the rebuilt apps/web Platform Integrations.

Loads the platform overview (/platforms) and the lightweight OpenClaw platform
summary (/platforms/openclaw) at desktop + mobile widths against the dev
server, captures console errors / uncaught page errors, and checks for
horizontal overflow. Exits non-zero on any failure.

Usage: python3 scripts/smoke-web-platforms.py [--base http://127.0.0.1:5176]
"""
import sys
...
if failures:
    ...
    for f in failures:
        print("  -", f)
    sys.exit(1)
print("SMOKE PASSED — both surfaces render, no console errors, no overflow.")
