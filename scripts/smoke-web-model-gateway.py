#!/usr/bin/env python3
"""Browser smoke for the rebuilt apps/web Model Gateway.

Loads each Model Gateway data-view at desktop + mobile widths against the dev
server, captures console errors / uncaught page errors, and checks for
horizontal overflow. Exits non-zero on any failure.

Usage: python3 scripts/smoke-web-model-gateway.py [--base http://127.0.0.1:5176]
"""
import sys
...
VIEWS = ["overview", "providers", "providercfg", "models", "accounts", "apps", "usage"]
...
with sync_playwright() as p:
    ...
    for size_name, w, h in SIZES:
        ...
        for view in VIEWS:
            errors.clear()
            url = f"{BASE}/#/model-gateway?view={view}"
