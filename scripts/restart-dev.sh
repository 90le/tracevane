...
backend_pid="$(start_background "STUDIO_API_PORT=$BACKEND_PORT npm run dev:api" "$BACKEND_LOG_FILE" "$BACKEND_PID_FILE")"
...
frontend_pid="$(start_background "STUDIO_USE_EXTERNAL_API=1 STUDIO_API_PORT=$BACKEND_PORT STUDIO_WEB_PORT=$FRONTEND_PORT npm run dev:web" "$FRONTEND_LOG_FILE" "$FRONTEND_PID_FILE")"
