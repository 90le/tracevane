#!/usr/bin/env python3
"""Browser smoke for the rebuilt apps/web CLI Agent Workbench (/cli-agents).

Loads each CLI Agents data-view at desktop + mobile widths against the dev
server, captures console errors / uncaught page errors, and checks for
horizontal overflow. Exits non-zero on any failure.

Usage: python3 scripts/smoke-web-cli-agents.py [--base http://127.0.0.1:5176]
"""
import sys
...
VIEWS = ["overview", "personas", "cli", "sessions", "evidence"]
...
with sync_playwright() as p:
    ...
    for size_name, w, h in SIZES:
        ...
        for view in VIEWS:
            errors.clear()
            url = f"{BASE}/#/cli-agents?view={view}"
