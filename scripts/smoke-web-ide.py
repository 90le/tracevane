#!/usr/bin/env python3
"""Browser smoke for the rebuilt apps/web Workspace IDE read workbench (/ide).
...
asserts the live read APIs (/api/files/browse + /api/git/status) return data
...
try:
    ...
    if root_id:
        ...
        print(f"api: /api/git/status branch={status.get('branch')} available={status.get('available')} changes={len(status.get('changes', []))}")
...
with sync_playwright() as p:
    ...
    for size_name, w, h in SIZES:
        ...
        print(f"{tag:<22} rootLen={root_len:<5} overflow={overflow:<4} errors={len(errors)}  {status}")
        ctx.close()
    browser.close()
...
if failures:
    ...
    for f in failures:
        print("  -", f)
    sys.exit(1)
print("SMOKE PASSED — /ide renders, live read APIs return data, no console errors, no overflow.")
