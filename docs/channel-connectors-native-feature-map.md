...
- 已完成：F3e IM command control + native passthrough：`/help`、`/command`、`/cmd`、`/status`、`/usage`、`/tokens`、`/agent`、`/model`、`/mode`、`/reasoning`、`/effort`、`/name`、`/rename`、`/search`、`/find`、`/dir`、`/cd`、`/compact`、`/new`、`/reset`、`/stop`、`/display`、`/stream`、`/tools`、`/alias`；session override 独立存储，`/mode yolo`、`/reasoning high` 等只作用于当前 IM session；`/reasoning` 按 CC Go 支持序号和 `low|medium|high|xhigh|default`，切换后断开旧 Agent 续接，Codex/Claude Code/OpenCode runner 分别写入 `model_reasoning_effort`、`--effort`、`--variant`；`/name` 与 `/search` 管理当前 IM session 的本地 Agent sessions；未知 `/xxx` 默认透传给当前 Agent，冲突命令用 `/native <命令>`；显式 `/native` 会进入 runner 的 `nativeCommand` 模式，不注入 IM history/group/attachment policy；Codex `/help`/`/version` 映射为真实 CLI 参数，非交互 `exec` 不可靠 slash 明确拒绝；Claude Code/OpenCode 普通原生命令进入对应 CLI 输入；`/native /compact`/`/native /compress` 是强制原生 compact contract，没有 live persistent compact session 时拒绝伪透传；`/compact` 走 native-first，失败或不支持时用 Studio Gateway `/responses/compact` 生成当前 IM history summary，并断开旧 Agent 续接；`/usage` 从 Gateway runtime usage ledger 汇总当前 IM session 最近 Agent run 的真实 token usage；`/model` 后继续复用同一 Codex thread，`/cd` 和 `/new` 会断开旧续接；`/dir` 按 CC Go 支持最近目录、`/dir -` 和历史序号切换；`/stream` 与 `/tools` 分别控制 IM 中间态和工具/思考消息；`/alias add/del/list` 管理 binding 级命令别名；`/stop` 按当前 binding + IM session 取消最新 active CLI Agent run。
...
- 已完成：持久 TUI/session driver 最小 pool contract、daemon 状态面和 Codex app-server driver 实验路径：默认不替代 one-shot `exec/resume`；metadata 请求 persistent 的 binding 会在 `/status` / runtime 中显示；Codex 已覆盖 `thread/start`、`turn/start`、`turn/interrupt`、`thread/compact/start` 的 fake transport 合同，并通过真实 Studio Gateway `turn/start`、原生 `/compact`、`turn/interrupt` live smoke；IM `/stop` 已通过 daemon fake app-server 回归。
