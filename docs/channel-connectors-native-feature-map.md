...
- 已完成：F4 Studio 原生出站消息合同：Agent 可声明 `studio-channel-messages` manifest；Octo 已支持 `dm:user_uid`、`group:group_no`、`thread:group____shortId`、`mentionUids`、`mentionAll`、`onBehalfOf/on_behalf_of/respondAs` 和 content 内 `@[uid:显示名]` 自动 mention 抽取；发送前会转为可见 `@显示名` 并附带 Octo `mention.entities/uids`，transport 层也会兜底把只有 mention metadata 的消息补成可见 `@uid`，persona/OBO 身份会按插件合同发送顶层 `on_behalf_of`，包括普通文本、typing、文件/媒体和 manifest 消息；Octo DM 只用于 human UID，`dm:<*_bot>` 在群/thread 内会重写为当前群/thread @，私聊场景会拒绝并提示改用群/thread mention 或 Studio 内部 agent routing；Agent 群上下文会展示成员 bot/human 类型和正确协作规则，避免误称缺少 Feishu/Octo API 权限；Feishu 已按 OpenClaw target 合同支持 `chat:oc_xxx`、`open_id:ou_xxx`、`user_id:u_xxx`、`dm:ou_xxx/u_xxx` 文本与 Markdown(post) 目标并映射官方 `receive_id_type`。daemon 会剥离机器协议、发送消息、记录 declared/sent/request/errors；`agent-run-live --require-outbound-message` 已能验证 declared/sent 真实日志，最近 24h Octo 群 outbound-message 命中 8 条成功 run；真实 Octo human DM、thread、@其它 Studio/外部 bot 和 Feishu open_id/user_id/Markdown live smoke 待做。
- 已完成：F4 IM history context：按 session 保存最近 user/assistant 脱敏摘要，Agent prompt 注入短上下文；本地 IM history 已标为 previous-turn context，禁止复答旧消息或沿用旧能力误判；`/compact` 将 history 替换为 `compact-summary`，`/new` / `/reset` 清理 history。
