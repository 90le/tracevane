...
- `smoke-channel-connectors-agent-run-live.mjs` 已新增 `--require-tool-output`，真实 IM smoke 可验证工具结果不是空占位。
...
2. Feishu/Octo 私聊 live smoke：继续出站文件、权限审批、Octo 视频和 compact 抽查；入站文件/图片和 Feishu 入站视频已进入 staged-path 验收。
