...
- 已完成：F4 Studio 原生出站消息合同：Agent 可声明 `studio-channel-messages` manifest；Octo 已支持 `dm:user_uid`、`group:group_no`、`thread:group____shortId`、`mentionUids` 和 `mentionAll`；Agent 群上下文会展示 Octo Bot API 成员列表并提示 `@[uid:显示名]` 语法，避免误称缺少 Feishu/Octo API 权限；Feishu 先支持显式 `chatId` 文本发送。daemon 会剥离机器协议、发送消息、记录 declared/sent/request/errors；真实 Octo 私聊其它 bot/成员与群 @ live smoke 待做。
...
- 下一步：先做 Octo `/octo groups/members/search` 与 `studio-channel-messages` 私聊/@群成员 live smoke；继续按 Octo 插件迁移 `octo_management` 剩余建群/改群/加减成员、thread、GROUP.md/thread MD/persona context 和菜单；随后按同一模式补 Feishu open_id/user_id 私聊、富文本、文档/云盘 skills 和更多平台 adapter。
