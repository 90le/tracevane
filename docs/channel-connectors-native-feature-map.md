...
- 已完成：持久 TUI/session driver 最小 pool contract、daemon 状态面和 Codex app-server driver 原型：默认不替代 one-shot `exec/resume`；metadata 请求 persistent 的 binding 会在 `/status` / runtime 中显示，但 effective mode 仍明确为 one-shot；Codex 原型已覆盖 `thread/start`、`turn/start`、`turn/interrupt`、`thread/compact/start` 的 fake transport 合同，下一步做 live Gateway smoke 后再接 daemon 实验路径。
