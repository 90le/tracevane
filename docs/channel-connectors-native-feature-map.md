...
- Feishu daemon 已有 native-first wiring 回归；真实长连接 auto compact 已用 event log smoke 证明进入 Agent-native compact；Codex 与 Claude Code 显式 `/compact` live 已验证。
...
2. Feishu 显式 `/compact` live smoke：继续验证 OpenCode。
