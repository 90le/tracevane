...
- Feishu 菜单命令面已改为清爽二级配置架构：主卡片直达 Agent、模型、权限/推理、进度显示、视觉、工作目录六个配置页，只保留 `/new`、`/compact`、`/stop` 和“更多”；视觉/显示页使用单按钮切换当前状态；会话详情、缓存、命令/Skills、Agent 原生命令等低频项只在二级页出现，文本 `/help` 复用同一结构。
