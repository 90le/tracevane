...
- 已完成：F4 Studio 原生出站消息合同：Agent 可声明 `studio-channel-messages` manifest；Octo 已支持 `dm:user_uid`、`group:group_no`、`thread:group____shortId`、`mentionUids`、`mentionAll` 和 content 内 `@[uid:显示名]` 自动 mention 抽取；Agent 群上下文会展示 Octo Bot API 成员列表、bot/human 类型和私聊/@协作能力，避免误称缺少 Feishu/Octo API 权限；Feishu 先支持显式 `chatId` 文本发送。daemon 会剥离机器协议、发送消息、记录 declared/sent/request/errors；真实 Octo 私聊其它 bot/成员与群 @ live smoke 待做。
