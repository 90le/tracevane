...
- 已完成：F3f Feishu daemon 长连接基础合同：使用官方 SDK `WSClient` / `EventDispatcher` 接 `im.message.receive_v1`、`card.action.trigger`、`application.bot.menu_v6`；按 CC/官方集群推送约束，同一 Feishu App 多 binding 共享单条 WS，本机多进程用用户级全局 owner lock；支持 chatId 过滤、thread/root 字段保留、command-router 回复和 Agent runner 回包；按最新 OpenClaw 外层循环处理 terminal error 并 `1s..30s` 退避重建；默认不启用当前 SDK lower-case `pingTimeout` 额外 watchdog；SDK `reconnecting` 超过 10s 时回收到外层重建；connected-idle / zero-inbound / verified-ingress / generic watchdog 重建关闭；启动期无 raw event / dispatcher callback 时执行 15s 起步、最多 5 次 bounded ingress validation；`connected` 与真实入站 `ingressState`、raw event、global/lifecycle `dispatcherCallbacks`、business `receivedMessages` 分离；`/health` 会把 `silent` owner 视为不健康。假在线问题进入 `feishu-long-connection-issue-tracker.md` 专项，不以重启自愈作为完成标准。
