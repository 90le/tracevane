...
- 已完成：Feishu card/menu/progress loop：Session、Agent、Model、Permission、WorkDir、Display 子卡片，文本 slash 与卡片点击共用 command-router；卡片无卡片执行动作异步发送普通文本结果且不弹 toast，导航动作刷新卡片；Agent 运行保留 processing reaction，支持单张 Progress card send/patch、Codex `command_execution` 工具过程展示、`/stream` 与 `/tools` 控制，以及 upstream JSON error envelope 清洗和失败去重。
