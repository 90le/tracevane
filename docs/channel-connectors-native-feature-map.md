...
- 已完成：F3f Feishu daemon 长连接基础合同：使用官方 SDK `WSClient` / `EventDispatcher` 接 `im.message.receive_v1`、`card.action.trigger`、`application.bot.menu_v6`；按 CC/官方集群推送约束，同一 Feishu App 多 binding 共享单条 WS，本机多进程用 owner lock；支持 chatId 过滤、thread/root 字段保留、command-router 回复和 Agent runner 回包；默认 60s SDK half-open timeout，connected-idle / zero-inbound 业务层强制重建关闭；`connected` 与真实入站 `ingressState` 分离。假在线问题进入 `feishu-long-connection-issue-tracker.md` 专项，不以重启自愈作为完成标准。
