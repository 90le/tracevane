...
- 已完成：F3e IM command control + native passthrough：`/help`、`/command`、`/cmd`、`/status`、`/agent`、`/model`、`/mode`、`/dir`、`/cd`、`/compact`、`/new`、`/reset`、`/stop`、`/display`、`/stream`、`/tools`；session override 独立存储，`/mode yolo` 等只作用于当前 IM session；未知 `/xxx` 默认透传给当前 Agent，冲突命令用 `/native <命令>`；显式 `/native` 会进入 runner 的 `nativeCommand` 模式，不注入 IM history/group/attachment policy；Codex `/help`/`/version` 映射为真实 CLI 参数，非交互 `exec` 不可靠 slash 明确拒绝；Claude Code/OpenCode 原生命令进入对应 CLI 输入；`/compact` 走 Studio Gateway `/responses/compact` 生成当前 IM history summary，并断开旧 Agent 续接；`/model` 后继续复用同一 Codex thread，`/cd` 和 `/new` 会断开旧续接；`/dir` 按 CC Go 支持最近目录、`/dir -` 和历史序号切换；`/stream` 与 `/tools` 分别控制 IM 中间态和工具/思考消息；`/stop` 按当前 binding + IM session 取消最新 active CLI Agent run。
