...
- 已完成：F3f Feishu daemon 长连接基础合同：使用官方 SDK `WSClient` / `EventDispatcher` 接 `im.message.receive_v1`、`card.action.trigger`、`application.bot.menu_v6`；按 CC/官方集群推送约束，同一 Feishu App 多 binding 共享单条 WS，本机多进程用用户级全局 owner lock；支持 chatId 过滤、thread/root 字段保留、command-router 回复和 Agent runner 回包；按最新 OpenClaw 外层循环处理 terminal error 并 `1s..30s` 退避重建；默认启用当前 SDK lower-case `pingTimeout=3` liveness watchdog，并包装 SDK `pingLoop()` 把有效心跳调度压到 `pingIntervalMs=30000`；SDK `reconnecting` 超过 10s 时回收到外层重建；SDK 继续发 ping 但超过 `pongTimeoutMs=15000` 未收到 pong 时回收当前 client；若控制帧超过 55s 未更新，`transportStale` 会让 `/health` 和前端判定不健康并回收；connected-idle / zero-inbound / verified-ingress / generic watchdog 重建关闭；raw event / dispatcher callback 只作为业务入站诊断，默认不因空闲期无业务消息重建；`connected` 与真实入站 `ingressState`、raw event、global/lifecycle `dispatcherCallbacks`、business `receivedMessages`、pong overdue、transport stale 分离。假在线问题进入 `feishu-long-connection-issue-tracker.md` 专项，不以重启自愈作为完成标准。
