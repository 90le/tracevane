...
- 已完成：F3e IM command control + native passthrough：`/help`、`/command`、`/cmd`、`/status`、`/usage`、`/tokens`、`/agent`、`/model`、`/mode`、`/reasoning`、`/effort`、`/name`、`/rename`、`/search`、`/find`、`/dir`、`/cd`、`/compact`、`/new`、`/reset`、`/stop`、`/display`、`/stream`、`/tools`；session override 独立存储，`/mode yolo`、`/reasoning high` 等只作用于当前 IM session；`/reasoning` 按 CC Go 支持序号和 `low|medium|high|xhigh|default`，切换后断开旧 Agent 续接，Codex/Claude Code/OpenCode runner 分别写入 `model_reasoning_effort`、`--effort`、`--variant`；`/name` 与 `/search` 管理当前 IM session 的本地 Agent sessions；未知 `/xxx` 默认透传给当前 Agent，冲突命令用 `/native <命令>`；显式 `/native` 会进入 runner 的 `nativeCommand` 模式，不注入 IM history/group/attachment policy；Codex `/help`/`/version` 映射为真实 CLI 参数，非交互 `exec` 不可靠 slash 明确拒绝；Claude Code/OpenCode 原生命令进入对应 CLI 输入；`/compact` 走 Studio Gateway `/responses/compact` 生成当前 IM history summary，并断开旧 Agent 续接；`/usage` 从 Gateway runtime usage ledger 汇总当前 IM session 最近 Agent run 的真实 token usage；`/model` 后继续复用同一 Codex thread，`/cd` 和 `/new` 会断开旧续接；`/dir` 按 CC Go 支持最近目录、`/dir -` 和历史序号切换；`/stream` 与 `/tools` 分别控制 IM 中间态和工具/思考消息；`/stop` 按当前 binding + IM session 取消最新 active CLI Agent run。
...
- 已完成：F3f Feishu daemon 长连接：使用官方 SDK `WSClient` / `EventDispatcher` 接 `im.message.receive_v1`、`card.action.trigger`、`application.bot.menu_v6`；按 CC 约束同一 Feishu App 多 binding 共享单条 WS；支持 chatId 过滤、thread/root 字段保留、command-router 回复和 Agent runner 回包；SDK `pingTimeout=10s` 捕获真实死 socket，reconnecting/reconnected 会落日志，watchdog 在非 connected 超过 45s 后兜底重建；启动 30s 无入站做一次 no-inbound sanity reconnect，当前生命周期收过事件后默认静默超过 5 分钟才做 low-frequency idle refresh，减少频繁重连和平台重投。
