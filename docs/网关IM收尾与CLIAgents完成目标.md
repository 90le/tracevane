...
## 11. 2026-06-25 操作台化二次优化

继续按“不要像介绍页面，而是实际高价值功能”收敛：

- 删除运行台底部“边界说明”面板，替换为“待处理操作”。
- “待处理操作”只展示失败、可停止、可删除的 Run，直接提供停止、删除、打开 owner 操作。
- 删除启动 / 修复页底部“依赖引用”面板，替换为“修复队列”。
- “修复队列”只展示阻断项：缺 CLI、Gateway 不可用、PTY 不可用，并提供安装、复制提示、打开网关、打开 IDE。
- 启动 / 修复主表的第三列从“安装 / 修复提示”改为“下一步”，直接显示动作链。

原则：域边界与产品解释放文档；用户界面只放状态、动作、结果、证据。
