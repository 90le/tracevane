...
> 更新：2026-06-17
> 文档规则：本文件只保留目标、边界、验收和阶段计划；当前进度写到 `tracevane-gateway-progress.md`。
...
- 历史代理端口和旧代理服务都不再是正式模型链路；`18796` 只可作为 Tracevane Gateway daemon 默认本地端口。
- IM Channel Connectors 是独立产品域，不属于模型 relay，也不放入 App Connections。

## 2. 协议目标

Tracevane Gateway 必须让任意 provider 无论原生协议是什么，都能对外暴露三类常见客户端协议：
...
- Gateway key 是本地统一鉴权 key，可由用户编辑或生成；真实 upstream key 留在 Tracevane secret store，不透传给客户端。
...
GPT / ChatGPT / Codex 账户接入是 Gateway 下一阶段目标，参考 `docs/tracevane-gateway-account-provider-plan.md`。
...
- 单口模式：OpenClaw 只挂载 Tracevane UI / control API；模型请求默认仍写 daemon loopback，单口 endpoint 只作为可选 ingress/proxy。
- OpenClaw Gateway、Tracevane API 或 Tracevane UI 崩溃时，Gateway daemon 继续服务已接管客户端。
...
- OpenClaw Agent 管理不承载 Tracevane Gateway / IM / CLI Profile 主线；长期目标是 Tracevane 自建 Gateway + Channel Connectors 可脱离 OpenClaw 独立运行。
...
- 上下文预算和 Agent-native compact 优先，Tracevane/Gateway compact 兜底。

停止扩展：

- active `tracevane-channel-skill`、Feishu/Octo platform action、文档/群/管理类 API 工具层。
...
- 核验结论、拒绝方案、风险和验证计划写入 `docs/research-first-development-checklist.md`、本文件、`tracevane-gateway-progress.md` 或专项 tracker。
...
- Channel Connectors 私聊链路可在 Tracevane/OpenClaw 崩溃后继续由 daemon + Gateway 提供服务。
- 生产前后端无用户可见的旧模型链路入口。
...
| 阶段 | 状态 | 目标 |
| --- | --- | --- |
| Phase A | 完成 | 固定 Tracevane Gateway 命名、API contract、旧链路删除清单 |
