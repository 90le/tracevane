...
> 2026-06-25 IA correction: the live product is no longer planned as many equal admin pages. Model Gateway / IM Channels / CLI Agents are the three core operating domains. `External` and `Long Tasks` remain compatibility deep-links only: external capability evidence is a Platform/support concern, and supervised work is a CLI Agents/owner-domain projection rather than a primary domain.
...
| Chat / Workspace / Recovery / Platform | Local chat, IDE/files/terminal/Git evidence, service recovery, substrate support and integration evidence. | Replacing the three core domains above. |
...
- Do not promote read-only aggregation pages (`/external`) or cross-domain projections (`/long-tasks`) to core navigation domains.
