...
- IM Agent runner 策略固定为混合架构：默认 one-shot `exec/resume` 保稳定；Codex 持久 session driver 已接入 metadata 实验路径，显式开启时使用 `codex app-server`，失败回退 one-shot。
...
- 持久 session driver 当前只对 Codex metadata 实验路径开放；真实 Codex app-server 已验证 `initialize/thread-start`，daemon fake app-server 已验证 `turn/start` 接线，但尚未完成真实 Codex app-server + Studio Gateway 模型调用、`/compact`、`turn/interrupt` 端到端 smoke。
- Feishu 历史未 ACK 事件可能仍会被平台重投一次；持久化去重会记录并跳过，最终仍需用户发送全新消息复验 live 回复。

## 下一步

1. 做真实 Codex app-server + Studio Gateway 隔离 HOME turn smoke，验证 `turn/start` 的模型调用和 progress 映射。
2. 验证真实 Codex persistent `/compact` 与 `turn/interrupt`，并把 `/stop` 的 session stop/kill 行为纳入 live smoke。
