...
- 通过：`node --test tests/system/channel-connectors-service.test.mjs`，53 个 Channel Connectors 子测试通过；覆盖 Codex resume 参数顺序、Feishu/Octo 文件收发、Feishu transport-smoke 文件发送入口、Agent/config 自定义命令扫描/展开/添加/删除、进度/工具事件和 daemon 合同。
