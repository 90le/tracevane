...
- 本轮继续增强 active route smoke preflight：脚本会在 provider missing / disabled / scope mismatch 时输出结构化 `preflightFailures`，并且不写 active-provider；当前本机 `gmn` 诊断为 `model_gateway_provider_disabled`，`routeSmokes=[]`、`restoreMismatches=[]`，复查 `/providers` 仍为 `activeProviders={}`。GMN 后续必须先启用并确认上游不再 502 后，才可声明三协议通过。
