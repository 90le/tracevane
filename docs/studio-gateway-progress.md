...
- IM Agent runner 策略固定为混合架构：默认 one-shot `exec/resume` 保稳定；持久 TUI/session driver 已有 pool 合同和 daemon 状态面，后续只在显式开启时逐 Agent 接入真实 driver。
...
- 持久 session driver 目前是可测试合同、pool 基础设施和 daemon 状态面；尚未接入 daemon 默认运行路径，也没有真实 Codex/Claude/OpenCode TUI/PTY driver。
- Feishu 历史未 ACK 事件可能仍会被平台重投一次；持久化去重会记录并跳过，最终仍需用户发送全新消息复验 live 回复。

## 下一步

1. 做 Codex 持久 driver 原型：原生 slash、连续流、Agent-side compact、crash 后回退 one-shot。
2. 把真实 persistent pool session 显示到 daemon `/status`，并增加 stop/kill/status runtime 回归。
