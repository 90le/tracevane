...
- Octo 大文件 COS 直传已接入：`transport-smoke` 支持 STS 探测、COS PUT 直传和 direct upload + send media；返回脱敏 bucket/region/key/cdn/credential fields，不暴露临时密钥。

## 最近验证

- 通过：`npm run build:api`。
- 通过：`node --test tests/system/model-gateway-service.test.mjs`，51 个 Model Gateway 子测试通过。
- 通过：`node --test tests/system/channel-connectors-service.test.mjs`，44 个 Channel Connectors 子测试通过。
...
- 出站文件基础链路已覆盖小/中型本地文件，Octo 已具备 COS 直传；高级 `yolo` 权限仅放宽本地路径根限制，不绕过平台上传限制。后续仍需做自动选择 multipart/direct upload、真实大文件限额和更多平台文件收发实测。
...
3. 继续迁移 Claude Code / OpenCode 视觉输入、OCR、语音/STT/TTS、文件上传自动策略和更多平台 adapter。
