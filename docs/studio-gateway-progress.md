...
- 平台 skill 默认来源已切为 Studio 内置 registry：Octo/Feishu skill 随 Studio 发布并进入 `/skills`、普通 IM prompt 和 Codex/Claude 原生 skill 投影；默认不扫描 `~/.openclaw/extensions/octo/skills` 或 OpenClaw Feishu 扩展目录。Feishu doc/drive/perm/wiki 已新增 `studio-feishu-actions` manifest：支持 doc `read/list_blocks/get_block`、drive `list/info`、perm `list`、wiki `spaces/nodes/get/search`，daemon 使用当前 Feishu binding 执行并返回结构化结果；写文档、建目录、移动/删除、权限增删和 wiki mutation 先显式阻断，避免 Agent 伪造远端执行成功。
