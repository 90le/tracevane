...
- 自动 channel skill 支持分两层：普通 IM turn 会给 Codex/Claude Code/OpenCode 注入 Studio runtime skill 摘要和 Runtime Action Index；同时 Codex `CODEX_HOME/skills` 与 Claude `CLAUDE_CONFIG_DIR/skills` 会收到当前渠道 platform skill 的 Studio runtime 投影。平台默认 skills 由 Studio 内置管理（Octo `octo-bot-api`；Feishu `messaging/doc/drive/perm/wiki`），binding `channelSkillDirs` 仅作为显式扩展；OpenClaw/Octo 插件 skills 只作参考源，不再是运行时默认来源。
...
- 平台 skill 默认来源已切为 Studio 内置 registry：Octo/Feishu skill 随 Studio 发布并进入 `/skills`、普通 IM prompt 和 Codex/Claude 原生 skill 投影；默认不扫描 `~/.openclaw/extensions/octo/skills` 或 OpenClaw Feishu 扩展目录。Feishu doc/drive/perm/wiki 先作为 Studio-owned action catalog 注入，并明确没有 runtime tool 结果时不得伪造远端执行成功。
...
- Feishu `doc/drive/perm/wiki` skills 已由 Studio 自管并能进入 Agent prompt/native skill 投影，但当前还是 action catalog；真正读写飞书文档、云盘、权限和知识库需要后续实现 Studio native Feishu tool/API 执行器。
...
3. 对照 Octo/Feishu 插件继续迁移平台 native tools：优先补 Feishu doc/drive/perm/wiki 执行器，再做更多 Octo 菜单和多 bot 协作 live smoke。
