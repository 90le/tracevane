...
- 自动 channel skill 支持分两层：普通 IM turn 会给 Codex/Claude Code/OpenCode 注入 Studio runtime skill 摘要和 Runtime Action Index；同时 Codex `CODEX_HOME/skills`、Claude `CLAUDE_CONFIG_DIR/skills`、OpenCode 隔离 `opencode.json.instructions` 都会收到当前渠道 platform skill 的 Studio runtime 投影。平台默认 skills 由 Studio 内置管理（Octo `octo-bot-api`；Feishu `messaging/doc/drive/perm/wiki`），binding `channelSkillDirs` 仅作为显式扩展；Feishu doc/drive/perm/wiki 已通过 `studio-feishu-actions` 支持只读执行，mutation 待审批型执行器；OpenClaw/Octo 插件 skills 只作参考源，不再是运行时默认来源。
...
- 平台 skill 默认来源已切为 Studio 内置 registry：Octo/Feishu skill 随 Studio 发布并进入 `/skills`、普通 IM prompt，以及 Codex/Claude/OpenCode 的 runner 原生配置投影；默认不扫描 `~/.openclaw/extensions/octo/skills` 或 OpenClaw Feishu 扩展目录。Feishu doc/drive/perm/wiki 已新增 `studio-feishu-actions` manifest：支持 doc `read/list_blocks/get_block`、drive `list/info`、perm `list`、wiki `spaces/nodes/get/search`，daemon 使用当前 Feishu binding 执行并返回结构化结果；写文档、建目录、移动/删除、权限增删和 wiki mutation 先显式阻断，避免 Agent 伪造远端执行成功。
...
- 通过：`node --test --test-name-pattern "extracts Feishu action manifests|executes Feishu read-only actions|native Channel Connectors IM commands switch agent, model, and permission per session" tests/system/channel-connectors-service.test.mjs`，覆盖 `studio-feishu-actions` 解析、Feishu wiki 只读 action 使用 tenant token 执行、mutation 阻断、Runtime Action Index 注入 Codex 原生 skill 投影、IM channel skill context、Studio 内置 Octo/Feishu skill 默认发现和外部 OpenClaw plugin skill 目录不再作为默认来源。
...
- Feishu `doc/drive/perm/wiki` skills 已由 Studio 自管并能进入 Agent prompt/native skill 投影；只读 action 已接入 `studio-feishu-actions`，但写文档、建目录、移动/删除、权限增删和 wiki mutation 仍需审批型 Studio native executor。
...
3. 对照 Octo/Feishu 插件继续迁移平台 native tools：优先给 Feishu doc/drive/perm/wiki mutation 动作补审批型执行器和 live smoke，再做更多 Octo 菜单和多 bot 协作 live smoke。
