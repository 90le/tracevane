...
- 通过：`node --test tests/system/channel-connectors-service.test.mjs`，54 个 Channel Connectors 子测试通过；覆盖 Codex resume 参数顺序、Feishu/Octo 文件收发、Feishu transport-smoke 文件发送入口、Agent/config 自定义命令扫描/展开/添加/删除、Skill 扫描/调用、Commands 菜单卡片、Claude Code stream-json 进度、图片输入与 `--resume` 续接、进度/工具事件和 daemon 合同。
