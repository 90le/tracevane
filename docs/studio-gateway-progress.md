...
- Claude Code / OpenCode native compact 已覆盖 driver 层、Octo daemon 私聊入口、Feishu native-first wiring 和 Feishu 真实长连接 auto compact；Feishu 显式 `/compact` 已验证 Codex 与 Claude Code，仍需 OpenCode live smoke。
...
2. 做 Feishu 显式 `/compact` live smoke 的 OpenCode 轮次，证明三个 Agent 的 slash 命令真实入口都走 persistent driver 而不是 one-shot 或 Gateway fallback；可用 `scripts/smoke-channel-connectors-feishu-compact-live.mjs --mode explicit --agent opencode --wait --json` 等待用户真实消息。
