...
- 新增 `smoke:channel-connectors:native-cli-sessions`：隔离 HOME + mock Studio Gateway，直接调用 daemon 同款 native persistent driver，覆盖 Claude Code 普通 turn、Claude Code `/compact`、OpenCode 普通 turn、OpenCode `/compact`。
...
- 通过：`node scripts/smoke-channel-connectors-native-cli-sessions.mjs --strict --json`，本机真实 Claude Code / OpenCode 均通过普通 turn + 原生 compact；OpenCode 命中 `opencode.db` fallback 或 stdout JSONL 路径。
...
- Claude Code/OpenCode 普通 turn + `/compact` 已有真实 CLI mock-Gateway smoke；工具、文件、权限、视觉输入和 IM live 链路仍需逐项验收。
...
1. 扩展真实 Claude Code / OpenCode persistent smoke：工具/权限、文件输入输出、视觉输入、`/stop` 和 IM live 链路，并确认 one-shot 默认路径不受影响。
