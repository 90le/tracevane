...
- 上下文管理策略固定为 native-first：Gateway/Channel 负责模型预算与触发决策；`/compact` 手动入口已在 daemon 中优先尝试 live persistent Agent 原生 compact；Studio Gateway `/responses/compact` 只作为不支持、失败或 one-shot 不可靠时的兜底。
...
- Detect Provider 会从常见 `/models` 字段识别上下文/输出预算，例如 `context_length`、`contextWindow`、`max_output_tokens`、`maxOutputTokens`。
...
- `/status` 已能显示上下文预算；`/compact` 手动入口已 native-first，但还不会根据预算自动触发。
