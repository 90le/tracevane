...
- CLI heartbeat 风险评估：单纯“有 stdout/stderr”只能证明 CLI 仍有 liveness，不能证明模型/工具有真实进展；因此 runner 新增 `process/heartbeat-stall` 诊断层，持续只有 TUI 心跳但没有结构化进展时写入 progress/event log 和 active run 状态。该诊断是 `running` 非终态，不刷新 heartbeat timeout，也默认不发到 Feishu/Octo 过程消息，避免自我续命和 IM 刷屏。
...
- Gateway account provider 初版：
  ...
  - 进一步降低 heartbeat 策略风险：runner 区分 CLI liveness 与结构化进展，持续只有心跳时发出 `process/heartbeat-stall` 诊断；诊断不会终止任务、不会重置 heartbeat timeout、默认不推送到 IM 过程消息，只作为 event log / active run 判据。
