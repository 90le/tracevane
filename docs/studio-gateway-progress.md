...
1. 继续加固 Gateway 稳定性：endpoint profile fallback、同模型多端点优先级、协议优选/回退和错误 envelope 已有本地回归；下一步转向真实 Codex account、GLM/GMN 等 provider 在 Codex / Claude Code / OpenCode 客户端协议下的 smoke。
