...
- 自动 channel skill 支持分两层：普通 IM turn 会给 Codex/Claude Code/OpenCode 注入 Studio runtime skill 摘要和 Runtime Action Index；同时 Codex `CODEX_HOME/skills`、Claude `CLAUDE_CONFIG_DIR/skills`、OpenCode 隔离 `opencode.json.instructions` 都会收到当前渠道 platform skill 的 Studio runtime 投影。平台默认 skills 由 Studio 内置管理（Octo `octo-bot-api`；Feishu `messaging/doc/drive/perm/wiki`），同名内置 skill 优先于 binding 外部目录；binding `channelSkillDirs` 仅作为显式自定义扩展；Feishu doc/drive/perm/wiki 已通过 `studio-feishu-actions` 支持只读执行和首批审批型 mutation；OpenClaw/Octo 插件 skills 只作参考源，不再是运行时默认来源。
...
- Platform skill 自动映射补 Runtime Action Index：从 Studio 内置 Octo/Feishu skill 标题抽取最多 16 个运行时动作，放入 IM prompt 与 Codex/Claude 原生 skill 投影；Feishu 文档类 skills 保留 OpenClaw action catalog，但显式标注 doc/drive/perm/wiki native tool 仍待接入。
...
- Feishu `doc/drive/perm/wiki` skills 已由 Studio 自管并能进入 Agent prompt/native skill 投影；`studio-feishu-actions` 已支持只读 action 与首批审批型 mutation。仍未完成的是 Feishu doc 内容写入、append/insert/update/delete block、表格和图片/附件上传，需要继续迁移 OpenClaw docx markdown/block 转换管线。
...
3. 对照 Octo/Feishu 插件继续迁移平台 native tools：优先补 Feishu docx 内容 mutation 管线和 live smoke，再做更多 Octo 菜单和多 bot 协作 live smoke。
