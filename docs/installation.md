...
| 模式 | 用途 | OpenClaw 要求 |
| --- | --- | --- |
| `gateway`（推荐） | 挂载到现有 OpenClaw Gateway 的 `/tracevane`，与 OpenClaw 共用同一个端口，同时保留 `3760` 回退入口 | 必须已有可运行的 OpenClaw Gateway |
...
Gateway 单口模式由 OpenClaw 宿主负责鉴权，访问地址与 token 会在安装结束时打印，也可在 `~/.openclaw/openclaw.json` 中查看。
