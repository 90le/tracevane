...
| 前端 | 完成 | `/system` 轻量化；`/system/recovery` lazy route、service 状态按钮切换、动作后刷新保护、历史/备份分页、CLI 自动修复、Studio bundle 重建、gateway 服务托管和进程接管策略展示完成 | 发布前做浏览器视觉 QA |
| 验证 | 部分完成 | 本轮 recovery/system 隔离编译、源码 smoke、合约/前端文本测试和 `git diff --check` 通过；全局 build 被 model-gateway 脏改阻塞 | model-gateway 收敛后重跑全量 build/typecheck |

## 验证

- 通过：`npx tsc --noEmit --target ES2022 --module NodeNext --moduleResolution NodeNext --types node apps/api/modules/system/bootstrap.ts apps/api/modules/openclaw-recovery/repair.ts apps/api/modules/openclaw-recovery/store.ts types/openclaw-recovery.ts`
...
- 通过：源码 smoke，覆盖 `repairSystemBootstrap()` 清理旧 `plugins.installs.studio`/打开 `gateway.controlUi.enabled`，以及 `inspectStudioWebBundle()` 缺失/完整资产判断。
- 通过：`git diff --check`
