...
- Starting implementation included transitional custom search controls and custom local search highlight path from M1.x.
...
| 2026-07-01 | MFC.3 | Complete | Added persisted Monaco option preferences for minimap, word wrap, and sticky scroll; large-readonly profile overrides expensive options off. |
...
| MFC.3 — Preferences and performance tuning | Complete | Font size/theme/minimap/word-wrap/sticky-scroll preferences persist in `tracevane:file-manager:online-editor-preferences:v1`; online editor smoke asserts option application and large-readonly overrides. |
...
|---|---|---|---|
| 2026-07-01 | `npm run typecheck:web` | Pass | Validated Monaco wrapper/profile/search/preference UI TypeScript changes. |
| 2026-07-01 | `TRACEVANE_WEB_SMOKE_URL=http://127.0.0.1:5177 node tests/file-manager/file-manager-online-editor.smoke.mjs` via isolated dev server | Pass | Validated tabs, minimize/restore/maximize, Monaco-native Find/Replace widget entry, Monaco option preferences, large-readonly option overrides, save/reload/close/conflict/capacity flows. |
