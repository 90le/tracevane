...
M13-C 不做：Go hover/definition runtime、启动新的 language server、修改 `/api/lsp/hover` 或 `/api/lsp/definition` 行为、新增 provider installer、新增 UI 控件或命令。下一步进入 M13-D Go / gopls Hover + Definition Guarded Proof Plan。
