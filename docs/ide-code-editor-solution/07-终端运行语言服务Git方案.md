...
102. M12-O：Toolchain provider acceptance / next heavy provider decision（下一步）
```
...
M12-N 不做：安装/下载 `rust-analyzer`、PATH discovery、auto install、Cargo metadata 探测、用户 command/args/env/cwd override、Rust rich interactions、Java/JDT LS runtime、clangd runtime、Go rich interactions、第二套 LSP/Files/Search API、Git/Debug/Terminal 新能力或 File Manager Online Editor 产品壳变更。下一步 M12-O 进入 Toolchain provider acceptance / next heavy provider decision。
