...
M11-T 明确不做：ESLint fix、format、code action、fix on save、monorepo glob workingDirectories、frontend/user command/env/cwd/runtime/options、JSON/HTML/CSS bins、第二套 LSP/Files/Search API 或替换既有 TS/JS interaction provider。M11-U 已完成 monorepo / workingDirectories hardening plan；M11-V 已完成 guarded implementation，下一步 M11-W 进入 provider acceptance / next provider decision。
...
M11-V 明确不做：ESLint fix、formatting、code actions、fix on save、glob/auto workingDirectories、用户/前端自定义 provider 配置、system binary discovery、auto install、第二套 LSP/Files/Search API、替换既有 TS/JS hover/completion/definition/references provider 或 File Manager Online Editor 产品壳变更。下一步 M11-W 进入 External provider acceptance / next provider decision。
