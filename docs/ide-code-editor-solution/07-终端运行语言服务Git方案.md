...
- 下一步进入 M7-F-C Breakpoints + editor reveal foundation；Debug 仍不和 LSP/Git 同时追完整。
```
...
M7-F-C：Breakpoints + editor reveal foundation（下一步）。
...
17. M7-F-C：Breakpoints + editor reveal foundation（下一步）
18. M7-F/M7.x：Debug Adapter Protocol 研究与最小实现
```
