...
M10-D 不新增 runtime 功能，不改变 semantic tokens/workspace symbols API，不推进完整 tsserver/typescript-language-server、不推进 watcher-backed index、不推进 AI semantic search、不推进 force push/merge/rebase 或 Debug/Git/Terminal 新能力。
...
58. M11-A：post-M10 IDE intelligence roadmap and release gate plan（下一步）
```
