...
```txt
P0：先做 M11-B IDE Command Palette / Go to Symbol shell foundation。
...
M11-A 不新增 runtime 代码，不改变 LSP/Git/Debug/Terminal API，不推进 watcher index、多语言 LSP 或 Command Palette 实现；它只确定下一阶段顺序和验收门槛。
...
59. M11-B：IDE Command Palette / Go to Symbol shell foundation（下一步）
```
