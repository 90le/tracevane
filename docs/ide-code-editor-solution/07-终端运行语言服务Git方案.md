...
M10-B 不做 workspace symbols、semantic token delta/range、多语言全集、完整 tsserver/typescript-language-server 进程、watcher-driven symbol index、Debug/Git/Terminal 新能力或 File Manager Online Editor 产品壳变更。下一步 M10-C 进入 workspace symbols foundation。
...
56. M10-C：LSP workspace symbols foundation（下一步）
```
