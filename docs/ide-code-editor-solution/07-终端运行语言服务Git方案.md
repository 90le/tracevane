...
M12-P 已完成，记录见 [`archive/m12-p-clangd-runtime-proof-plan.md`](./archive/m12-p-clangd-runtime-proof-plan.md)。这是 docs-only research / implementation plan：基于 clangd 官方 installation、compile commands、configuration、indexing 与 troubleshooting 文档，定义 C/C++ 作为第三个 toolchain-backed provider 的受控 runtime proof 策略。
...
M12-R 已完成，记录见 [`archive/m12-r-toolchain-provider-acceptance-java-decision.md`](./archive/m12-r-toolchain-provider-acceptance-java-decision.md)。这是 docs-only acceptance / decision closeout：验收 M12-G 到 M12-Q 的 toolchain provider status/config/trust gate、Go/gopls guarded diagnostics proof、Rust/rust-analyzer guarded diagnostics proof 与 C/C++/clangd guarded diagnostics proof，并决定下一步进入 Java / Eclipse JDT LS runtime proof plan。
...
M12-U 已完成，记录见 [`archive/m12-u-toolchain-provider-acceptance-closeout.md`](./archive/m12-u-toolchain-provider-acceptance-closeout.md)。本阶段验收并冻结 Go / gopls、Rust / rust-analyzer、C/C++ / clangd、Java / Eclipse JDT LS 四个 toolchain-backed diagnostics proof 的共同 guard、status surface、degraded behavior 和后续路线。
...
M13-A 已完成，记录见 [`archive/m13-a-toolchain-post-diagnostics-roadmap-plan.md`](./archive/m13-a-toolchain-post-diagnostics-roadmap-plan.md)。本阶段基于 M12-U 的 heavy provider diagnostics closeout，重新核对 gopls、rust-analyzer、clangd 与 Eclipse JDT LS 上游契约，并决定 M13 先强化 Provider Status / setup guidance UX，再选择单 provider rich interaction proof 或 installer policy。
...
M13-C 已完成，记录见 [`archive/m13-c-single-provider-rich-interaction-decision.md`](./archive/m13-c-single-provider-rich-interaction-decision.md)。本阶段不实现 runtime，而是选择 Go / gopls hover + definition guarded proof 作为第一个 toolchain-backed rich interaction 候选。
...
M13-D 已完成，记录见 [`archive/m13-d-go-gopls-hover-definition-plan.md`](./archive/m13-d-go-gopls-hover-definition-plan.md)。本阶段不改 runtime，而是规划 Go / gopls hover + definition guarded proof 的实现边界：复用现有 hover/definition API、IDE Monaco LSP provider、M12-K Go diagnostics guard、external gateway、Files root/path guard 和 Provider Status degraded 口径。
...
M13-E 已完成，记录见 [`archive/m13-e-go-gopls-hover-definition-summary.md`](./archive/m13-e-go-gopls-hover-definition-summary.md)。Go rich interaction 现在只开放 hover + definition 两个最小能力：复用 `/api/lsp/hover`、`/api/lsp/definition`、M12-K Go diagnostics guard、external language server gateway 和 Files root/path guard；无 trusted config、无 go.work/go.mod、缺 gopls binary 或 request failure 时返回 empty/degraded。
...
M13-F 已完成，记录见 [`archive/m13-f-toolchain-rich-interaction-acceptance-decision.md`](./archive/m13-f-toolchain-rich-interaction-acceptance-decision.md)。本阶段不改 runtime，而是验收 Go / gopls hover + definition guarded proof，冻结后续 toolchain rich interaction 的统一 guard/degraded 模板，并选择 Rust / rust-analyzer 作为下一 rich provider 计划对象。
