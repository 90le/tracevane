...
- M7-F-C Breakpoints + editor reveal foundation 已完成；下一步进入 M7-F-D 最小真实 adapter proof。Debug 仍不和 LSP/Git 同时追完整。
```
...
18. M7-F-D：最小真实 adapter proof（下一步）
19. M7-F-E/M7.x：Debug acceptance closeout / hardening
```
