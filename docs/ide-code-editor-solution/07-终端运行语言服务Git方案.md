...
M11-C 不做 runtime index、独立 symbol daemon、第二套 Files/Search/LSP API、完整源码持久化、多语言 LSP、完整 tsserver/typescript-language-server 长驻进程、AI semantic search、Git force/merge/rebase、Debug parity、Terminal 新能力或 File Manager Online Editor 产品壳变更。下一步 M11-C-B 进入受控实现；若 direct workspace symbols 性能足够，也可跳到 M11-D multi-language LSP provider research plan。
...
61. M11-C-B：watcher-backed symbol index guarded implementation（下一步）
```
