...
- M7-E-A 已复用现有 Git API 接入 diff 查看；M7-E-B 已复用现有 Git API 接入 stage/unstage；M7-E-C 已复用现有 Git API 接入 staged commit。
...
13. M7-E-D：Git branch / upstream status foundation 或 M7 Git acceptance closeout
