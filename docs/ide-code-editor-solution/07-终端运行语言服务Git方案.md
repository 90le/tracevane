...
M13-C 不做：Go hover/definition runtime、启动新的 language server、修改 `/api/lsp/hover` 或 `/api/lsp/definition` 行为、新增 provider installer、新增 UI 控件或命令。M13-D 已完成 Go / gopls Hover + Definition Guarded Proof Plan；下一步进入 M13-E Go / gopls Hover + Definition Guarded Implementation。
...
M13-D 不做：runtime implementation、启动 gopls、修改 hover/definition API 行为、新增 tests/smoke 脚本、installer/discovery、Rust/clangd/Java rich interactions。下一步进入 M13-E Go / gopls Hover + Definition Guarded Implementation。
