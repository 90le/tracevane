...
79. M11-O：External provider acceptance / heavy provider expansion decision（下一步）
```
...
M11-N 明确不做：Python hover/completion/definition/references/rename/format/code action、virtualenv/interpreter discovery、Pylance 私有能力、用户自定义 provider command/env、system binary discovery、auto install、第二套 LSP/Files/Search API、Git force/merge/rebase、Debug parity、Terminal 新能力或 File Manager Online Editor 产品壳变更。下一步 M11-O 进入 external provider acceptance / heavy provider expansion decision。
