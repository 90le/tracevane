...
- M7-F-C Breakpoints + editor reveal foundation 与 M7-F-D 最小真实 adapter proof 已完成；下一步进入 M7-F-E Debug acceptance closeout。Debug 仍不和 LSP/Git 同时追完整。
```
...
M7-F-E：Debug acceptance closeout（下一步）。
```
...
19. M7-F-E：Debug acceptance closeout（下一步）
20. M7.x：Debug hardening
```
