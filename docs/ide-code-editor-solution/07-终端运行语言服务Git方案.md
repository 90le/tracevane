...
M11-A 不新增 runtime 代码，不改变 LSP/Git/Debug/Terminal API，不推进 watcher index、多语言 LSP 或 Command Palette 实现；它只确定下一阶段顺序和验收门槛。M11-B 已新增 Command Palette / Go to Symbol shell，但仍不推进 watcher index、多语言 LSP、Git remote/history rewrite、Debug parity 或 Terminal 新能力。
...
60. M11-C：watcher-backed symbol index research and minimal plan（下一步）
```
