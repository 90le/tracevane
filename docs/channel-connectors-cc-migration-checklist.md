...
| P1 | Feishu 私聊 | 进行中：长连接 live 稳定；Markdown、入站文件/图片/视频 staged-path live、出站文件 7 天窗口历史 live、native-first compact wiring、真实长连接 auto compact、三 Agent 显式 `/compact` smoke 已补 | 出站文件 24h 新样本、Feishu 权限审批复验 |
| P1 | Octo 私聊 | 进行中：长连接 live 稳定；Markdown 已验证；入站文件/图片 staged-path live、出站文件 7 天窗口历史 live、权限 24h live、媒体 payload 文本保留已补 | 出站文件 24h 新样本、Octo 视频、compact live smoke |
...
- `node scripts/smoke-channel-connectors-agent-run-live.mjs --since-minutes 1440 --require-ok --require-reply --require-tool --require-tool-output --min-runs 1 --limit-runs 5 --json` 通过，最近 24h 匹配 5 条带可见工具输出的成功 IM run。
- `node --test tests/system/channel-connectors-agent-run-live-script.test.mjs`，7/7 通过，覆盖 `--require-tool-output`、入站图片/视频/文件 staged local path 正反向证据，以及 human 输出只展示匹配 run。
...
- `node scripts/smoke-channel-connectors-agent-run-live.mjs --since-minutes 1440 --require-ok --require-permission-prompt --require-permission-resolved --min-runs 1 --limit-runs 3` 通过，最近 24h 匹配 Octo 权限审批成功样本。
- `node scripts/smoke-channel-connectors-agent-run-live.mjs --since-minutes 10080 --require-ok --require-reply --require-file --min-runs 1 --limit-runs 3` 通过，7 天窗口内匹配 Feishu/Octo 出站文件成功样本；最近 24h 无新样本。
...
2. 私聊出站文件 24h 新样本、Feishu 权限、compact 抽查继续做 live smoke；入站文件/图片、Feishu 入站视频和 Octo 权限已验收，Octo 视频仍需真实消息样本。
