...
| P2 | Claude Code runner | `agent/claudecode` | 进行中：`--verbose`、stream-json progress、图片输入、session resume 已对齐 | stream-json、permission prompt、session resume、tool event、文件/图片输入 |
...
3. 然后继续迁移 Claude Code permission/session/file 能力，并迁移 OpenCode runner 的成熟流式/权限/文件能力。
