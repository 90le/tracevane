...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter | 进行中：对齐 CC Go/官方 SDK，默认关闭 Node SDK `pingTimeout` 和 connected-idle 主动重建；SDK 自管心跳/重连，Studio 非 connected watchdog 默认 180s 慢速熔断；启动 90s 无任何入站时最多 1 次 delivery-silence renewal，防止 SDK `connected=true` 但消息不投递；只读 live soak 脚本已覆盖旧 30s 策略失败 | 快速 ACK、事件去重、断线恢复、idle 策略、重启后不重复触发 Agent；继续做 30-60min live soak 与真实入站复验 |
