...
| P1 | Octo(dmwork) 长连接与媒体 | `platform/dmwork` | 进行中：Octo 纯文本权限审批已进入串行门控；出站文件默认 STS/COS 直传，旧 multipart 410 自动回退；Bot API transport 已补 read receipt、event ack、群/成员/Space、thread、history sync、file download URL、GROUP.md、THREAD.md、delete-thread、voice context；按 Octo 插件 v1.0.15 补 account/bot ID 大小写归一、广播 mention gate 和 daemon 入站 read receipt；daemon 已接入 Bot API 群成员、短历史上下文、GROUP.md/THREAD.md prompt context、文件下载 URL staging、平台 skill context、`studio-channel-messages` Octo human DM/group/thread + bot group/thread @mention + `on_behalf_of` persona 出站身份合同；Octo @ 按插件 `@[uid:displayName] -> @displayName + mention.entities/uids` 渲染，transport 层也会把只有 mention metadata 的消息补成可见 `@uid`，避免隐藏 @；Octo progress 气泡用 5s best-effort 发送并带 `client_msg_no`，避免慢 REST 把旧过程回复拖到最终阶段补发；`dm:<*_bot>` 会在群/thread 内重写为当前群/thread 可见 @，私聊场景拒绝 bot DM；`/octo` 已覆盖 groups/members/search/info/threads/thread/thread-members/history/group-md/thread-md/voice-context 和 admin-gated create/update/add/remove/create-thread/delete-thread/join/leave/set-group-md/set-thread-md/set-voice-context/delete-voice-context，查询命令已通过真实配置非发送 smoke，扩展命令已通过 mock 回归 | WuKongIM、ACK、heartbeat、重连、COS/文件/图片收发、Markdown 回复稳定；下一步做真实 Octo human DM、群/thread @其它 Studio/外部 bot 协作 smoke，补 persona/OBO 入站触发和更完整菜单 |
...
| P1 | 文件/消息收发 1:1 + Studio transport | CC file/media/message flow | 进行中：Feishu/Octo service smoke 已覆盖文件发送入口；Agent run live smoke 已能验证入站文件 staging 和出站文件发送；Octo auto 上传默认 STS/COS 并覆盖旧 `/v1/bot/file/upload` 410 回退；新增 `studio-channel-messages`，Octo 支持 human DM、group/thread 文本、`@[uid:显示名]`、`mentionUids`、`mentionAll`、`onBehalfOf/on_behalf_of/respondAs`，发送前会生成可见 `@显示名`、Octo mention entity 和可选顶层 `on_behalf_of`；bot 协作按 Octo 平台规则走群/thread @，不走 bot DM；Feishu 按 OpenClaw target 合同支持 `chat/open_id/user_id/dm` 文本与 Markdown(post) 目标并映射到官方 `receive_id_type` | 入站 staging、出站 manifest、原始文件名、yolo 权限、大文件策略、Feishu/Octo live smoke；Octo human DM、群/thread @其它 Studio/外部 bot、Feishu open_id/user_id 文本/Markdown 真实发送和异步回复汇总需真实平台验收 |
...
1. 做真实 Octo human DM、群/thread @成员和 @其它 Studio/外部 bot live smoke；当前平台只保证消息可发，等待多 bot 异步回复并自动汇总需后续 Studio 内部协作调度。
...
3. 对照 Octo 插件继续迁移 persona/OBO 入站触发、grantor 信任、更完整菜单和群/thread 管理细节；随后按同一模式补 Feishu 文档/云盘 skills。
