...
| P1 | 工具/思考/过程显示 | 继续推进：非飞书过程回复标题已移除 | 三个 Agent 都稳定提取工具名、输入、stdout/stderr、exit/status、思考流、过程回复和最终回复分类 |
...
- `node --test tests/system/channel-connectors-service.test.mjs`，97/97 通过。

## 下一步

1. 稳定 Codex / Claude Code / OpenCode 工具流和回复解析。
2. 评估 Codex / Claude Code / OpenCode 思考流支持矩阵，补齐解析和 `/thinking` 开关验收。
