...
| P1 | Octo(dmwork) 长连接与媒体 | `platform/dmwork` | 进行中：Octo 纯文本权限审批已进入串行门控；出站文件默认 STS/COS 直传，旧 multipart 410 自动回退；Bot API transport 已补 read receipt、event ack、群/成员/Space、thread、history sync、file download URL、GROUP.md、THREAD.md、delete-thread；按 Octo 插件 v1.0.15 补 account/bot ID 大小写归一、广播 mention gate 和 daemon 入站 read receipt；daemon 已接入 Bot API 群成员、按 bot 最新回复 seq 分段的短历史上下文、GROUP.md/THREAD.md prompt context、文件下载 URL staging、平台 skill context、`studio-channel-messages` Octo DM/group/thread + `@[uid:显示名]` mention 发送合同；`/octo` 已覆盖 groups/members/search/info/threads/thread/thread-members/group-md/thread-md 和 admin-gated create/update/add/remove/create-thread/delete-thread/join/leave/set-group-md/set-thread-md，查询命令已通过真实配置非发送 smoke，扩展命令已通过 mock 回归 | WuKongIM、ACK、heartbeat、重连、COS/文件/图片收发、Markdown 回复稳定；下一步做真实 Octo 私聊/@群成员 live smoke，补 persona/voice context 和更完整菜单 |
...
| P1 | 文件/消息收发 1:1 + Studio transport | CC file/media/message flow | 进行中：Feishu/Octo service smoke 已覆盖文件发送入口；Agent run live smoke 已能验证入站文件 staging 和出站文件发送；Octo auto 上传默认 STS/COS 并覆盖旧 `/v1/bot/file/upload` 410 回退；新增 `studio-channel-messages`，Octo 支持 DM/group/thread 文本和 mention，普通回复和 manifest content 都能解析 `@[uid:显示名]`，Feishu 暂支持显式 chatId 文本 | 入站 staging、出站 manifest、原始文件名、yolo 权限、大文件策略、Feishu/Octo live smoke；Octo 私聊其它 bot/成员、群 @、thread 发送需真实平台验收 |
...
1. 做真实 Octo 私聊/@群成员 live smoke，确认 Agent 能通过 `studio-channel-messages` 主动询问其它成员/机器人并整理回复。
...
3. 对照 Octo 插件继续迁移 persona/voice context、更完整菜单和群/thread 管理细节；随后按同一模式补 Feishu open_id/user_id 私聊、富文本、文档/云盘 skills。
