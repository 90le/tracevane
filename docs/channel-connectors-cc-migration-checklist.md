...
| P1 | Octo(dmwork) 长连接与媒体 | `platform/dmwork` | 进行中：Octo 纯文本权限审批已进入串行门控；出站文件默认 STS/COS 直传，旧 multipart 410 自动回退；Bot API transport 已补 read receipt、event ack、群/成员/Space、thread、history sync、file download URL；按 Octo 插件 v1.0.15 补 account/bot ID 大小写归一、广播 mention gate 和 daemon 入站 read receipt；daemon 已接入 Bot API 群成员、按 bot 最新回复 seq 分段的短历史上下文、文件下载 URL staging、平台 skill context、`studio-channel-messages` Octo DM/group/thread + `@[uid:显示名]` mention 发送合同；`/octo` 已覆盖 groups/members/search/info/threads/thread/thread-members 和 admin-gated create/update/add/remove/create-thread/join/leave，查询命令已通过真实配置非发送 smoke，扩展命令已通过 mock 回归 | WuKongIM、ACK、heartbeat、重连、COS/文件/图片收发、Markdown 回复稳定；下一步做真实 Octo 私聊/@群成员 live smoke，补 GROUP.md/thread MD/persona context、delete-thread |
