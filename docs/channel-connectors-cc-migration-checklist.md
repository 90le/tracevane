...
1. 先补 native-first compact 执行：基于已接入的 `/status` 预算，继续复刻 CC Go `ContextCompressor` 和 auto-compress 冷却策略；压缩执行顺序固定为 Agent 原生优先、Studio compact 兜底。
