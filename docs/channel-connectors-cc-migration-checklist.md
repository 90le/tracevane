...
| P1 | 工具/思考/中间态显示 | CC progress renderer | 进行中：Claude `tool_use`/`tool_result` 递归提取对象、数组和 stdout/stderr，不再显示空工具结果；OpenCode NDJSON 按 CC Go 拆分 completed `tool_use` 为工具调用 + 工具输出；assistant 过程回复只接受 `assistant/intermediate`，Codex/Claude/OpenCode 最终回复统一 `assistant/final`；Feishu 可恢复 `user/tool_result` error/failed 只显示为失败工具结果，不再锁死整轮卡片状态；live smoke 已能用窗口级硬保护验收最终回复不进过程消息和 Feishu 成功 run 进度卡 completed；`/thinking` / `/process` / `/tools` 三路独立控制 | 私聊显示过程回复、思考和工具结果；最终回复不重复显示为过程回复；群聊默认静默；飞书单卡 patch，Octo 文本 Markdown 不刷屏 |
