...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter | 进行中：对齐 CC Go/官方 SDK，同 App 进程内共享 WS，并新增用户级全局 owner lock 防多进程多连接；默认不启用 Node SDK `wsConfig.pingTimeout`、启动期 renewal、已验证静默 renewal，让官方 SDK 拥有 keepalive/reconnect；`EventDispatcher` 使用 `verificationToken/encryptKey`，runtime 区分 `connected`、真实入站 `ingressState`、`dispatcherCallbacks` 与 business `receivedMessages`；zero-inbound / connected-idle 业务层强制重建默认关闭；假在线专项已建档，后续先按 CC Go 和 OpenClaw Node/TS 完整复刻再整合 Studio guardrail | 快速 ACK、事件去重、断线恢复、idle 策略、重启后不重复触发 Agent；继续做 30-60min live soak 与真实入站复验 |
