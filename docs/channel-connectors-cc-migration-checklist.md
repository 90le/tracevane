...
| P1 | Codex runner 1:1 迁移 | `agent/codex` | 进行中：one-shot 主链路可用，`/new`/`/reset`/Studio `/compact` 自动合同、Feishu/Octo service smoke、无副作用 probe、命令 live smoke 和 Agent run live 观测脚本已覆盖；真实 Feishu/Octo `/compact`、`/new`、`/reset` no-send apply 已通过；工具流与 Markdown 用户可见 live 已通过用户反馈和日志 smoke；one-shot 最终回复/manifest 保真已锁测试，文件 live 仍待收口 | `exec/resume` 参数、thread、cwd、permission、tool stream、file manifest、stop/new/reset/compact 全部按 CC 验收 |
...
| P2 | 命令全集 | CC slash/native commands + CommandProvider + SkillRegistry | 进行中：Claude Code/Gemini Agent 命令文件、`/commands add/del` prompt 命令、`/skills` 与 Skill invocation、CC Go exact-first/唯一前缀/歧义透传和 `/commands` 子命令缩写已接入 | `/help/status/current/list/history/name/search/dir/cd/new/reset/model/mode/reasoning/display/stream/tools/native/stop` 均文本与卡片可用；`/commands` 能列出 config + Agent 命令，命中后按 CC 占位符规则展开并交给 Agent；`/skills` 能列出并调用 SKILL.md；`addexec` 等 shell 执行面待单独验收 |
...
1. 先锁 Codex `exec/resume` 稳定路径；`/compact`、`/new`、`/reset` 已完成真实 apply no-send 验收，工具流和 Markdown 渲染已完成 live 收口；下一步用 Agent run live 观测脚本复验发文件。现有工具流和 `studio-channel-files` 合同只对照补缺，不推翻重写。
2. 再按 CC 复刻 Feishu 菜单/设置卡片、剩余 CommandProvider 命令细节和 Octo 文本/Markdown 命令体验。
3. 然后继续迁移 Claude Code AskUserQuestion/file 能力，并迁移 OpenCode runner 的成熟流式/权限/文件能力。
4. Codex app-server 继续保留 beta，不阻塞稳定 live 路线。

## Codex 完成判断

- 结论：Codex 未全部迁移完成。
- 已完成：默认 live 回到 one-shot `codex exec/resume`，Gateway key/config、thread resume、图片参数顺序、工具/文件 manifest、one-shot 最终回复/manifest 保真、工具流与 Markdown 用户可见 live、`/stop`、queue、`/new`/`/reset` 清 session/history、Studio `/compact` 调 Gateway compact 并清旧续接，以及 Feishu/Octo service smoke、recent-session 命令 smoke、Agent run live 观测脚本、adapter dry-run 无副作用合同、真实 `/compact`、`/new`、`/reset --apply --no-send-reply` 和 app-server beta 测试已有覆盖。
- 未完成：Feishu/Octo 真实 live apply 下的文件发送用户可见实测收口；Codex app-server 仍是 metadata beta，不算默认完成。
