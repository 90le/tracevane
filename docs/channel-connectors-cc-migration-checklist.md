...
| P1 | Feishu 菜单/卡片 1:1 复刻 | `platform/feishu` card/menu | 进行中：Commands tab 已接入 config/Agent 命令列表；CC Card `RenderText()` 式文本 fallback、`/help <section>` 分组帮助、session list 删除按钮和 `/whoami`/`/version` session 按钮已接入 | 主菜单、设置子卡、下拉、分页、执行结果卡/文本、callback ACK 均可真实操作 |
