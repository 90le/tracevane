...
| P2 | durable queue | 进行中：pending-agent-run store 已接入 Octo/Feishu；Octo daemon restart 回归已通过 | daemon 重启不丢失未开始任务；继续补 Feishu live smoke 和运行态可见性 |
...
4. durable queue：补 Feishu live smoke 和运行态 pending/replay 可见性。
