...
- `node --test tests/system/channel-connectors-agent-run-live-script.test.mjs`，8/8 通过，覆盖 `--require-tool-output`、入站图片/视频/文件 staged local path、Feishu 卡片审批 command 形态，以及 human 输出只展示匹配 run。
