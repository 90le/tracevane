...
1. 先补上下文预算与自动压缩：复刻 CC Go `ContextUsageReporter`、剩余上下文 footer/status、`ContextCompressor` 和 auto-compress 冷却策略，并接入 Studio Gateway 模型窗口；压缩执行顺序固定为 Agent 原生优先、Studio compact 兜底。
