...
| P1 | 工具/思考/流式显示 | CC progress renderer | 进行中：Claude `user/tool_result` 纯输出不再被进度解析丢首行，单行结果不再显示“无输出” | 私聊显示进度和工具结果；群聊默认静默；飞书单卡 patch，Octo 文本 Markdown 不刷屏 |
...
2. 先迁移 Codex app-server requestApproval pending approval 回包闭环，再扩展真实 Claude/OpenCode 视觉输入和 IM live 文件上传 smoke。
