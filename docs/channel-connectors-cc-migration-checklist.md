...
| P1 | Codex runner 1:1 迁移 | `agent/codex` | 进行中：one-shot 主链路可用，`/new`/`/reset`/Studio `/compact` 自动合同、Feishu/Octo service smoke、无副作用 probe 和命令 live smoke 脚本已覆盖，未达到全量 live apply 完成 | `exec/resume` 参数、thread、cwd、permission、tool stream、file manifest、stop/new/reset/compact 全部按 CC 验收 |
...
1. 先锁 Codex `exec/resume` 稳定路径，用命令 live smoke 脚本复验 Feishu/Octo `/new`、`/reset`、`/compact`，再复验发文件、工具流、Markdown 最终回复；现有工具流和 `studio-channel-files` 合同只对照补缺，不推翻重写。
...
- 结论：Codex 未全部迁移完成。
- 已完成：默认 live 回到 one-shot `codex exec/resume`，Gateway key/config、thread resume、图片参数顺序、工具/文件 manifest、`/stop`、queue、`/new`/`/reset` 清 session/history、Studio `/compact` 调 Gateway compact 并清旧续接，以及 Feishu/Octo service smoke、recent-session 命令 smoke 脚本、adapter dry-run 无副作用合同和 app-server beta 测试已有覆盖。
- 未完成：Feishu/Octo 真实 live apply 下的文件发送、工具流、最终 Markdown、new/reset/compact 用户可见实测收口；Codex app-server 仍是 metadata beta，不算默认完成。
