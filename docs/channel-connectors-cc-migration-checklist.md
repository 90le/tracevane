...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter | 进行中：对齐 CC Go/官方 SDK，默认关闭 Node SDK `pingTimeout`、zero-inbound 和 connected-idle 主动重建；SDK 自管心跳/重连，Studio 非 connected watchdog 默认 180s 慢速熔断且可关闭 | 快速 ACK、事件去重、断线恢复、idle 策略、重启后不重复触发 Agent |
