...
- IM Agent active run 必须在 session lease 成功后立即进入 runtime / cancel registry；模型路由、auto compact、历史拉取、附件 staging 等 preflight 失败必须写 `agent.run.preflight_failed` 并释放 session guard。native auto compact 可忽略同一 messageId 的 preflight active run，但不得放行其它 messageId 的并发 run。
- Octo 群历史 cutoff 只能在最终回复完整投递 `replyDeliveryStatus=delivered` 后推进；恢复旧日志时才兼容没有 `replyDeliveryStatus` 的 `replySent=true` 事件。Feishu 失败态进度卡终态 patch 失败必须文本兜底，并记录 `progressCardFinalDelivered` / `progressCardFinalError`。
...
| P1 | Codex runner | 进行中：结构化 stdout/stderr、混合 content 工具结果、resume 图片参数顺序、app-server 视觉 args 证据、Gateway Responses->Chat 图片映射、图片 native smoke 已补；app-server turn 超时改为空闲超时，普通静默默认 3 分钟，审批请求和批准工具会刷新/延长 idle，长 gpt5.5 多工具任务有进度时不再被总时长误杀，fallback 恢复型 timeout 不进入用户进度流；Codex 默认 session driver 已切到 app-server persistent，daemon status/runtime 显示 `defaultMode=persistent` 与 `reason=codex-app-server`，显式 metadata 可回退 one-shot；one-shot 兼容 runner 心跳超时已与 Claude/OpenCode 对齐，stdout/stderr TUI 刷新会续期等待，只有停止心跳才报 `process/heartbeat-timeout`；Codex 子 agent/parallel task TUI 计数会写 `process/async-task` 并进入 bounded async idle grace；持续只有 TUI 心跳会记录 `process/heartbeat-stall` 非终态诊断且不刷新 timeout，重复诊断退避节流；权威失败终态覆盖 exit 0，权威完成终态后 lingering CLI grace 收尾，grace 期间 `/stop` 仍由取消获胜；未知结构化 CLI 事件写入 bounded `protocol/unknown-event`，exit 0 但无最终回复时返回兼容提示；app-server 缓冲同 tick 终态通知、unknown terminal status 保守失败、transport close/dispose 立即 settle；persistent 创建/执行失败会写 `turn.failed` / `turn.fallback` 并按命令策略降级；IM preflight active run、delivering、delivery failed、delivered-only cutoff 和同消息 native compact 豁免已补；本地矩阵已覆盖 stdout、stderr CR-only TUI、async child-task idle grace、heartbeat-only stall、idleTimeout 替代总超时、静默 timeout、协议降级和终态 race；视频按 staged file 交给 Agent | app-server persistent 默认；`exec/resume` one-shot 兼容 fallback；thread、cwd、permission、tool stream、file manifest、stop/new/reset/compact 按 CC 验收 |
| P1 | Claude Code runner | 进行中：native compact、结构化/混合 tool_result、过程/最终回复、图片 native smoke 已补；默认 session driver 已切到 stream-json persistent，one-shot 仅作显式 opt-out/fallback；one-shot 兼容 runner 心跳超时已与 Codex/OpenCode runner 对齐，`Imagining...` / `esc interrupt` 等 stderr/TUI 刷新会续期等待；`deep-research ... 3/18 agents done` 等子 agent harness 状态会写 `process/async-task` 并进入 bounded async idle grace；持续只有 TUI 心跳会记录 `process/heartbeat-stall` 非终态诊断且不刷新 timeout，重复诊断退避节流；权威失败终态覆盖 exit 0，权威完成终态后 lingering CLI grace 收尾，grace 期间 `/stop` 仍由取消获胜；persistent `result` 的 error-like subtype 按失败处理，未知结构化事件写入 bounded `protocol/unknown-event`；本地矩阵已覆盖 stdout、stderr CR-only TUI、async child-task idle grace、heartbeat-only stall、idleTimeout 替代总超时、静默 timeout、协议降级和终态 race；视频按 staged file 交给 Agent | stream-json、permission prompt、session resume、tool event、文件/图片/视频输入、native compact/stop live driver |
| P1 | OpenCode runner | 进行中：parser 已对齐；结构化/混合 stdout/stderr/exitCode、native compact、图片 native smoke 已补；默认 session driver 已切到 `run --session` persistent，one-shot 仅作显式 opt-out/fallback；one-shot 兼容 runner 心跳超时已与 Codex/Claude runner 对齐，`Imagining...` / `esc interrupt` 等 stderr/TUI 刷新会续期等待；OpenCode parallel/task TUI 计数会写 `process/async-task` 并进入 bounded async idle grace；持续只有 TUI 心跳会记录 `process/heartbeat-stall` 非终态诊断且不刷新 timeout，重复诊断退避节流；权威失败终态覆盖 exit 0，权威完成终态后 lingering CLI grace 收尾，grace 期间 `/stop` 仍由取消获胜；未知结构化 CLI 事件通过 one-shot runner 写入 bounded `protocol/unknown-event`，exit 0 但无最终回复时返回兼容提示；本地矩阵已覆盖 stdout、stderr CR-only TUI、async child-task idle grace、heartbeat-only stall、idleTimeout 替代总超时、静默 timeout、协议降级和终态 race；视频按 staged file 交给 Agent | JSON/SQLite fallback、session、tool stream、文件/图片/视频输入、native compact/stop live driver |
| P1 | Feishu 私聊 | 进行中：长连接 live 稳定；Markdown、入站文件/图片/视频 staged-path live、出站文件 24h live、权限审批 24h live、命令 reaction + reply-to、compact command progress、进度卡条数可在卡片内设置、工作目录快捷切换/分页/搜索、`home/parent/recent/child` 文本快捷命令和最近/子目录直达列表、主卡/更多页直达真实配置页、配置页分区和单按钮切换、卡片 action 同步 callback response、native-first compact wiring、三 Agent 显式 `/compact` 24h live 已补；进度卡片中间工具/步骤错误不再提前切 failed 终态；失败态终态卡 patch 失败会文本兜底并记录 `progressCardFinal*` 诊断 | 持续抽查 |
...
- `node --test tests/system/channel-connectors-agent-session-driver.test.mjs`，16/16 通过；`node --test tests/system/channel-connectors-service.test.mjs`，124/124 通过，覆盖默认 persistent、显式 one-shot opt-out/fallback、终态 grace、取消 race、未知 CLI 协议降级、无最终回复兼容提示、async child-task idle grace / grace 后 timeout、Claude persistent error subtype/unknown event、Codex app-server 同 tick 终态通知缓冲、unknown terminal status、transport close settle、Feishu 进度卡终态 dirty、Agent `running -> delivering -> cleanup` 生命周期、投递期 `/stop` 文案、`replyDeliveryStatus` 合同、preflight cleanup 和同消息 native compact 豁免。
- `npm run smoke:channel-connectors:agent-heartbeat-local -- --json`，19/19 通过；`node --test tests/system/channel-connectors-agent-heartbeat-local-script.test.mjs`，2/2 通过。
...
- `node scripts/smoke-channel-connectors-agent-heartbeat-local.mjs --json` 与 `npm run smoke:channel-connectors:agent-heartbeat-local -- --json` 均 19/19 通过，合成本地子进程覆盖三 Agent stderr CR-only TUI、stdout 心跳、async child-task idle grace、heartbeat-only stall 诊断、idleTimeout 替代总超时、静默 heartbeat timeout 和非 runtime Agent 固定超时。
- `node --test tests/system/channel-connectors-agent-heartbeat-local-script.test.mjs`，2/2 通过，覆盖本地 heartbeat smoke 帮助文本和完整矩阵。
- `node --test --test-name-pattern "native Channel Connectors process runner" tests/system/channel-connectors-service.test.mjs`，33/33 通过，覆盖 Codex / Claude Code / OpenCode TUI 心跳续期、stdout 心跳、CR-only TUI 刷新、async child-task idle grace、async grace 后 timeout、heartbeat-only stall 诊断、stall 诊断不刷新 timeout、重复 stall 诊断退避节流、权威失败终态覆盖 exit 0、权威完成终态后 lingering CLI grace 收尾、idleTimeout 替代总超时、静默 heartbeat timeout、最后活动流诊断、非 runtime Agent 固定 timeout、三 Agent 工具流和权限处理。
- `node --test --test-name-pattern "native Channel Connectors daemon owns Feishu long-connection ingress" tests/system/channel-connectors-service.test.mjs` 通过，覆盖 Feishu 进度卡片中间错误和过程 completed 均不提前切终态。
