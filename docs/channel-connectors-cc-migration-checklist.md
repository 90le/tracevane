...
| P1 | Octo(dmwork) 长连接与媒体 | `platform/dmwork` | 进行中：Octo 纯文本权限审批已进入串行门控；出站文件默认 STS/COS 直传，旧 multipart 410 自动回退；Bot API transport 已补 read receipt、event ack、群/成员/Space、thread、history sync、file download URL；按 Octo 插件 v1.0.15 补 account/bot ID 大小写归一、广播 mention gate 和 daemon 入站 read receipt；daemon 已接入 Bot API 群成员、短历史上下文、文件下载 URL staging、平台 skill context 和 `studio-channel-messages` Octo DM/group/thread + mention 发送合同 | WuKongIM、ACK、heartbeat、重连、COS/文件/图片收发、Markdown 回复稳定；下一步迁移 `octo_management` 等价工具、GROUP.md/thread MD/persona context、Space 成员搜索和真实 Octo 私聊/@群成员 live smoke |
