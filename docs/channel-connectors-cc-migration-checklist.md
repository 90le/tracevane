...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter + latest OpenClaw `extensions/feishu/src/monitor.transport.ts` | 进行中：同 App 用户级全局 owner lock；每 cycle 一个官方 SDK `WSClient` + `EventDispatcher`；默认启用 SDK lower-case `pingTimeout=3` watchdog；包装 SDK `pingLoop()` 把有效 ping interval clamp 到 30s；terminal error 后按 OpenClaw 外层循环重建；SDK `reconnecting` 超过 10s 时回收到外层循环；`pongTimeoutMs=15000` 外层兜底；55s control-frame stale 判死；connected-idle / zero-inbound / verified-ingress / generic watchdog 重建默认关闭；startup ingress validation 改为 opt-in 诊断 | 已通过 75s live smoke：4 ping / 5 pong / 0 violations；下一步做用户新消息业务入站复验；若再假在线，先按专项文档抓 runtime/log/lock/process，不先重启 |
| P1 | Octo(dmwork) 长连接与媒体 | `platform/dmwork` | 进行中 | WuKongIM、ACK、heartbeat、重连、COS/文件/图片收发、Markdown 回复稳定 |
| P1 | 工具/思考/中间态显示 | CC progress renderer | 进行中：Claude `user/tool_result` 纯输出不再被进度解析丢首行，单行结果不再显示“无输出”；assistant 过程回复已接入 `/process` 展示，`/thinking` / `/process` / `/tools` 三路独立控制 | 私聊显示过程回复、思考和工具结果；群聊默认静默；飞书单卡 patch，Octo 文本 Markdown 不刷屏 |
...
| P1 | Codex app-server 权限批准 | `agent/codex/appserver_session.go` pending approvals | 已完成 driver 合同：`item/commandExecution/requestApproval`、`item/fileChange/requestApproval`、`item/permissions/requestApproval` 复用 Studio IM permission resolver，并按 CC Go 回写 decision / turn-scoped permissions；真实 IM live approval smoke 待做 | 飞书按钮和文本 `/approve`/`/deny`/`/allow-all` 在真实 Codex app-server 运行中审批场景闭环 |
