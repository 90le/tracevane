...
2. 继续迁移 Claude Code AskUserQuestion 卡片精修/file 能力，并迁移 OpenCode runner 的成熟流式/权限/文件能力。
3. 迁移 Claude Code / OpenCode live interactive session driver 后再扩展原生 compact capability；只在 runner 有真实原生 compact/compress 合同时加入，不支持时继续 Studio fallback。
