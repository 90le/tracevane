...
| P2 | 命令全集 | CC slash/native commands + CommandProvider | 进行中：Claude Code/Gemini Agent 命令文件已接入 | `/help/status/current/list/history/name/search/dir/cd/new/reset/model/mode/reasoning/display/stream/tools/native/stop` 均文本与卡片可用；`/commands` 能列出当前 Agent 命令文件，命中后按 CC 占位符规则展开并交给 Agent |
