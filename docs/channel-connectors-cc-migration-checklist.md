...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter | 进行中：对齐 CC Go/官方 SDK，同 App 进程内共享 WS，并新增本机 owner lock 防多进程多连接；默认 60s SDK half-open timeout，zero-inbound / connected-idle 业务层强制重建默认关闭；runtime 区分 `connected` 与真实入站 `ingressState`；假在线专项已建档，后续先按 CC Go 和 OpenClaw Node/TS 完整复刻再整合 Studio guardrail | 快速 ACK、事件去重、断线恢复、idle 策略、重启后不重复触发 Agent；继续做 30-60min live soak 与真实入站复验 |
