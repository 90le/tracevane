...
| P1 | Feishu 私聊 | 进行中：长连接 live 稳定；Markdown、入站文件/图片/视频 staged-path live、native-first compact wiring、真实长连接 auto compact、三 Agent 显式 `/compact` smoke、图片/视频 runner 合同已补 | 出站文件、权限审批复验 |
| P1 | Octo 私聊 | 进行中：长连接 live 稳定；Markdown 已验证；入站文件/图片 staged-path live、图片/视频 runner 合同、媒体 payload 文本保留已补 | 出站文件、权限审批、Octo 视频、compact live smoke |
...
2. 私聊出站文件、权限、compact 抽查继续做 Feishu 与 Octo live smoke；入站文件/图片和 Feishu 入站视频已用 staged-path 脚本验收，Octo 视频仍需真实消息样本。
