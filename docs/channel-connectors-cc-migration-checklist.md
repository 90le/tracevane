...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter | 进行中：默认关闭 Node SDK `pingTimeout` 和 zero-inbound renewal，保留 metadata opt-in；非 connected watchdog 默认 20s 自愈 | 快速 ACK、事件去重、断线恢复、idle 策略、重启后不重复触发 Agent |
