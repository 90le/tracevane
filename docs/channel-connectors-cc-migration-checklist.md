...
| P1 | 上下文预算与自动压缩 | `ContextUsageReporter` / `ContextCompressor` / auto-compress | 进行中：Gateway `/v1/models` 已暴露 `contextWindow/maxOutputTokens`；Channel `/status` 已接 resolved model 预算、Gateway usage 优先和 IM history 估算兜底；手动 `/compact` 和 daemon 自动触发均已 native-first，Codex app-server 已覆盖 `thread/compact/start`；`/native /compact` 已收紧为只走真实 native compact contract，Claude Code/OpenCode one-shot 拒绝伪执行，persistent fake CLI contract 已覆盖；自动触发用剩余上下文阈值判断，成功后记录 used-token baseline 防重复触发，失败/阻塞才进入 retry cooldown；最近 native/fallback/skipped、effective used、summary/error/retry 已在 `/status` 和管理页可观测 | `/status` 显示剩余上下文；Gateway usage 优先、估算兜底；达到阈值优先触发 Agent 原生 compact/compress；不支持、失败或 one-shot 不可靠时降级 Studio compact；强制 native compact 不允许伪透传；成功 baseline 和失败 retry 记录可观测；Claude/OpenCode 需补真实 CLI smoke |
| P2 | Claude Code runner | `agent/claudecode` | 进行中：`--verbose`、stream-json progress、图片输入、session resume、权限自动回包、IM 批准、Feishu 权限按钮卡片、AskUserQuestion 基础回答闭环、persistent stream-json `/compact` fake CLI contract 已对齐 | stream-json、permission prompt、session resume、tool event、文件/图片输入；AskUserQuestion 卡片精修和真实 persistent smoke 待迁移/验收 |
| P2 | OpenCode runner | `agent/opencode` | 进行中：one-shot JSON progress、`run --session` 续接、sessionID 提取和 persistent `/compact` fake CLI contract 已对齐 | 原生命令、session、tool stream、权限、文件/图片输入；真实 persistent smoke 待验收 |
...
2. 跑真实 Claude Code / OpenCode persistent smoke：普通对话、工具/权限、文件输入输出、`/compact`、`/stop`。
