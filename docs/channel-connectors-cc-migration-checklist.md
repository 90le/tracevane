...
当前已记录偏离：CC Go `/compress` 调当前 Agent `CompressCommand()`；Studio 默认 `/compact`/`/compress` 是 Channel 自建压缩，调用 Gateway `/responses/compact` 摘要 IM history 并清旧续接。CLI 原生压缩只走 `/native /compact`，且 Codex one-shot 不伪执行交互式 compact；Codex persistent beta 已验证原生 app-server compact。
...
| P1 | Feishu 长连接稳定性 | `platform/feishu` daemon/adapter + latest OpenClaw `extensions/feishu/src/monitor.transport.ts` | 进行中：同 App 用户级全局 owner lock；每 cycle 一个官方 SDK `WSClient` + `EventDispatcher`；SDK `pingTimeout=3`；terminal error 后按 OpenClaw 外层循环重建；SDK `reconnecting` 超过 10s 时回收到外层循环；connected-idle / zero-inbound / verified-ingress / generic watchdog 重建默认关闭；5 分钟 live soak 326 samples / 0 violations；post-patch 用户消息即时入站并完成；10 分钟 idle smoke 648 samples / 0 violations / logEvents=0 | 继续做真实 SDK reconnect 后消息复验；若再假在线，先按专项文档抓 runtime/log/lock/process，不先重启 |
...
| P1 | 上下文预算与自动压缩 | `ContextUsageReporter` / `ContextCompressor` / auto-compress | 待迁移：已确认 CC Go contract；Studio 已有 Gateway model `contextWindow/maxOutputTokens` 和 App Connection `autoCompactTokenLimit` 字段，但未接 live Channel budget | `/status` 显示剩余上下文；Gateway usage 优先、估算兜底；达到阈值按冷却触发 Studio compact；原生 compact 仅在持久/交互式 runner 支持时执行 |
...
1. 先补上下文预算与自动压缩：复刻 CC Go `ContextUsageReporter`、剩余上下文 footer/status、`ContextCompressor` 和 auto-compress 冷却策略，并接入 Studio Gateway 模型窗口。
...
4. Codex app-server 继续保留 beta，不阻塞稳定 live 路线；native compact 只作为持久 session 增强。
