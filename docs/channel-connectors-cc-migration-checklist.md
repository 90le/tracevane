...
| P1 | Channel Connectors CLI Profile 管理面 | 进行中：`/channel-connectors/profiles` 已作为独立工作台承接 Profile、Gateway 模型、上下文预算、IM 绑定、持久会话和事件记录；Profile 复制、删除保护、设为默认、模型网关跳转、事件筛选和当前 Profile 活动 session 批量停止已补；`/agents/:agentId/cli` 已删除 | 后续继续补更细的 CLI app 配置、真实编辑流和浏览器交互验收 |
