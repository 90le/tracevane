...
| P1 | 工具/思考/中间态显示 | CC progress renderer | 进行中：Claude `tool_use`/`tool_result` 递归提取对象、数组和 stdout/stderr，并用 `tool_use_id` 将结果回填到对应工具名；OpenCode NDJSON 按 CC Go 拆分 completed `tool_use` 为工具调用 + 工具输出；进度渲染层按 `rawType/itemType` 区分 `tool_use/tool_result`，Feishu 卡片和 Octo/纯文本不再把 Claude/OpenCode 工具调用误显示成“工具结果”，并按工具名显示“命令输出 / 读取结果 / 检索结果”等语义标签；工具识别补齐 `exec_command`、`TodoRead`、`find`、`webopen/webfind`、图像查看和 MCP；Codex one-shot、Codex app-server、Claude Code 和 OpenCode 的 assistant 正文统一缓冲，若后续仍有工具/思考/运行事件则标记 `assistant/intermediate`，只剩 terminal 则标记 `assistant/final`；Codex app-server delta 仍不逐 token 刷过程；思考流只展示上游真实 `reasoning/thinking`，OpenCode 已传 `--thinking`，Claude Code 已解析 `thinking` block；Feishu 可恢复 `user/tool_result` error/failed 只显示为失败工具结果，不再锁死整轮卡片状态；live smoke 已能用窗口级硬保护验收最终回复不进过程消息和 Feishu 成功 run 进度卡 completed；`/thinking` / `/process` / `/tools` 三路独立控制 | 私聊显示过程回复、思考和工具结果；最终回复不重复显示为过程回复；群聊默认静默；飞书单卡 patch，Octo 文本 Markdown 不刷屏 |
