...
| P1 | Feishu 私聊 | 进行中：长连接 live 稳定；Markdown 已验证；native-first compact wiring 和真实长连接 auto compact smoke 已补 | 文件/图片、权限审批、工具流卡片、显式 `/compact` live smoke |
...
2. 做 Feishu 显式 `/compact` live smoke。
