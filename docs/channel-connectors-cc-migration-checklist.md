...
| P1 | Codex runner 1:1 迁移 | `agent/codex` | 进行中 | `exec/resume` 参数、thread、cwd、permission、tool stream、file manifest、stop/new/reset/compact 全部按 CC 验收 |
| P1 | Feishu 菜单/卡片 1:1 复刻 | `platform/feishu` card/menu | 进行中 | 主菜单、设置子卡、下拉、分页、执行结果卡/文本、callback ACK 均可真实操作 |
...
| P1 | 文件收发 1:1 + Studio transport | CC file/media flow | 进行中 | 入站 staging、出站 manifest、原始文件名、yolo 权限、大文件策略、Feishu/Octo live smoke |
...
| P2 | 命令全集 | CC slash/native commands | 进行中 | `/help/status/current/list/history/name/search/dir/cd/new/reset/model/mode/reasoning/display/stream/tools/native/stop` 均文本与卡片可用 |
...
1. 先锁 Codex `exec/resume` 稳定路径，复验 Feishu/Octo 发文件、工具流、Markdown 最终回复。
2. 再按 CC 复刻 Feishu 菜单/设置卡片和 Octo 文本/Markdown 命令体验。
