...
| P1 | 工具/思考/中间态显示 | CC progress renderer | 进行中：Claude `user/tool_result` 纯输出不再被进度解析丢首行，单行结果不再显示“无输出”；assistant 过程回复已接入 `/process` 展示，且仅 `assistant/intermediate` 会进入渠道过程消息；Codex app-server delta 不再逐 token 推送，Codex/Claude/OpenCode 最终回复统一为 `assistant/final` 并只走最终结果渲染；`/thinking` / `/process` / `/tools` 三路独立控制 | 私聊显示过程回复、思考和工具结果；最终回复不重复显示为过程回复；群聊默认静默；飞书单卡 patch，Octo 文本 Markdown 不刷屏 |
