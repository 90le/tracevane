...
| P2 | Claude Code runner | `agent/claudecode` | 进行中：`--verbose`、stream-json progress、图片输入、session resume、权限自动回包已对齐 | stream-json、permission prompt、session resume、tool event、文件/图片输入；IM 交互式批准待迁移 |
...
3. 然后继续迁移 Claude Code IM 交互式 permission/file 能力，并迁移 OpenCode runner 的成熟流式/权限/文件能力。
