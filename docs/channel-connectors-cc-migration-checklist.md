...
1. 先补 native-first auto compact：基于已接入的 `/status` 预算和手动 `/compact` native-first 执行，继续复刻 CC Go `ContextCompressor` 和 auto-compress 冷却策略。
