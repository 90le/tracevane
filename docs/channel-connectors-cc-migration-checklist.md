...
| P1 | 工具/思考/过程显示 | 继续推进：非飞书过程回复标题已移除；Codex reasoning summary、Octo `/thinking` 过滤和 OpenCode live reasoning 已补 | 三个 Agent 都稳定提取工具名、输入、stdout/stderr、exit/status、思考流、过程回复和最终回复分类 |
...
2. 在能力展示里区分 parser 支持和 live 输出支持：Claude Code 当前无 `thinking` item，OpenCode 仅在支持 reasoning 的模型上有 `reasoning` part。
