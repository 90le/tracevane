...
| `model-gateway.html` | 模型网关 | 分层多视图 | 4 主区（概览/Provider/模型/用量）+ 子视图下钻：Provider 配置子页、账号池（仅账号制 Provider 显示入口）、客户端接入（概览下钻）。apply/rollback 走 Dialog + diff 预览。IA 见 `aurora-design-system.md` §8.2。 |
