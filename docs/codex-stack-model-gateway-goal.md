...
- OpenAI 官方 Responses provider 后续必须按原生 `/v1/responses/compact` 验收；MLAMP 这类 dual-compatible base 不作为官方 compact 能力证明。

成熟度门槛：

- 不能只靠 mocked route test 宣称完成；必须有真实 provider smoke 和真实 CLI smoke。
- 先完成 Anthropic-compatible 与 OpenAI Chat-compatible provider 的成熟度；OpenAI 官方 Responses/compact provider 等用户提供真实 base/key 后补齐。
...
- OpenAI 官方 Responses smoke：等用户提供真实 OpenAI Platform base/key 后验证 `/v1/responses` 与原生 `/v1/responses/compact`；凭据不得写入文档、测试 fixture 或 git。
