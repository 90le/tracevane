...
| Gateway Service & Config | daemon 安装/启动/状态、用户自定义 provider 配置、secret 写入、模型列表/默认模型、active provider、smoke |
...
| Gateway UI | Provider Center、App Connections、Runtime、Smoke 的新页面；参考旧 CPA 管理页的运行态布局和 cc-switch 的 provider 表单，但不内置具体 vendor 预设，也不复用旧 Codex Stack / CPA / Compact 文案、诊断矩阵、安装修复复杂度 |
...
| Phase D | 先新建 Studio Gateway 服务与配置面：daemon 状态/启停、provider 配置、secret、模型列表/默认模型、smoke；UI 借鉴旧 CPA 的运维入口和 cc-switch 的 Provider 管理体验 |
