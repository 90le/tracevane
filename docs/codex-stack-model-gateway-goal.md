...
> 状态：Phase C deletion completed; Phase B core matrix completed; Phase B2 maturity hardening pending
> 更新：2026-06-05
...
- 先完成 Anthropic-compatible 与 OpenAI Chat-compatible provider 的成熟度；OpenAI Responses 官方 provider 等用户提供真实 base/key 后补齐。
...
- BigModel Anthropic smoke base：`https://open.bigmodel.cn/api/anthropic`
- MLAMP OpenAI Responses-compatible smoke base：`https://llm-gateway.mlamp.cn/v1`，model `gpt-5`；该 base 同时支持 Chat Completions，优先作为 Responses 原生 provider 成熟度样本。
- OpenAI 官方 Responses smoke：使用本机已登录/可用凭据验证；凭据不得写入文档、测试 fixture 或 git。
