...
- `providercfg`: Aurora `subpage / subpage-grid / cfg / save-bar` Provider create/edit child view for base URL, API format, auth, structured endpoint profile rows, structured model catalog rows, network, reasoning, metadata and app scopes. Create mode is a draft context: it must not show selected Provider health, selected Provider endpoint rows, saved-key update buttons or delete actions.
