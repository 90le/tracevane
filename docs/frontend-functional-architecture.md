...
- React Router routing/layout docs rechecked for Model Gateway child-flow ownership: `https://reactrouter.com/start/framework/routing`
- TanStack Query mutation/update docs rechecked for Provider/App Connection writes: `https://tanstack.com/query/v5/docs/framework/react/guides/updates-from-mutation-responses`, `https://tanstack.com/query/v5/docs/framework/react/guides/optimistic-updates`
- MDN `<dialog>` and WAI-ARIA modal dialog pattern rechecked for destructive confirmation flows: `https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/dialog`, `https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/`
...
- `overview`: Status-console entry that answers "what will route where, and what needs attention?" It shows Gateway readiness, active route rows, route-level check actions and attention items from live Gateway APIs. Low-value KPI walls and prototype-only sample incidents are not allowed.
- `providers`: Aurora `page-head / tablewrap` Provider workbench. The header separates primary creation paths (`登录 Codex`, `新建 API Provider`) from search/filter/detect tools. Rows show identity, type, status and common actions only; raw protocol/source/latency/date strings are not piled into the list.
- `providercfg`: Aurora `subpage / cfg / save-bar` Provider create/edit child view with section navigation: 基础, Endpoint, 模型 and 高级. It exposes the same real fields but does not show every field at once. Delete lives only in the advanced danger zone and requires dialog confirmation.
...
- `apps`: Aurora client-connection child view reached from the Overview "客户端接入" action, not a fifth primary tab. It owns route application into Codex/Claude Code/OpenCode local client config, shows preview/apply/rollback actions and keeps every write behind preview/confirmation/result evidence.

Model Gateway must treat `docs/prototypes/pages/model-gateway.html`, `docs/model-gateway-ia-contract.md` and `docs/model-gateway-detailed-goal.md` as the page-level visual, IA, boundary and execution contract. React implementation may bind real Gateway APIs into the prototype slots, but missing fields must use prototype placeholders or empty states instead of changing layout, adding new cards or inventing alternate styles. The implemented view set must match the prototype exactly: `overview / providers / providercfg / models / accounts / apps / usage`. Main tabs stay limited to `overview / Provider / models / usage`; secondary views are reached only from prototype actions. Provider mutation, route selection, app connection writes and destructive actions must stay behind focused child flows or confirmation-backed controls. Provider create and edit are separate mental states even when they share the same prototype child view: create starts empty/draft, saves the key together with the Provider and hides dangerous operations until the Provider exists. Provider model editing uses controlled structured rows for model id and alias in the default flow; context/output/capability/pricing remain behind explicit advanced flows rather than being dumped into creation. Overview is status/navigation only and must not host Provider/account/model/client creation flows.
