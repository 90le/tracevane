...
- 2026-06-21：继续按 React Router / TanStack Query 现有合同与本地 Model Gateway API 合同推进 `/model-gateway`。本轮只读取 `/api/model-gateway/status`、`runtime`、`providers`、`app-connections`、`usage`、`daemon-service`，把原型 Provider/模型/接入/用量面板替换成真实 React 只读页；不接入 `active-provider`、`active-route-smoke`、app connection apply/rollback 或 provider mutation，避免在无确认流时引入写操作风险。
