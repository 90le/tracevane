...
- 2026-06-20：重新核验 Codex app connection 的自动 compact 预算。OpenAI/codex 官方仓库 issue #10365 中有 maintainer 说明 `model_auto_compact_token_limit` 存在但不建议随意修改，因为阈值随模型变化；issue #16033 报告显式 `model_context_window` / `model_auto_compact_token_limit` 后 auto-compaction 不触发；issue #14456 说明 profile-scoped context/compact 键不受支持；issue #19409 专门记录 `gpt-5.5` 在 Codex Desktop/App 中出现 400k/1M catalog 与 session `258400` effective window 不一致。Tracevane runtime 也记录到 `gpt-5.5` 经 Gateway `/v1/responses` 返回 `context_length_exceeded`，同段日志没有 `/v1/responses/compact` 请求；本地 Codex session 记录显示 Codex 看到的 `model_context_window` 曾被 Gateway 配置覆盖到 950k/1M。阶段性决策：Codex app connection 暂时不再写入顶层 `model_context_window` / `model_auto_compact_token_limit`，并在应用配置时清理旧顶层键，让 Codex 自己按当前模型和运行时决定 auto compact；其它客户端的模型预算配置保持不变。后续是否需要按模型专项预算，以用户下一次现场复现反馈为准。本地回归覆盖 Codex 旧阈值清理和不再写入 compact/context 顶层键。
