...
- GLM `glm-5.2` 双端点矩阵已实测：Chat 端点 `https://open.bigmodel.cn/api/coding/paas/v4` 可承接 Codex Responses->Chat adapter 和 Claude Code Anthropic->Chat adapter；Anthropic 端点 `https://open.bigmodel.cn/api/anthropic/v1/messages` 可承接 Claude Code 原生 Anthropic passthrough，也可承接 Codex Responses->Anthropic adapter。修复点是 adapter 路由必须使用上游原生协议 endpoint override，例如 Codex `/v1/responses` 转 Anthropic provider 时要使用 `endpoints.anthropic_messages=/v1/messages`，不能退回默认 `/messages`。
