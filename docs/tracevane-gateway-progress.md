...
- 本轮检查 Codex 自动 compact 配置：Gateway runtime 记录到 `gpt-5.5` 经 `/v1/responses` 返回 `context_length_exceeded`，且同段日志没有 `/v1/responses/compact` 请求，说明撞窗前 Codex 没触发自动 compact。进一步核验 OpenAI/codex 官方仓库 issue #10365、#16033、#14456、#19409：`model_auto_compact_token_limit` 是存在的配置键，但 maintainer 不建议随意修改；显式 context/compact 配置、profile 作用域、`gpt-5.5` catalog/session effective window 漂移都已有相关报告。按用户最新决策，本轮不继续固定或猜测 Codex compact 阈值：Codex app connection 暂时不再写入顶层 `model_context_window` / `model_auto_compact_token_limit`，应用配置时会清理旧顶层键，让 Codex 自己按当前模型和运行时决定 auto compact；其它客户端的模型预算配置保持不变。后续是否需要模型专项预算，等待下一次现场复现反馈。
