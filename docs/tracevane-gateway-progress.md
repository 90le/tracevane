...
- Gateway account provider 初版：
  ...
  - Gateway provider/endpoint Retry-After 收口：普通 passthrough JSON 错误会把上游 `error.code` 写入 runtime；HTTP 429 / `Retry-After` 会立即打开 provider/endpoint circuit 并持久化 `retryAfterUntil`，route selection 在到期前不 probe，已有 backup provider 或 backup endpoint 时直接回退。
  ...
  - Gateway endpoint profile 错误归属补齐：回归锁定 endpoint profile 命中后遇到 HTML/非 JSON HTTP 错误时，passthrough 与 adapter-required 路径的响应 header、runtime log、upstreamUrl 和 health 都归属到具体 endpoint profile，而不是只落到 provider。
...
- 本轮验证通过：`node --test tests/system/channel-connectors-service.test.mjs`，127/127 通过，覆盖默认 persistent、显式 one-shot fixture、busy/delivery guard、process runner、Feishu/Octo daemon、Codex app-server、Claude/OpenCode persistent compact、fallback 和 stop。
...
- 本轮补强 provider smoke 串行化：发现 active-route smoke 和 protocol-matrix smoke 都会临时改 active provider，若并发运行会互相覆盖并恢复到错误状态，导致“测试说通过但用户真实配置被污染”的假结论。`scripts/smoke-model-gateway-active-routes.mjs` 现先获取本地跨进程锁，再读取/修改 activeProviders；已有锁会在修改前返回 `model_gateway_active_route_smoke_lock_timeout` JSON 失败，系统测试覆盖成功锁元数据、锁等待失败零写入、失败后恢复、restore mismatch。
- 本轮补强空工具分片防护：针对用户反馈 GLM/Codex 出现 `call` 残片后任务未继续推进，检查 Chat-compatible streaming adapter 后发现空 `delta.tool_calls` 或无工具内容的 `finish_reason=tool_calls` 可能被转成假 `function_call` / `tool_use`，让 Codex/Claude Code 进入错误的工具等待状态。已在 Codex Responses SSE 与 Anthropic Messages SSE adapter 中忽略完全空的 tool delta，并在非流式/流式 Anthropic 映射中只在真实工具存在时输出 `tool_use` stop reason；新增系统回归覆盖空 tool delta、无工具 finish reason、正常单工具/并行工具分片不回退。
- 本轮真实工具验证：重启 `tracevane-model-gateway.service` 后执行 `node scripts/smoke-model-gateway-protocol-matrix.mjs --json --timeout-ms 240000` 通过，GLM `coding-anthropic`、GLM `coding-chat`、Codex account Responses 均 `ok=true`；执行 `node scripts/smoke-channel-connectors-agent-runner-direct.mjs --agents codex,claude-code,opencode --model glm/glm-5.2 --timeout-ms 360000 --min-process-replies 1 --min-tool-outputs 3 --json` 通过，三个 agent 均完成 3 次真实 shell 工具调用、3 个 stdout 工具输出和最终总结。
- 本轮继续补强 Responses 反向桥：检查 Responses SSE -> Chat / Anthropic streaming adapter 后发现孤立空 `response.function_call_arguments.delta/done` 也可能创建默认 `call_x/tool` 假工具块。现要求 Responses 工具事件必须有既有工具状态或 item/call/name 身份才会创建工具生命周期；`response.completed.output` fallback 同样跳过无身份工具项。新增回归覆盖空 Responses 工具事件不会输出 Chat `tool_calls` 或 Anthropic `tool_use`，正常 Responses 工具流仍通过。
- 本轮继续补强 Anthropic 反向桥：复查 Anthropic Messages SSE -> Chat / Responses streaming adapter 后发现另一类假工具风险：缺少 `id`/`name` 的 `content_block_start` `tool_use` 空壳、孤立 `input_json_delta`、以及只有 `message_delta.stop_reason=tool_use` 但没有真实工具块的响应，都会被旧逻辑补成默认 `call_x/tool`。现改为只有可用 `tool_use` 身份才创建工具状态，孤立参数 delta 直接忽略，Chat finish reason 只有真实工具存在时才输出 `tool_calls`；新增系统回归覆盖空 Anthropic 工具流和非流式空工具 stop，不再把占位事件传给客户端。
- 本轮继续收紧半身份工具调用：非流式 Chat/Anthropic/Responses adapter 和 streaming Responses/Anthropic helper 现在都要求工具调用同时有调用 id 与工具名，才会映射为跨协议工具生命周期。缺失 `id` 的 Anthropic `tool_use`、缺失 `call_id` 的 Responses `function_call`、缺失 `id` 的 Chat `tool_calls` 不再由 Gateway 合成默认 id/name 后暴露给客户端，避免下一轮 tool result 无法与上游真实调用对账。新增回归覆盖非流式和流式半身份 Anthropic/Responses 工具事件，并确认合法工具流、并行工具流、history restore 仍通过。
- 本轮 protocol matrix 稳定性：真实矩阵曾暴露 GLM Anthropic endpoint 偶发 `fetch failed`，而单路由 GLM Anthropic 和 GLM Chat proof 均可通过；因此将 `smoke-model-gateway-protocol-matrix.mjs` 的 GLM stage 拆为 `glm-anthropic-messages` 与 `glm-chat-completions` 两个独立 stage，再加 Codex account Responses stage。拆分后真实 `node scripts/smoke-model-gateway-protocol-matrix.mjs --json --timeout-ms 240000` 三项 proof 均 `ok=true`，失败时也能明确归因到具体 endpoint。
- 本轮继续清理孤儿工具结果：Chat `tool` message、Anthropic `tool_result`、Responses `function_call_output` 缺少 call id 时都会被 adapter 丢弃，不再传给上游模型。新增系统回归覆盖 Chat -> Responses、Chat -> Anthropic、Anthropic -> Chat、Codex Responses -> Chat 四条路径，确保无 id 工具结果不会成为上游 `function_call_output` / `tool_result` / `role=tool` 历史。`smoke-model-gateway-active-routes.mjs` 也新增 transient proof retry：仅对 `fetch failed` / reset / timeout 类 smoke 失败默认重试一次，并在结果里记录 `attempts`；本轮真实 protocol matrix 中 GLM Anthropic 第一次 fetch failed、第二次成功，最终三项 proof 均 `ok=true`。
- 本轮继续补强 Chat-compatible streaming 工具分片乱序：Codex Responses SSE 和 Anthropic Messages SSE adapter 现在会按 upstream tool index 缓冲“arguments 先到、id/name 后到”的 Chat `delta.tool_calls`，只有真实 `id + name` 到齐后才发出 Responses `function_call` 或 Anthropic `tool_use`；如果整条流只有 arguments、没有工具身份，则丢弃该孤立参数，不再合成默认 `call_x/tool`。随后继续把底层 streaming helper 收紧为“新建工具块必须有真实 id/name，缺失则返回 null 并忽略”，避免未来新增路径误用 helper 时重新制造 synthetic 工具身份。新增系统回归覆盖 Codex 与 Anthropic 两条路径的乱序参数、孤立 `call call call` 参数和正常工具流，避免 GLM/Codex 场景中 `call` 残片被升级成假工具等待并导致任务提前结束。
- 本轮继续补强 Responses SSE 终态合同：Responses SSE -> Chat / Anthropic adapter 现在必须看到 `response.completed` 才能输出正常 Chat `stop` 或 Anthropic `end_turn/message_stop`。如果上游只输出 partial delta 后直接 `[DONE]` 或断流，不再伪装成成功结束，而是向已开始的客户端流写入协议内 error，并把 runtime requestLog 记为 failure，错误码为 `model_gateway_responses_stream_missing_completed`。新增系统回归覆盖 truncated Responses stream 不会被 finalize 成成功。
- 本轮继续补强 Chat SSE 终态合同：Chat-compatible SSE -> Codex Responses / Anthropic Messages adapter 现在必须看到最终 `finish_reason` 才能正常完成。若上游只输出 partial delta 后直接 `[DONE]` 或断流，Gateway 会输出 Responses `response.failed` 或 Anthropic `error`，并把 runtime requestLog 记为 failure，错误码为 `model_gateway_chat_stream_missing_finish_reason`。新增系统回归覆盖 truncated Chat stream 不会被 finalize 成 Codex `response.completed` 或 Anthropic `message_stop`。
- 本轮继续补强 Anthropic SSE 终态合同：Anthropic Messages SSE -> Chat adapter 现在必须看到 `message_stop` 才能输出正常 Chat `stop`。若上游只输出 `content_block_delta` 后断流，Gateway 会向已开始的 Chat SSE 写入 error，并把 runtime requestLog 记为 failure，错误码为 `model_gateway_anthropic_stream_missing_message_stop`。新增系统回归覆盖 truncated Anthropic stream 不会被 finalize 成 Chat `finish_reason=stop`。
- 本轮检查 Codex 自动 compact 配置：Gateway runtime 记录到一次 `gpt-5.5` 经 `/v1/responses` 返回 `context_length_exceeded`，且同段日志没有 `/v1/responses/compact` 请求，说明当时 Codex 没在撞窗前触发自动 compact；OpenAI/codex issue #10365、#16033、#14456、#19409 也提示显式阈值、profile 作用域和 effective window 漂移都存在历史坑位。但进一步查官方 manual 与源码后，不能把这个现场失败归因成“Codex 不支持预算键”：官方 config 示例列出 `model_context_window` / `model_auto_compact_token_limit`，源码会按 resolved context 派生/夹紧 compact 阈值，scope 只接受 `total` / `body_after_prefix`。
- 本轮继续用真实 Codex 小预算实测纠正方向：`codex exec --json --ephemeral -c model_context_window=2000 -c model_auto_compact_token_limit=200 -c model_auto_compact_token_limit_scope="total"` 可运行，resume 测试 session JSONL 中记录 `model_context_window:1900` 与 `context_compacted`，证明顶层预算键进入运行时并能触发 auto compact。因此 Gateway 恢复并保留 Codex App Connection 预算写入：按所选 app model 合并 catalog/profile 预算后写 `model_context_window` 与 `model_auto_compact_token_limit`；前端恢复“统一写入 Codex、Claude Code、OpenCode、OpenClaw 的模型、预算和运行偏好”语义，静态和系统回归锁定该行为。
- 本轮继续优化 Provider Center 大模型候选选择：识别到几百个模型时，候选列表仍分页渲染；批量按钮从“选择当前筛选”改为“选择当前可见”，实现只遍历 `visibleDetectedCandidateModels`，避免用户搜索/筛选后误把未渲染的整套候选都导入配置。静态回归锁定文案和遍历对象。
- 本轮继续优化模型配置表批量编辑：预算和能力批量按钮从“应用到全部”改为“应用到可见”，实现只遍历 `visibleDraftModelRowEntries`，避免分页隐藏模型在用户不可见时被批量覆盖。静态回归锁定该行为。
- 本轮继续加固 provider 自动识别：`detect-provider` 现在把用户传入 timeout clamp 到 1s-30s，并用同一个 timeout 覆盖 headers + body 读取；单个 `/models` 或协议 probe 响应 body 最大 2MB，超限会记录为结构化 probe failure，不再让管理接口因超大/慢速模型目录卡住。系统回归覆盖 oversized `/models` 不持久化 registry/secrets。
...
- 本轮当前会话救援收口：短暂保留过 filesystem rescue alias 以避免已打开 Codex TUI 在旧 cwd 缺失时报错；确认当前会话和 dev 进程均使用 `/home/binbin/.openclaw/extensions/tracevane` 后，旧 alias 已移除。扩展目录下只保留 Tracevane 仓库目录；真实配置、服务和历史 metadata 均以 Tracevane 新目录为准。
