...
- Codex Chat-compatible bridge 已恢复 custom/freeform tool 语义：请求发往 Chat endpoint profile 时可临时映射为 function tool `{ input }`，但 Chat response、Chat SSE 和 codex history restore 回 Codex 时会恢复 `custom_tool_call` / `custom_tool_call_output`；真实 `claude-opus-4-8` Codex smoke 已验证 `apply_patch -> cat -> printf` 三步工具链最终返回 `CUSTOM_CHAIN_DONE`，不再报 `tool apply_patch invoked with incompatible payload` 或工具后提前结束。
