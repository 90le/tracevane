...
## 2026-06-22 重建决策（取代逐页 raw 迁移）

`apps/web-vue` 的"原型 HTML 片段 raw 渲染 + page-mounts + 手写 hash 路由"方案已被判定为功能不足、架构混乱、与 Aurora 偏离、工程基础薄弱，决定整体重建：

- 删除 `apps/web-vue`，新建 `apps/web`（React + Vite + Tailwind v4 + **shadcn/ui 真正落地** + React Router 真路由 + TanStack Query 类型化数据层），feature-sliced 架构。
- Aurora 视觉语言重建为 Tailwind v4 `@theme` token，shadcn/Radix 组件主题化到 Aurora；原型 `styles.css`/`pages/*.html` 作为 token 与 IA 真相源。
- 后端优先只消费现有 API；确需聚合视图时允许新增 BFF，但先按 research-first 记录。
- 采用大爆炸式：先迁移发布路径（installer/pack/workspace/plugin/dist/dev）→ 删 `web-vue` → 脚手架 `apps/web` → **优先做满 Model Gateway**（按 `模型网关信息架构契约.md` + `模型网关详细目标.md`）→ 验证 → 其余域逐计划补齐。
- 设计与计划：`docs/superpowers/specs/2026-06-22-frontend-rebuild-design.md`、`docs/superpowers/plans/2026-06-22-frontend-rebuild-model-gateway.md`。

执行状态（分支 `frontend-rebuild`）：重建已落地。`apps/web-vue` 已删除，`apps/web` 已脚手架并完成设计系统（Aurora token + shadcn 组件 + 共享布局/三态）、应用壳/HashRouter/typed API client/数据层，以及 **Model Gateway 七个 view 全部实现**（overview/providers/providercfg/models/accounts/apps/usage）。发布工具链与系统测试已随新结构 green-up，全仓库无 `web-vue` 残留；typecheck/build、web 合同测试、pack dry-run 与 dev 运行态均通过。浏览器端 Playwright smoke 待在有浏览器环境补跑；其余域按后续计划补齐。

下文的逐页迁移记录保留为历史背景；新增前端工作以本节为准。
