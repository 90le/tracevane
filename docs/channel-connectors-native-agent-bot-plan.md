...
- Feishu 长连接按 CC Go、最新 OpenClaw、官方 SDK 与真实故障证据约束：同 App 只允许一个本机 WS owner，进程内扇出、跨进程用用户级全局 lock 防多连接；每个 owner cycle 使用官方 SDK `WSClient` + `EventDispatcher`；默认启用 SDK lower-case `pingTimeout=3`，避免半开 socket 继续显示 `connected`；SDK terminal error 后按 OpenClaw 外层循环关闭当前 client 并 `1s..30s` 退避重建；SDK 已进入 `reconnecting` 且超过 5s 时也回收到同一外层循环；SDK 继续发 ping 但超过 `pongTimeoutMs=8000` 未收到 pong 时回收当前 client；有效 ping 默认 clamp 到 10s，23s control-frame stale 判死；不做 connected-idle / zero-inbound / verified-ingress / generic watchdog 重建；raw event / dispatcher callback 只作为业务入站诊断，默认不因空闲期无业务消息重建；Feishu `create_time` 与会话水位线会跳过补投旧消息；runtime 暴露 `connected`、`ingressState`、raw event、pong overdue、global/lifecycle `dispatcherCallbacks`、owner lock 和入站验证状态；`/health` 不再把 `silent` 或 `pongOverdue` owner 视为健康；假在线问题按 `feishu-long-connection-issue-tracker.md` 专项跟踪，不以手工重启作为完成标准。
...
测试策略：Codex 真实 smoke 使用隔离 `CODEX_HOME`，不改用户正式配置；Claude Code / OpenCode native persistent smoke 使用隔离 `HOME` + mock Studio Gateway，不改用户正式配置。
...
| F4 | 进行中：长回复拆分、Feishu thread/reply session、附件 metadata/staging、Octo URL staging、Feishu/Octo 出站文件、Octo COS 直传与自动分流、Octo CC Go 长连接基线、图片非视觉模型保护、Gateway vision 模型自动选择、Codex 原生图片输入、OpenCode 原生图片 `--file` 输入、轻量 history context、群聊 context、长回复 group buffer、reply buffer 查看命令/菜单、飞书群成员拉取、Feishu 会话列表/切换子卡、`/current`/`/list`/`/history [n]` 信息增强、Feishu/Octo 私聊进度与群聊静默默认、assistant 过程回复展示、`/thinking`/`/process`/`/tools` 三路显示控制、持久 session driver pool 合同/状态面/Codex app-server 真实 Gateway `turn/start`、`/compact`、`turn/interrupt` smoke、Codex app-server requestApproval driver 合同、Claude Code stream-json persistent 普通 turn/Bash tool-use/文件 manifest/compact/stop cancel、Claude tool_result 纯输出渲染、OpenCode `run --session` persistent 普通 turn/文件 manifest/compact/stop cancel、OpenCode SQLite fallback、daemon `/stop`、内部 userMessage 过滤、生命周期进度噪音过滤和 unfinished turn 超时中断已完成；当前 live binding 默认仍是 one-shot，真实 IM approval smoke、真实 Claude/OpenCode 权限/视觉、真实 IM 文件/工具流式复验、OCR、语音 STT/TTS、长回复预览冻结、真实大文件限额验证、更多设置型卡片待补 |
...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 已覆盖 thread 复用、连续 turn、原生 `/compact`、stop/interrupt、内部 prompt echo 过滤、生命周期进度噪音过滤、unfinished turn 超时中断、permission mode -> approvalPolicy/sandbox，以及 `item/commandExecution/requestApproval`、`item/fileChange/requestApproval`、`item/permissions/requestApproval` driver 回包合同，并通过真实 Studio Gateway `turn/start`、`/compact` 与 `turn/interrupt` live smoke；真实 IM approval smoke 还未完成。Claude Code stream-json 和 OpenCode `run --session` 已按 CC Go `AgentSession.Send(CompressCommand())` 合同补 persistent `/compact` 回归，并通过隔离 HOME + mock Gateway 真实 CLI 普通 turn/文件 manifest/compact/stop smoke；Claude Code 额外覆盖 Bash tool-use，且 `user/tool_result` 纯输出不会再被进度渲染吞掉首行或显示“无输出”；OpenCode 额外覆盖空 stdout SQLite fallback，且取消路径不会读取旧 `opencode.db` 输出；OpenCode runner 现在会把 Studio Gateway 短模型自动转成 `studio-gateway/<model>`，每轮生成隔离 OpenCode config，并把 session 数据放在 Channel runtime dataHome；旧全局 sessionId 在当前 dataHome 不存在时自动新建，避免 IM 切换 OpenCode 后被 stale session 卡死；OpenCode error envelope 会显示真实 `data.message/name/ref`；本机真实 Gateway + `glm-5` 短对话已通过。OpenCode 视觉输入已按 CC Go `--file` 图片参数构建。metadata 请求 persistent 会在 `/status` / runtime 中显示；正式 live 默认仍保留 one-shot，按 CC Go 的各 Agent 子进程模型继续补成熟度。
...
- Feishu 长连接稳定性已按最新 OpenClaw、Lark SDK 与 live 故障证据进一步收敛：同 App 共享 WS、消息快速 ACK 后后台派发；默认启用 SDK lower-case `pingTimeout=3`，并包装 SDK `pingLoop()` 将有效心跳调度 clamp 到 10s；terminal error、超过 5s 的 SDK `reconnecting`、`pongTimeoutMs=8000` overdue 和 23s control-frame stale 都回到外层重建循环；Feishu `create_time` 与会话水位线会跳过补投旧消息；zero-inbound / connected-idle / verified-ingress / generic watchdog 重建默认关闭；用户级全局 owner lock 防止同 App 多进程随机抢投递；runtime 暴露 raw event、dispatcher callback、`pongOverdue` 和 `transportStale` 诊断；前端会话行已显示绑定对应的 Feishu 长连接健康，避免把 Agent session 空闲误认为平台连接正常。假在线问题进入专项文档继续验收。
...
- Feishu card/menu 已按 CC 卡片结构拆成主菜单 Dashboard、分组菜单和可操作子卡；主菜单已精简为入口按钮网格，Session 页拆分查看区/执行区，callback toast 使用页面/动作级短回显。Current Session / History / Agent Sessions 子卡读取真实 Agent session 与 IM history store，`/list` 和 `/switch <序号|sessionId前缀>` 可在当前 IM session 内切换已知续接；Session List 已补 CC Go delete-mode `checker` 批量删除表单，Feishu `form_value` 统一落到 `/delete id1,id2`。普通 slash、`%help` 兼容命令与卡片点击共用 command-router；Agent 运行支持 processing reaction、单张 Progress card send/patch、`command_execution` 工具过程展示、`/thinking`、`/process` 与 `/tools` 开关，以及 upstream JSON error envelope 清洗和失败去重。
- Feishu/Octo 进度显示已按 CC group buffer 语义统一：私聊默认显示过程回复、思考和工具过程；群聊默认不发送中间过程，只发送最终回复或最终错误；显式 `/thinking` 控制思考、`/process` 控制过程回复、`/tools` 控制工具；过程回复只接受 `assistant/intermediate`，Codex app-server delta 和 Codex/Claude/OpenCode 最终回复不会再重复进入过程卡片/进度文本；Feishu 卡片和 Octo 私聊文本已按 CC progress renderer 优化工具输入、工具结果、exit/status 和 TodoWrite 展示，Octo 工具事件优先发送避免被思考进度节流吞掉。
