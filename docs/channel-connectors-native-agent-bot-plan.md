...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 已覆盖 thread 复用、连续 turn、原生 `/compact`、stop/interrupt、内部 prompt echo 过滤和 unfinished turn 超时中断合同，并通过真实 Studio Gateway `turn/start`、`/compact` 与 `turn/interrupt` live smoke。metadata 请求 persistent 会在 `/status` / runtime 中显示并走 Codex app-server 实验路径；正式 live 默认仍保留 one-shot。
