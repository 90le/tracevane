...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 原型已覆盖 thread 复用、连续 turn、原生 `/compact` 和 stop/interrupt 的 fake transport 合同。metadata 请求 persistent 会在 `/status` / runtime 中显示，但当前 effective mode 仍是 one-shot，尚未接入默认 daemon 路径。
