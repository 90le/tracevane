...
> 状态：Studio 原生 Channel daemon 路线；Octo/Feishu 基础闭环、CC-style 菜单/会话子卡和 native CLI persistent compact real CLI smoke 已完成
> 更新：2026-06-10
...
| F4 | 进行中：长回复拆分、Feishu thread/reply session、附件 metadata/staging、Octo URL staging、Feishu/Octo 出站文件、Octo COS 直传与自动分流、Octo CC Go 长连接基线、图片非视觉模型保护、Gateway vision 模型自动选择、Codex 原生图片输入、轻量 history context、群聊 context、长回复 group buffer、reply buffer 查看命令/菜单、飞书群成员拉取、Feishu 会话列表/切换子卡、`/current`/`/list`/`/history [n]` 信息增强、Feishu/Octo 私聊进度与群聊静默默认、持久 session driver pool 合同/状态面/Codex app-server 真实 Gateway `turn/start`、`/compact`、`turn/interrupt` smoke、Claude Code stream-json persistent 普通 turn/Bash tool-use/文件 manifest/compact、OpenCode `run --session` persistent 普通 turn/文件 manifest/compact/stop cancel、OpenCode SQLite fallback、daemon `/stop`、内部 userMessage 过滤、生命周期进度噪音过滤和 unfinished turn 超时中断已完成；当前 live binding 默认仍是 one-shot，继续补真实 Claude/OpenCode 权限/视觉、Claude Code stop、真实 IM 文件/工具流式复验、OCR、语音 STT/TTS、长回复预览冻结、真实大文件限额验证、更多设置型卡片 |
...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 已覆盖 thread 复用、连续 turn、原生 `/compact`、stop/interrupt、内部 prompt echo 过滤、生命周期进度噪音过滤和 unfinished turn 超时中断合同，并通过真实 Studio Gateway `turn/start`、`/compact` 与 `turn/interrupt` live smoke。Claude Code stream-json 和 OpenCode `run --session` 已按 CC Go `AgentSession.Send(CompressCommand())` 合同补 persistent `/compact` 回归，并通过隔离 HOME + mock Gateway 真实 CLI 普通 turn/文件 manifest/compact smoke；Claude Code 额外覆盖 Bash tool-use；OpenCode 额外覆盖 stop cancel，且取消路径不会读取旧 `opencode.db` 输出；OpenCode 1.16 空 stdout 时会 best-effort 读取 `opencode.db` 恢复输出。metadata 请求 persistent 会在 `/status` / runtime 中显示；正式 live 默认仍保留 one-shot，按 CC Go 的各 Agent 子进程模型继续补成熟度。
...
1. 扩展真实 Claude Code / OpenCode persistent smoke：权限批准、视觉输入、Claude Code stop 和 IM live 文件上传。
2. 按 `channel-connectors-cc-migration-checklist.md` 的 P1 顺序继续复刻 Feishu/Octo 菜单、设置卡片、长连接与媒体收发细节。
3. 继续复验 Codex `exec/resume` 稳定 live 路径：Feishu/Octo 发文件、工具流、Markdown 最终回复。
