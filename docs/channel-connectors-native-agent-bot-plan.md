...
- Feishu 长连接按 CC Go、最新 OpenClaw、官方 SDK 与真实故障证据约束：同 App 只允许一个本机 WS owner，进程内扇出、跨进程用用户级全局 lock 防多连接；每个 owner cycle 使用官方 SDK `WSClient` + `EventDispatcher`；默认启用 SDK lower-case `pingTimeout=3`，避免半开 socket 继续显示 `connected`；SDK terminal error 后按 OpenClaw 外层循环关闭当前 client 并 `1s..30s` 退避重建；SDK 已进入 `reconnecting` 且超过 10s 时也回收到同一外层循环；SDK 继续发 ping 但超过 `pongTimeoutMs=15000` 未收到 pong 时回收当前 client；不做 connected-idle / zero-inbound / verified-ingress / generic watchdog 重建；raw event / dispatcher callback 只作为业务入站诊断，默认不因空闲期无业务消息重建；runtime 暴露 `connected`、`ingressState`、raw event、pong overdue、global/lifecycle `dispatcherCallbacks`、owner lock 和入站验证状态；`/health` 不再把 `silent` 或 `pongOverdue` owner 视为健康；假在线问题按 `feishu-long-connection-issue-tracker.md` 专项跟踪，不以手工重启作为完成标准。
