...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 已覆盖 thread 复用、连续 turn、原生 `/compact` 和 stop/interrupt 的 fake transport 合同，并通过真实 Studio Gateway `turn/start` 与 `/compact` live smoke。metadata 请求 persistent 会在 `/status` / runtime 中显示并走 Codex app-server 实验路径，默认仍保留 one-shot。
