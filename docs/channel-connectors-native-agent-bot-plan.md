...
| F4 | 进行中：长回复拆分、Feishu thread/reply session、附件 metadata/staging、Octo URL staging、Feishu/Octo 出站文件、Octo COS 直传与自动分流、Octo CC Go 长连接基线、图片非视觉模型保护、Gateway vision 模型自动选择、Codex 原生图片输入、OpenCode 原生图片 `--file` 输入、轻量 history context、群聊 context、长回复 group buffer、reply buffer 查看命令/菜单、飞书群成员拉取、Feishu 会话列表/切换子卡、`/current`/`/list`/`/history [n]` 信息增强、Feishu/Octo 私聊进度与群聊静默默认、持久 session driver pool 合同/状态面/Codex app-server 真实 Gateway `turn/start`、`/compact`、`turn/interrupt` smoke、Codex app-server requestApproval driver 合同、Claude Code stream-json persistent 普通 turn/Bash tool-use/文件 manifest/compact/stop cancel、Claude tool_result 纯输出渲染、OpenCode `run --session` persistent 普通 turn/文件 manifest/compact/stop cancel、OpenCode SQLite fallback、daemon `/stop`、内部 userMessage 过滤、生命周期进度噪音过滤和 unfinished turn 超时中断已完成；当前 live binding 默认仍是 one-shot，真实 IM approval smoke、真实 Claude/OpenCode 权限/视觉、真实 IM 文件/工具流式复验、OCR、语音 STT/TTS、长回复预览冻结、真实大文件限额验证、更多设置型卡片待补 |
