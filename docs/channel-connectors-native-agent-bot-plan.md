...
- Feishu/Octo 进度显示已按 CC group buffer 语义统一：私聊默认显示过程回复、思考和工具过程；群聊默认不发送中间过程，只发送最终回复或最终错误；显式 `/thinking` 控制思考、`/process` 控制过程回复、`/tools` 控制工具；Feishu 卡片和 Octo 私聊文本已按 CC progress renderer 优化工具输入、工具结果、exit/status 和 TodoWrite 展示，Octo 工具事件优先发送避免被思考进度节流吞掉。
