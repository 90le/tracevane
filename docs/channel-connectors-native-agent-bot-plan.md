...
> 状态：已切换为 Studio 原生实现路线；F4 long-reply split started
> 更新：2026-06-06
...
| F4 | 进行中：长回复拆分已完成；继续补图片/文件、语音、群聊 mention、thread/reply、长回复预览冻结、流式预览 |
...
1. F4：补图片/文件、群聊成员/history context、长回复 group buffer 和治理策略。
2. 继续迁移 CC/OpenClaw 的 thread isolation、文件/图片和多平台 adapter。
