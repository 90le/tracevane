...
- F4 Octo 出站媒体合同已落地：参考 OpenClaw Octo 插件 COS STS 能力与 bot-api skill，transport smoke 支持 `upload-file`、`upload-and-send-media`、`upload-credentials`、`direct-upload-file` 和 `direct-upload-and-send-media`；`upload-and-send-media` 默认走 STS + COS PUT 直传，图片使用 Octo image payload，普通文件使用 file payload；显式 `octoUploadStrategy=multipart` 才保留旧 `/v1/bot/file/upload`，auto 遇到旧 multipart 404/405/410 会回退直传；本机 `studio-cc` 小文本文件、STS 和 COS 直传真实 smoke 已通过。
