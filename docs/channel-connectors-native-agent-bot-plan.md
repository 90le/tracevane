...
- Channel daemon 后续持久 session driver 已先锁 pool 合同并接入状态面；Codex app-server driver 已覆盖 thread 复用、连续 turn、原生 `/compact`、stop/interrupt、内部 prompt echo 过滤、生命周期进度噪音过滤、unfinished turn 超时中断和 permission mode -> approvalPolicy/sandbox 合同，并通过真实 Studio Gateway `turn/start`、`/compact` 与 `turn/interrupt` live smoke；运行中的 `item/*/requestApproval` pending approval 回包还未迁移。Claude Code stream-json 和 OpenCode `run --session` 已按 CC Go `AgentSession.Send(CompressCommand())` 合同补 persistent `/compact` 回归，并通过隔离 HOME + mock Gateway 真实 CLI 普通 turn/文件 manifest/compact/stop smoke；Claude Code 额外覆盖 Bash tool-use，且 `user/tool_result` 纯输出不会再被进度渲染吞掉首行或显示“无输出”；OpenCode 额外覆盖空 stdout SQLite fallback，且取消路径不会读取旧 `opencode.db` 输出；OpenCode 1.16 空 stdout 时会 best-effort 读取 `opencode.db` 恢复输出；OpenCode 视觉输入已按 CC Go `--file` 图片参数构建。metadata 请求 persistent 会在 `/status` / runtime 中显示；正式 live 默认仍保留 one-shot，按 CC Go 的各 Agent 子进程模型继续补成熟度。
...
1. 按 CC Go 迁移 Codex app-server `item/*/requestApproval` pending approval 与 IM `/approve` 回包闭环。
