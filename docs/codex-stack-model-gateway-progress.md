...
- Phase B2 仍需持续按 `/tmp/cc-switch-src` 对齐剩余 error envelope 边缘行为；strict smoke 已覆盖 CLI 启动、Claude tool/summary、OpenClaw agent provider/model/usage、Gateway HTTP compact/tool-history/error envelope、Responses->Chat streaming `include_usage`、provider-declared reasoning/thinking 映射、parallel tool-call index grouping、Chat SSE error -> Responses `response.failed`、BigModel Chat/Anthropic live provider matrix，以及 GMN Responses-native substitute `/v1/responses` + `/v1/responses/compact` live proof。
...
- Live smoke 新增 `gmn-responses` provider，并用 GMN `gpt-5.4` 完成 Responses-native 替代验收：basic、stream、compact、error-envelope。
...
1. 继续按 `/tmp/cc-switch-src` 做剩余 error envelope 边缘行为对齐。
