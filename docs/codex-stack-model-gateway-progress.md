...
- 已补入 daemon survivability 目标、status lifecycle contract、最小 daemon entrypoint 和 supervisor/install API contract；当前仍缺真实 service manager 启停验证、UI/安装脚本接管和真实崩溃恢复测试。
...
- 当前状态：目标、进度跟踪、shared type、status API contract、daemon entrypoint、supervisor template/API contract 和 system test 已补齐；真实 supervisor start/restart、UI health、安装脚本接管和真实 crash-survivability tests 尚未实现。
...
1. 将 daemon service contract 接入 Codex Stack 安装/接管流程：让一键安装写入 daemon service template，Codex takeover 默认指向 daemon loopback endpoint，而不是 CPA/compact proxy 或 OpenClaw 单口 mount。
