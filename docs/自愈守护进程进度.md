...
- 低优先级附带能力：当 OpenClaw Gateway 能启动但 Tracevane 插件 `/tracevane` 控制面的前端静态资源缺失时，可受控检查/重建 Tracevane web bundle；这只用于保证 OpenClaw 托管 Tracevane UI 可打开，不属于 OpenClaw 本体配置修复。
- 本轮补齐 OpenClaw `2026.6.8` 后发现的配置漂移修复：Gateway auth 会迁移到 `OPENCLAW_GATEWAY_TOKEN` env SecretRef，并同步 `.openclaw/.env` 与 `gateway.systemd.env`；重复的 `tracevane-local` `gatewayAuthToken` 会删除，避免多 token 来源互相覆盖。
...
- Recovery 备份现在会同时保存 `openclaw.json`、`.env`、`gateway.systemd.env` 和 `tracevane-local` secret sidecar；失败回滚会一并恢复，避免只回滚主配置而留下新的 token/env 状态。
