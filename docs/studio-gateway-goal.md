...
- Claude Code 权限验收必须证明：`control_request` 不能只作为进度展示，必须按 CC Go 合同回写 `control_response`；自动模式可 allow，保守模式必须 fail-safe deny 或经 IM 文本批准，后续继续迁移 Feishu 按钮卡片和 AskUserQuestion。
