...
- Feishu 长连接不得只相信 SDK `connected` 状态；Studio 必须记录真实 dispatcher callback、business message ingress 和后续 ping/pong 或等价健康证明；默认保持 OpenClaw 在当前 Lark SDK 下的有效行为，不启用 lower-case `pingTimeout` 额外 watchdog；所有 WS event handler 必须快速 ACK，业务处理后台化；死 socket 依赖 SDK/socket terminal lifecycle、官方重连与 OpenClaw-style 外层重建处理；启动期可用专项文档证明过的 bounded ingress validation 处理 `connected=true` 但未收到任何 dispatcher callback 的假在线场景，其他自动轮换必须先有专项文档证据。发布前必须评估并记录是否采用 webhook/hybrid ingress 或 Studio-owned WS transport 来达到 Octo 级稳定目标。
