...
> 状态：Gateway core matrix、daemon lifecycle、Provider Center、App Connections、真实 provider/CLI smoke 已完成；Channel Connectors 私聊迁移进行中
> 更新：2026-06-15
> 文档规则：本文件只保留目标、边界、验收和阶段计划；当前进度写到 `studio-gateway-progress.md`。

## 1. 最终目标

旧 `Codex Stack -> CPA / Compact / cc-connect` 混合链路停止演进。正式目标是：
...
- 生产前后端不再暴露 `/api/codex-stack/*`、`features/codex-stack/**`、CPA/Compact 诊断矩阵、安装修复页或路由模型页。
- CPA `18795`、旧 Compact Proxy `18796`、CPA Gateway 都不再是正式模型链路；`18796` 只可作为 Studio Gateway daemon 默认本地端口。
- `cc-connect` / CC / Octo(dmwork) 是 Channel Connectors 域，不属于模型 relay，也不放入 App Connections。
...
- 成熟度参考 `/tmp/cc-switch-src`：SSE 重建、tool/history 映射、usage/cache、reasoning/thinking、error envelope、并发 tool call。
...
- GPT/ChatGPT account 与 Codex account 都作为本机账户型 provider，不恢复旧 CPA / Compact / Codex Stack。
- 首期不做网页 cookie 抓取；优先复用官方 Codex CLI/ChatGPT 登录、device auth、`~/.codex/auth.json` / keyring 或隔离 `CODEX_HOME`。
...
- 生产前后端无用户可见 `codex-stack`、CPA、Compact 旧链路。

## 8. 阶段计划

| 阶段 | 状态 | 目标 |
| --- | --- | --- |
| Phase A | 完成 | 固定 Studio Gateway 命名、API contract、迁移删除清单 |
| Phase B | 完成 | 核心协议矩阵 adapter 与 daemon routes |
| Phase B2 | 完成核心，持续加固 | cc-switch 级 SSE/tool/history/usage/reasoning/error 成熟度；BigModel 与 GMN smoke 已覆盖主要格子 |
...
| Phase D2 | 新增目标 | GPT/ChatGPT/Codex account-backed provider：登录、账户池、刷新、sticky/failover、quota/cooldown、三协议导出 |
