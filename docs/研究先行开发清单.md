...
5. 冲突记录：把拒绝名称、拒绝原因和来源写入 `docs/产品战略重置方案.md`。
...
任何新增或重构主产品页面、Workspace IDE 能力、Agent task surface、预览/编辑工作流、Observability/Eval 面板、Channel 操作流或 Gateway 可视化配置前，必须先更新或引用 `docs/产品对标策略.md`：
...
| P0 | 产品战略重置文档 | 已完成本轮 | `产品战略重置方案.md`、PRD、架构、进展和 README 改为 Tracevane 方向 |
| P0 | 产品 benchmark 策略 | 已完成本轮 | `产品对标策略.md` 记录 AI IDE、CLI Agent、app builder、observability/eval、workflow/runtime 的市场吸收和拒绝规则 |
...
- 2026-06-22：前端整体重建决策。复核 React/Vite/Tailwind v4/shadcn/ui/React Router v7/TanStack Query 当前官方资料（`vite.dev/guide`、`tailwindcss.com/docs/installation/using-vite`、`ui.shadcn.com/docs/installation/vite` 与 `tailwind-v4`、`reactrouter.com`、`tanstack.com/query` React）。结论：`apps/web-vue` 的原型片段 raw 渲染 + 手写 hash 路由方案功能不足、架构混乱、与 Aurora 偏离，决定删除并重建为 `apps/web`：React + Vite + Tailwind v4 + shadcn/ui 真正落地（components.json + Radix）+ React Router 真路由 + TanStack Query 类型化数据层，feature-sliced 架构；Aurora 视觉重建为 Tailwind v4 token，shadcn 组件主题化到 Aurora。后端优先只消费现有 API，确需聚合视图才新增 BFF 并在此记录。采用大爆炸式：先迁移发布路径（installer/pack/workspace/plugin/dist/dev）→ 删 `web-vue` → 脚手架 `apps/web` → 优先做满 Model Gateway（按 `模型网关信息架构契约.md` + `模型网关详细目标.md`）→ typecheck/build/系统合同测试/Playwright 桌面+移动 smoke 验证 → 其余域逐计划补齐。设计与计划见 `docs/superpowers/specs/2026-06-22-frontend-rebuild-design.md`、`docs/superpowers/plans/2026-06-22-frontend-rebuild-model-gateway.md`。各 view 精确 JSX 在实现期按现有 API 形态与 Aurora 原型 DOM 核验后落地，未验证前不臆造契约。
