...
1. **IM Channels P0**：第一轮已把平台绑定从“能看”推进到“能创建/编辑/删除/测试”：支持账号/机器人 metadata 填写、脱敏保存、启停、Feishu/Octo transport smoke 和 daemon native 绑定证据。已按 `IM渠道前端设计契约.md` 完成信息架构重构：概览 / 平台账号 / 绑定路由 / 会话投递 / 守护诊断，重做宽 Drawer，按平台字段模板替代 JSON-first 表单，并降级 Agent profiles / daemon native bindings 为相关证据。下一步聚焦 guided setup、更多 verified smoke 与按绑定过滤的投递诊断。详见 `IM渠道目标与设计.md`。
