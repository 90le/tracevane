...
- usage：`status/runtime` 已有基于 request log 的 provider/model/account 聚合 summary，并记录 tokens、image generation、images generated、image edits、audio input/output request；后续长期账本再按 gateway key、provider、account hash、model、alias、route、status、latency、TTFT、usage/media units 和价格映射记录。Channel 侧不重复做 token 产品化。
