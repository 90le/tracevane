...
- Codex 官方认证文档：ChatGPT 登录、API key 登录、device auth、`~/.codex/auth.json` / keyring 存储。
...
- ChatGPT/GPT 账户不做网页 cookie 抓取或通用网页反代；首期只走官方 Codex CLI/ChatGPT 登录可产生的本机凭据，或后续官方允许的 access token。
- Codex 账户纳入同一账户型 provider；优先复用 `codex login` / device auth / `CODEX_HOME` 隔离目录，不污染当前 Codex 任务进程。
...
- 账户登录：Codex browser/device auth、导入本机 `auth.json`、隔离 `CODEX_HOME` 登录。
...
- UI：Provider Center 增加 Account providers 工作区，支持登录、导入、刷新、禁用、健康、quota、代理、模型 alias、sticky/cooldown 策略。

## 验收

- 无真实 token 的单元测试覆盖 account schema、redaction、auth storage、route decision、sticky/failover/cooldown、model list merge。
- 隔离 `CODEX_HOME` smoke 验证 Codex 账户导入和普通请求，不污染当前 Codex CLI。
