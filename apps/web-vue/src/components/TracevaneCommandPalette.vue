<template>
  <Teleport to="body">
    <div
      v-if="open"
      class="tracevane-command-palette"
      role="presentation"
      @keydown.esc.prevent="closePalette"
    >
      <button
        type="button"
        class="tracevane-command-palette__backdrop"
        :aria-label="text('关闭命令面板', 'Close command palette')"
        @click="closePalette"
      ></button>

      <section
        class="tracevane-command-palette__panel"
        role="dialog"
        aria-modal="true"
        :aria-label="text('命令面板', 'Command palette')"
      >
        <header class="tracevane-command-palette__header">
          <Command class="tracevane-command-palette__mark" aria-hidden="true" />
          <label class="tracevane-command-palette__search">
          ...
          <button
            type="button"
            class="tracevane-command-palette__close"
            :aria-label="text('关闭', 'Close')"
            @click="closePalette"
          >
            <X class="tracevane-command-palette__close-icon" aria-hidden="true" />
          </button>
        </header>

        <div class="tracevane-command-palette__body">
          <section
            v-if="visibleCommands.length"
            class="tracevane-command-palette__section"
          >
            ...
            <button
              ...
              :key="command.id"
              type="button"
              class="tracevane-command-palette__item"
              :class="{ active: command.id === activeCommandId }"
              @mouseenter="activeCommandId = command.id"
              @click="runCommand(command)"
          >
              <span class="tracevane-command-palette__item-icon" aria-hidden="true">
                <component :is="resolveCommandIcon(command.icon)" />
              </span>
              <span class="tracevane-command-palette__item-main">
                <strong>{{ command.label }}</strong>
                <small>{{ command.detail }}</small>
              </span>
              <span class="tracevane-command-palette__path">{{ command.category }}</span>
            </button>
          </section>

          <p v-if="!visibleCommands.length" class="tracevane-command-palette__empty">
            {{ text('没有匹配的命令。', 'No matching command.') }}
          </p>
        </div>

        <footer class="tracevane-command-palette__footer">
...
import './tracevane-command-palette.css';
...
const actionCommands = computed<CommandItem[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      detail: 'tracevane.90le.cn',
      run: () => {
        if (typeof window !== 'undefined') {
          window.open('https://tracevane.90le.cn', '_blank', 'noreferrer');
        }
      },
    },
  ];
