<template>
  <div ref="rootRef" class="tracevane-select" :class="{ open: isOpen, disabled, invalid, 'open-up': openDirection === 'up' }">
    <button
      type="button"
      class="tracevane-select-trigger"
      ...
      @click="toggleOpen"
    >
      <span class="tracevane-select-value">{{ selectedLabel || placeholder }}</span>
      <ChevronDown class="tracevane-select-chevron" aria-hidden="true" />
    </button>

    <Teleport v-if="teleport" to="body">
      <div v-if="isOpen" ref="menuRef" class="tracevane-select-menu tracevane-select-menu-portal" role="listbox" :style="menuStyle" @pointerdown.stop>
        <button
          v-for="option in options"
          :key="option.value"
          type="button"
          class="tracevane-select-option"
        ...
        >
          ...
          <Check v-if="option.value === modelValue" class="tracevane-select-check" aria-hidden="true" />
        </button>
      </div>
    </Teleport>

    <div v-else-if="isOpen" ref="menuRef" class="tracevane-select-menu" role="listbox" @pointerdown.stop>
      <button
        v-for="option in options"
        :key="option.value"
        type="button"
...
import './tracevane-select.css';

export interface TracevaneSelectOption {
  value: string;
  label: string;
}
