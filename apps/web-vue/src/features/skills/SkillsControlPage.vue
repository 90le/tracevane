    ...
    <section class="skills-workspace-strip">
      ...
      <div class="skills-mode-strip" role="tablist" :aria-label="text('技能视图切换', 'Skills mode switch')">
        ...
        <button
          ...
          :aria-selected="mode === 'local-install'"
...
const activeModeTitle = computed(() => {
  if (mode.value === 'marketplace') return marketplaceHeadline.value;
  if (mode.value === 'local-install') return uploadHeadline.value;
  return installedHeadline.value;
});
const activeModeSummary = computed(() => {
  if (mode.value === 'marketplace') return marketplaceCopy.value;
  if (mode.value === 'local-install') return uploadCopy.value;
  if (!summary.value) return installedCopy.value;
  return text(
    `本地 ${summary.value.counts.total} 个技能，${summary.value.counts.ready} 个可直接使用，${summary.value.counts.needsSetup} 个需要补齐。`,
    `${summary.value.counts.total} local skills, ${summary.value.counts.ready} ready, ${summary.value.counts.needsSetup} need setup.`,
  );
});
