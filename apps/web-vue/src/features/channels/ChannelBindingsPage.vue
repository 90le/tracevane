<template>
  <section v-if="channel" class="channels-stage-view">
    ...
    <article v-if="editing" class="panel-card channels-form-panel">
      <div class="channels-stage-task-head">
        <div>
          <p class="eyebrow">{{ focusedAccount ? `${channel.type} · ${focusedAccount.id}` : channel.type }}</p>
          <h3>{{ editingBindingId ? text('修改绑定', 'Edit binding') : text('新增绑定', 'Create binding') }}</h3>
    ...
    <article class="panel-card binding-table">
      ...
      <div v-if="visibleBindings.length" class="binding-table">
        ...
        <div v-for="binding in visibleBindings" :key="binding.id" class="binding-table-row">
