...
function sortRun(
  left: AgentRuntimeRunSummary,
  right: AgentRuntimeRunSummary,
): number {
...
function peerKindLabel(peerKind?: string | null): string {
  ...
  if (["user", "private", "p2p", "direct", "single"].includes(value))
    return "私聊";
...
function bindingSourceLabel(
  binding: ChannelConnectorAgentSessionDriverBindingStatus | undefined,
): string {
...
function terminalTitle(
  ...
  const raw =
    ...
    session.sessionId;
  if (agentLabel && !raw.toLowerCase().includes(agentLabel))
    return `${agentLabel} · ${raw}`;
...
export function buildAgentRuntimeRunsPayload(input: {
  ...
  const bindingsById = new Map(
    (input.channelSessions?.bindings ?? []).map(
  ...
  for (const session of input.terminalSessions?.sessions ?? []) {
    ...
    const cli = inferredCliFromText(
      `${session.title} ${session.profileId ?? ""} ${session.sourceAction}`,
    );
  ...
  for (const session of input.channelSessions?.activeSessions ?? []) {
    ...
    const titleParts = [sourceLabel, session.agent, activeModel].filter(
      Boolean,
    );
    ...
    runs.push({
      ...
      actionLabel: "去 IM 渠道",
      actionReason:
        "IM 会话的 kill / reset / route override 由 IM Channels 拥有。",
      metadata: {
  ...
  return {
    ...
    totals: {
      ...
      failed: runs.filter(
        (run) => run.status === "failed" || run.status === "lost",
      ).length,
