...
const STUDIO_UPDATE_SITE_BASE = (process.env.OPENCLAW_STUDIO_UPDATE_SITE_BASE || 'https://studio.90le.cn').replace(/\/+$/g, '');
const STUDIO_UPDATE_MANIFEST_PATHS = [
  '/openclaw-studio-latest.json',
  '/studio-version.json',
  '/version.json',
];
const STUDIO_UPGRADE_STATE_RELATIVE_PATH = path.join('studio', 'upgrade-status.json');
...
async function fetchJsonWithTimeout(target: string, timeoutMs: number): Promise<Record<string, unknown> | null> {
  try {
...
async function fetchTextWithTimeout(target: string, timeoutMs: number): Promise<string> {
  try {
    const response = await fetch(target, {
      method: 'GET',
      headers: {
...
function buildDefaultStudioUpgradeStatus(config: StudioServerConfig): SystemStudioUpgradeStatusPayload {
  return {
...
function readStudioUpgradeStatus(config: StudioServerConfig): SystemStudioUpgradeStatusPayload {
  ...
  try {
    ...
    return {
      ...
      status:
        parsed.status === 'running' || parsed.status === 'succeeded' || parsed.status === 'failed'
          ? parsed.status
          : 'idle',
      running: parsed.running === true,
      pid: Number.isFinite(Number(parsed.pid)) ? Number(parsed.pid) : null,
      mode: parsed.mode === 'gateway' || parsed.mode === 'standalone' ? parsed.mode : null,
      ...
      logFile: normalizeString(parsed.logFile) || resolveStudioUpgradeLogPath(config),
...
function writeStudioUpgradeStatus(config: StudioServerConfig, payload: SystemStudioUpgradeStatusPayload): void {
...
function resolveConfiguredStudioMode(config: StudioServerConfig): {
  ...
  const studioConfig = entry && typeof entry === 'object' && !Array.isArray(entry)
    ? ((entry as Record<string, unknown>).config as Record<string, unknown> | undefined)
    : undefined;
  const transport = studioConfig?.transport && typeof studioConfig.transport === 'object'
    ? studioConfig.transport as Record<string, unknown>
    : {};
  const standalone = transport.standalone && typeof transport.standalone === 'object'
    ? transport.standalone as Record<string, unknown>
    : {};
  const gateway = transport.gateway && typeof transport.gateway === 'object'
  ...
  const mode: 'standalone' | 'gateway' = gatewayEnabled && !standaloneEnabled ? 'gateway' : 'standalone';
  const apiPort = Number(studioConfig?.apiPort || standalone.port || config.port);
  const basePath = normalizeString(gateway.basePath, config.transport.gateway.basePath || '/studio') || '/studio';
  return {
    mode,
    apiPort: Number.isFinite(apiPort) && apiPort > 0 ? Math.floor(apiPort) : 3760,
...
function readServiceSnapshot(): SystemServiceSnapshot {
  try {
    ...
    return {
      ...
      execMainPid: Number.isFinite(Number(map.ExecMainPID)) ? Number(map.ExecMainPID) : null,
...
function requireCommandSync(command: string, args: string[]): { stdout: string; stderr: string } {
  ...
  if (result.status && result.status !== 0 && !result.stdout && !result.stderr) {
...
function buildGatewaySnapshot(gatewayStatus: SystemCommandSnapshot): SystemGatewaySnapshot {
  ...
  return {
    ...
    portHints: Array.isArray(parsed.port?.hints) ? parsed.port.hints.map(String) : [],
  };
}

function buildStatusSummary(statusCommand: SystemCommandSnapshot): SystemStatusSummary {
  ...
  return {
    ...
    sessionCount: Number(parsed.agents?.totalSessions || parsed.sessions?.count || 0),
    ...
    updateLatestVersion: normalizeString(parsed.update?.registry?.latestVersion),
...
function parseStudioVersionFromIndexHtml(html: string): {
  ...
  const versionMatch = html.match(/const\s+STUDIO_VERSION\s*=\s*["']([^"']+)["']/i);
  const minOpenClawMatch = html.match(/const\s+OPENCLAW_MIN_VERSION\s*=\s*["']([^"']+)["']/i);
  return {
...
function extractReleaseFromManifest(
  ...
  const latestVersion = normalizeVersionCandidate(
    ...
    || payload.version
    || (payload.studio && typeof payload.studio === 'object' && (payload.studio as Record<string, unknown>).version)
    || (payload.release && typeof payload.release === 'object' && (payload.release as Record<string, unknown>).version),
  );
  const packageUrlRaw =
  ...
  const packageUrl = packageUrlRaw
    ? (/^https?:\/\//i.test(packageUrlRaw) ? packageUrlRaw : `${siteBase}/${packageUrlRaw.replace(/^\/+/, '')}`)
    : (latestVersion ? resolvePackageUrl(siteBase, latestVersion) : null);
  const minOpenClawVersion = normalizeVersionCandidate(
    payload.minOpenClawVersion
    || payload.minHostVersion
    || (payload.openclaw && typeof payload.openclaw === 'object' && (payload.openclaw as Record<string, unknown>).minHostVersion),
  );
  return {
    latestVersion,
    packageUrl,
...
async function queryStudioRelease(currentVersion: string): Promise<SystemStudioReleasePayload> {
  const checkedAt = new Date().toISOString();
  const notes: string[] = [];

  for (const manifestPath of STUDIO_UPDATE_MANIFEST_PATHS) {
    const target = `${STUDIO_UPDATE_SITE_BASE}${manifestPath}`;
    const payload = await fetchJsonWithTimeout(target, STUDIO_UPDATE_TIMEOUT_MS);
    if (!payload) {
      continue;
    }
    const extracted = extractReleaseFromManifest(payload, STUDIO_UPDATE_SITE_BASE);
    ...
    return {
      ...
      updateAvailable: compareVersionSegments(extracted.latestVersion, currentVersion) > 0,
      source: target,
  ...
  const indexHtml = await fetchTextWithTimeout(`${STUDIO_UPDATE_SITE_BASE}/`, STUDIO_UPDATE_TIMEOUT_MS);
  if (indexHtml) {
    ...
    if (parsed.latestVersion) {
      return {
        ...
        updateAvailable: compareVersionSegments(parsed.latestVersion, currentVersion) > 0,
        source: `${STUDIO_UPDATE_SITE_BASE}/`,
        packageUrl: resolvePackageUrl(STUDIO_UPDATE_SITE_BASE, parsed.latestVersion),
...
export interface SystemService {
  ...
  toggleDreaming(payload: DreamingToggleRequest): Promise<DreamingToggleResponse>;
  ...
  patchDeviceTrustSettings(payload: SystemDeviceTrustSettingsPatchRequest): Promise<SystemDeviceTrustSettingsPatchResponse>;
}
...
export function createSystemService(
  ...
  async function runCachedCommand(cacheKey: string, command: string, args: string[], timeout = 10_000): Promise<SystemCommandSnapshot> {
    const now = Date.now();
  ...
  function refreshStudioUpgradeStatus(): SystemStudioUpgradeStatusPayload {
    ...
    const next: SystemStudioUpgradeStatusPayload = {
      ...
      lastError: succeeded
        ? ''
        : normalizeString(current.lastError) || normalizeString(logTail.split(/\r?\n/).slice(-8).join('\n')),
    };
  ...
  async function runStudioUpgrade(payload: SystemStudioUpgradeRequest): Promise<SystemStudioUpgradeResponse> {
    ...
    const installerPath = path.join(config.projectRoot, 'install-openclaw-studio.sh');
    ...
    const mode: 'standalone' | 'gateway' = payload.mode === 'gateway' || payload.mode === 'standalone'
      ? payload.mode
      : configured.mode;
    const siteBaseRaw = normalizeString(payload.siteBase) || STUDIO_UPDATE_SITE_BASE;
    const siteBase = siteBaseRaw.replace(/\/+$/g, '');
    const basePathRaw = normalizeString(payload.basePath) || configured.basePath;
    const basePath = basePathRaw.startsWith('/') ? basePathRaw : `/${basePathRaw}`;
    const apiPortRaw = Number(payload.apiPort);
    const apiPort = Number.isFinite(apiPortRaw) && apiPortRaw > 0 ? Math.floor(apiPortRaw) : configured.apiPort;
    ...
    try {
      ...
      const failed: SystemStudioUpgradeStatusPayload = {
        ...
        finishedAt: new Date().toISOString(),
        logFile: logPath,
        lastError: error instanceof Error ? error.message : 'Failed to spawn installer',
      };
      writeStudioUpgradeStatus(config, failed);
      return {
        ok: false,
  ...
  return {
    ...
    async toggleDreaming(payload: DreamingToggleRequest): Promise<DreamingToggleResponse> {
    ...
    async getDiagnostics(): Promise<SystemDiagnosticsPayload> {
      ...
      const bootstrap = getSystemBootstrapSnapshot(config, bootstrapAutoApplied);
      const [gatewayStatus, status, doctor] = await Promise.all([
        runCachedCommand('gateway-status', 'openclaw', ['gateway', 'status', '--json'], 12_000),
        runCachedCommand('status-all', 'openclaw', ['status', '--json'], 12_000),
        runCachedCommand('doctor', 'openclaw', ['doctor'], 18_000),
      ]);
      const channels = Object.keys(openclawConfig.channels || {}).length;
      const bindings = Array.isArray(openclawConfig.bindings) ? openclawConfig.bindings.length : 0;
      const agents = Array.isArray(openclawConfig.agents?.list) ? openclawConfig.agents.list.length : 0;
      const cronJobs = readJsonFile<{ jobs?: Array<unknown> }>(path.join(config.openclawRoot, 'cron', 'jobs.json'), { jobs: [] });
    ...
    async approveDeviceTrust(payload: SystemDeviceTrustApproveRequest): Promise<SystemDeviceTrustApproveResponse> {
      return approveDeviceTrustRequest(config, payload);
    },

    async repairDeviceTrustHelper(): Promise<SystemDeviceTrustRepairResponse> {
      return repairStudioHelperDeviceTrust(config);
    },

    async patchDeviceTrustSettings(payload: SystemDeviceTrustSettingsPatchRequest): Promise<SystemDeviceTrustSettingsPatchResponse> {
