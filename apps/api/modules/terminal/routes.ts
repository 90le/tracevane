...
export function registerTerminalRoutes(router: StudioRouter, ctx: StudioApiContext): void {
  ...
  router.post('/api/terminal/install/stream', async (req, res, routeCtx) => {
    ...
    try {
      await routeCtx.services.terminal.streamInstallCli(target, async (event) => {
      ...
      if (!res.writableEnded) {
        res.write(`${JSON.stringify({
          type: 'error',
          message: error instanceof Error ? error.message : 'terminal_install_failed',
        })}\n`);
      }
    } finally {
      res.end();
