...
import { ensureDir, fileExists, readOpenClawConfig, writeJsonFile } from '../../core/state.js';
...
const SKILLHUB_INSTALL_DOC_URL = 'https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/install/skillhub.md';
const SKILLHUB_INSTALL_COMMAND = 'curl -fsSL https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/install/install.sh | bash';
const SKILLHUB_CLI_ONLY_COMMAND = 'curl -fsSL https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/install/install.sh | bash -s -- --cli-only';

const DEFAULT_SKILLS_SOURCES: Record<SkillsMarketplaceSourceId, Omit<SkillsMarketplaceSource, 'cliInstalled'>> = {
  'skillhub-tencent': {
    id: 'skillhub-tencent',
    name: 'SkillHub Tencent',
    region: 'domestic',
    preferred: true,
    description: '国内镜像与加速源，适合作为默认市场入口，支持镜像搜索与直连下载。',
...
function readSummaryFileCache(config: StudioServerConfig): SkillsSnapshotFileCache | null {
  const filePath = getSkillsSnapshotCacheFile(config);
  if (!fileExists(filePath)) return null;
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf-8')) as SkillsSnapshotFileCache;
  } catch {
    return null;
  }
}

function writeSummaryFileCache(config: StudioServerConfig, payload: SkillsSummaryPayload): void {
  const filePath = getSkillsSnapshotCacheFile(config);
  ensureDir(path.dirname(filePath));
  fs.writeFileSync(filePath, JSON.stringify({ ts: Date.now(), payload }, null, 2), 'utf-8');
}
...
function normalizeMissing(value: Partial<SkillMissingRequirements> | undefined): SkillMissingRequirements {
  const empty = createEmptyMissing();
  if (!value) return empty;

  return {
    bins: Array.isArray(value.bins) ? value.bins.filter((item): item is string => typeof item === 'string') : empty.bins,
    anyBins: Array.isArray(value.anyBins) ? value.anyBins.filter((item): item is string => typeof item === 'string') : empty.anyBins,
    env: Array.isArray(value.env) ? value.env.filter((item): item is string => typeof item === 'string') : empty.env,
    config: Array.isArray(value.config) ? value.config.filter((item): item is string => typeof item === 'string') : empty.config,
    os: Array.isArray(value.os) ? value.os.filter((item): item is string => typeof item === 'string') : empty.os,
  };
}
...
function detectApiKeyMode(value: unknown): SkillApiKeyMode {
  ...
  if (isPlainObject(value) && Object.keys(value).length > 0) return 'secret-ref';
  return 'none';
}
...
function deriveSourceCategory(source: string | undefined, slug: string, workspacePath: string | null, managedPath: string | null): SkillSummary['sourceCategory'] {
...
function parseSkillsJson(stdout: string): OpenClawSkillsSnapshot {
  ...
  for (let index = start; index < trimmed.length; index += 1) {
    ...
    if (char === '{') depth += 1;
      ...
      depth -= 1;
      if (depth === 0) {
        return JSON.parse(trimmed.slice(start, index + 1)) as OpenClawSkillsSnapshot;
      }
    }
  }
...
async function runJsonCommand(file: string, args: string[], cwd: string): Promise<{ stdout: string; stderr: string }> {
...
function readInstallMetadataFromDir(dirPath: string | null): SkillInstallMetadata | null {
  ...
  try {
    ...
    return {
      ...
      publishedAt: typeof raw.publishedAt === 'number' ? new Date(raw.publishedAt).toISOString() : null,
...
function getSkillPaths(workspaceDir: string, managedSkillsDir: string, slug: string): SkillSummary['paths'] {
  ...
  return {
    ...
    activePath: workspaceExists ? workspacePath : managedExists ? managedPath : null,
  };
}
...
function buildSummaryRecord(
  ...
  const sourceCategory = deriveSourceCategory(skill.source, slug, paths.workspacePath, paths.managedPath);
  ...
  return {
    ...
    paths,
    installMetadata: readInstallMetadataFromDir(paths.workspacePath || paths.managedPath),
  };
}

function buildConfigOnlySummary(slug: string, entry: SkillEntryRecord | undefined, workspaceDir: string, managedSkillsDir: string): SkillSummary {
  const paths = getSkillPaths(workspaceDir, managedSkillsDir, slug);
  const apiKeyMode = detectApiKeyMode(entry?.apiKey);
  const env = normalizeStringRecord(entry?.env);
  const config = normalizeConfigRecord(entry?.config);
  const disabled = entry?.enabled === false;
  const sourceCategory = deriveSourceCategory('config-only', 'config-only', paths.workspacePath, paths.managedPath);
...
function buildConfigState(entry: SkillEntryRecord | undefined): SkillConfigState {
  const apiKeyMode = detectApiKeyMode(entry?.apiKey);
  const apiKeyMasked = apiKeyMode === 'plaintext' && typeof entry?.apiKey === 'string'
...
function buildSkillsCounts(skills: SkillSummary[]): SkillsSummaryPayload['counts'] {
  return {
    total: skills.length,
    ...
    workspace: skills.filter((skill) => skill.sourceCategory === 'workspace').length,
    managed: skills.filter((skill) => skill.sourceCategory === 'managed').length,
    bundled: skills.filter((skill) => skill.sourceCategory === 'bundled').length,
    configured: skills.filter((skill) => skill.hasApiKey || skill.envKeys.length > 0 || skill.configKeys.length > 0).length,
    marketplaceInstalled: skills.filter((skill) => skill.installMetadata?.slug).length,
  };
}
...
function normalizeAgentRuntimeForConfig(value: unknown): Record<string, unknown> | null {
  if (!isPlainObject(value)) return null;
...
function sanitizeAgentRuntimesInConfig(openclawConfig: Record<string, unknown>): boolean {
  if (!isPlainObject(openclawConfig.agents) || !Array.isArray(openclawConfig.agents.list)) return false;
...
function createMarketplaceSourcePayload(sourceId: SkillsMarketplaceSourceId, status: { clawhubInstalled: boolean; skillhubInstalled: boolean }): SkillsMarketplaceSource {
  const definition = DEFAULT_SKILLS_SOURCES[sourceId];
  const cliInstalled = definition.cliName === 'skillhub'
...
function normalizeTencentMarketItem(item: Record<string, unknown>, installedSlugs: Set<string>): SkillsMarketplaceItem {
  ...
  return {
    ...
    summary: typeof item.description_zh === 'string' && item.description_zh.trim()
      ? item.description_zh
      : typeof item.description === 'string'
        ? item.description
        : '',
    summaryZh: typeof item.description_zh === 'string' ? item.description_zh : null,
    ...
    updatedAt: typeof item.updated_at === 'number' ? new Date(item.updated_at).toISOString() : null,
    installed: installedSlugs.has(slug),
  };
}

function normalizeClawhubMarketItem(item: Record<string, unknown>, installedSlugs: Set<string>): SkillsMarketplaceItem {
  const slug = typeof item.slug === 'string' ? item.slug : '';
  const latestVersion = isPlainObject(item.latestVersion) ? item.latestVersion : null;
  const stats = isPlainObject(item.stats) ? item.stats : null;
  const ownerName = typeof item.ownerName === 'string'
    ? item.ownerName
  ...
  const name = typeof item.displayName === 'string'
    ...
    : typeof item.name === 'string'
      ? item.name
      : slug;
  const detailUrl = typeof item.url === 'string'
    ? item.url
    : ownerName
  ...
  return {
    ...
    summary: typeof item.summary === 'string'
      ...
      : typeof item.description === 'string'
        ...
        : '',
    summaryZh: null,
    version: latestVersion && typeof latestVersion.version === 'string'
      ? latestVersion.version
      : typeof item.version === 'string'
        ? item.version
        : null,
    ownerName,
    downloads: stats && typeof stats.downloads === 'number' ? stats.downloads : null,
    stars: stats && typeof stats.stars === 'number' ? stats.stars : null,
    installs: stats && typeof stats.installsCurrent === 'number'
...
function sanitizeSkillSlug(value: string): string {
  ...
  if (!/^[a-zA-Z0-9@._/-]+$/.test(slug) || slug.includes('..') || slug.startsWith('/')) {
    throw new Error('Invalid skill slug');
  }
  return slug;
}

function resolveMarketplaceDownloadUrl(sourceId: SkillsMarketplaceSourceId, slug: string): string {
...
function pushRiskIndicator(target: SkillsPreflightIndicator[], indicator: SkillsPreflightIndicator): void {
  if (target.some((item) => item.key === indicator.key && item.detail === indicator.detail)) return;
...
function deriveRiskLevel(indicators: SkillsPreflightIndicator[]): SkillsRiskLevel {
  if (indicators.some((item) => item.severity === 'high')) return 'high';
  if (indicators.some((item) => item.severity === 'medium')) return 'medium';
  return 'low';
}

function analyzeExtractedSkillBundle(sourceId: SkillsMarketplaceSourceId, slug: string, skillRoot: string): SkillsPreflightResult {
  const files = collectFiles(skillRoot);
  const skillMdPath = path.join(skillRoot, 'SKILL.md');
  const textFiles = files
    .map((filePath) => ({ filePath, content: safeReadText(filePath) }))
    .filter((item): item is { filePath: string; content: string } => typeof item.content === 'string');
  ...
  const rules: Array<{ key: string; severity: SkillsRiskLevel; label: string; regex: RegExp; detail: string }> = [
    {
      key: 'pipe-shell',
      severity: 'high',
  ...
  for (const { filePath, content } of textFiles) {
    const relativePath = path.relative(skillRoot, filePath) || path.basename(filePath);
    ...
    for (const envName of extractQuotedList(envBlock?.[1])) suggestedEnv.add(envName);

    for (const match of content.matchAll(/\b(?:brew install|npm install -g|pnpm add -g|yarn global add|go install|pip install|curl -fsSL [^\n]+)\b/gi)) {
...
`, zipPath, outputDir], {
  ...
  throw new Error('Neither unzip nor python3 is available to extract marketplace downloads');
}
...
function locateSkillRoot(rootDir: string): string {
  ...
  const entries = fs.readdirSync(rootDir, { withFileTypes: true }).filter((entry) => !entry.name.startsWith('.'));
...
async function downloadAndExtractSkillBundle(downloadUrl: string): Promise<ExtractedSkillBundle> {
  const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'openclaw-studio-skill-'));
...
function cleanupExtractedSkillBundle(bundle: ExtractedSkillBundle | null): void {
  if (!bundle) return;
...
export interface SkillsService {
  ...
  saveSkillConfig(slug: string, payload: SkillConfigUpdatePayload): Promise<SkillConfigSaveResponse>;
  getSkillSecret(slug: string): Promise<SkillSecretPayload>;
  getMarketplaceSources(): Promise<SkillsMarketplaceSourcesPayload>;
  getMarketplace(options: MarketplaceQueryOptions): Promise<SkillsMarketplacePayload>;
  preflightMarketplaceSkill(payload: SkillsPreflightPayload): Promise<SkillsPreflightResult>;
  installMarketplaceSkill(payload: SkillsMarketplaceInstallPayload): Promise<SkillsMarketplaceInstallResponse>;
  updateInstalledSkill(payload: SkillsMaintenancePayload): Promise<SkillsMaintenanceResponse>;
  uninstallSkill(payload: SkillsMaintenancePayload): Promise<SkillsMaintenanceResponse>;
}
...
export function createSkillsService(config: StudioServerConfig): SkillsService {
  ...
  const marketplaceCache = new Map<string, CacheEntry<SkillsMarketplacePayload>>();
  let toolStateCache: CacheEntry<{ clawhubInstalled: boolean; skillhubInstalled: boolean }> | null = null;

  async function getToolState(): Promise<{ clawhubInstalled: boolean; skillhubInstalled: boolean }> {
  ...
  async function loadSummary(refresh = false): Promise<SkillsSummaryPayload> {
    ...
    if (!refresh) {
      ...
      if (fileCache?.payload) {
        ...
        if (ageMs <= SKILLS_SNAPSHOT_STALE_MS) {
          ...
          summaryCache = {
            value: { payload },
            expiresAt: Date.now() + Math.max(5_000, SKILLS_SNAPSHOT_TTL_MS - Math.min(ageMs, SKILLS_SNAPSHOT_TTL_MS)),
          };
          return payload;
        }
      }
    ...
    try {
      const result = await runJsonCommand('openclaw', ['skills', 'list', '--json'], config.openclawRoot);
      const snapshot = parseSkillsJson(result.stdout || result.stderr);
      const workspaceDir = snapshot.workspaceDir || path.join(config.openclawRoot, DEFAULT_WORKSPACE_DIRNAME);
      const managedSkillsDir = snapshot.managedSkillsDir || path.join(config.openclawRoot, 'skills');
      ...
      for (const item of snapshot.skills || []) {
        summaries.set(item.name, buildSummaryRecord(item, entries[item.name], workspaceDir, managedSkillsDir));
      }

      for (const slug of Object.keys(entries)) {
        if (!summaries.has(slug)) {
          summaries.set(slug, buildConfigOnlySummary(slug, entries[slug], workspaceDir, managedSkillsDir));
        }
      }

      const skills = Array.from(summaries.values()).sort((left, right) => left.name.localeCompare(right.name));
      ...
      const workspaceDir = path.join(config.openclawRoot, DEFAULT_WORKSPACE_DIRNAME);
      ...
      for (const slug of listSubdirectories(path.join(workspaceDir, 'skills'))) slugs.add(slug);
      ...
      writeSummaryFileCache(config, payload);
      console.warn('[studio][skills] fallback summary due to openclaw CLI failure:', error instanceof Error ? error.message : String(error));
      return payload;
    }
  }

  function persistSkillEntry(slug: string, payload: SkillConfigUpdatePayload): SkillConfigState {
    const openclawConfig = readOpenClawConfig(config);
    if (!isPlainObject(openclawConfig.skills)) openclawConfig.skills = {};
    if (!isPlainObject(openclawConfig.skills.entries)) openclawConfig.skills.entries = {};

    const entries = openclawConfig.skills.entries as Record<string, SkillEntryRecord>;
    ...
    if (entry.enabled === true && !entry.apiKey && !entry.env && !entry.config) {
  ...
  function removeSkillConfigEntry(slug: string): void {
    const openclawConfig = readOpenClawConfig(config);
    if (isPlainObject(openclawConfig.skills?.entries) && slug in openclawConfig.skills.entries) {
  ...
  async function runMarketplaceCliInstall(
    ...
    if (sourceId === 'skillhub-tencent' && await commandExists('skillhub')) {
      ...
      return {
        method: 'skillhub-cli',
        output: `${result.stdout}${result.stderr}`.trim() || `skillhub install ${slug}`,
      };
    }

    if (sourceId === 'clawhub' && await commandExists('clawhub')) {
      const result = await execFileAsync('clawhub', ['install', slug, '--workdir', workspaceDir, '--no-input'], {
        cwd: workspaceDir,
        timeout: 120_000,
        maxBuffer: 8 * 1024 * 1024,
      });
      return {
        method: 'clawhub-cli',
        output: `${result.stdout}${result.stderr}`.trim() || `clawhub install ${slug}`,
      };
    }
  ...
  async function fetchTencentMarketplace(options: MarketplaceQueryOptions, installedSlugs: Set<string>): Promise<SkillsMarketplacePayload> {
    const sort = options.sort || 'featured';
    const page = Math.max(1, options.page || 1);
    const pageSize = Math.min(50, Math.max(1, options.pageSize || 24));
    const query = (options.query || '').trim();
    const source = createMarketplaceSourcePayload('skillhub-tencent', await getToolState());
    const url = new URL(sort === 'featured' && !query ? 'https://lightmake.site/api/skills/top' : 'https://lightmake.site/api/skills');
    ...
    const payload = isPlainObject(data.data) ? data.data as Record<string, unknown> : {};
    const items = Array.isArray(payload.skills)
      ? payload.skills.filter((item): item is Record<string, unknown> => isPlainObject(item)).map((item) => normalizeTencentMarketItem(item, installedSlugs))
      : [];
  ...
  async function fetchClawhubMarketplace(options: MarketplaceQueryOptions, installedSlugs: Set<string>): Promise<SkillsMarketplacePayload> {
    const sort = options.sort || 'downloads';
    const query = (options.query || '').trim();
    const pageSize = Math.min(50, Math.max(1, options.pageSize || 24));
    const source = createMarketplaceSourcePayload('clawhub', await getToolState());
    ...
    const itemsRaw = Array.isArray(data.items)
      ? data.items
      : isPlainObject(data.data) && Array.isArray((data.data as Record<string, unknown>).items)
    ...
    const total = typeof data.total === 'number'
      ? data.total
      : isPlainObject(data.data) && typeof (data.data as Record<string, unknown>).total === 'number'
  ...
  return {
    ...
    async toggleSkill(payload: SkillTogglePayload): Promise<SkillTogglePayload> {
    ...
    async saveSkillConfig(slug: string, payload: SkillConfigUpdatePayload): Promise<SkillConfigSaveResponse> {
    ...
    async getSkillSecret(slug: string): Promise<SkillSecretPayload> {
      ...
      return {
        slug,
        apiKey: apiKeyMode === 'plaintext' && typeof entry?.apiKey === 'string' ? entry.apiKey : null,
        apiKeyMode,
    ...
    async getMarketplace(options: MarketplaceQueryOptions): Promise<SkillsMarketplacePayload> {
      const sourceId = options.sourceId;
      const query = (options.query || '').trim();
      const sort = options.sort || (sourceId === 'skillhub-tencent' ? 'featured' : 'downloads');
      const page = Math.max(1, options.page || 1);
      const pageSize = Math.min(50, Math.max(1, options.pageSize || 24));
      const cacheKey = JSON.stringify({ sourceId, query, sort, page, pageSize });
      ...
      try {
        const payload = sourceId === 'skillhub-tencent'
          ? await fetchTencentMarketplace({ sourceId, query, sort, page, pageSize }, installedSlugs)
          : await fetchClawhubMarketplace({ sourceId, query, sort, page, pageSize }, installedSlugs);
        ...
        const source = createMarketplaceSourcePayload(sourceId, await getToolState());
    ...
    async preflightMarketplaceSkill(payload: SkillsPreflightPayload): Promise<SkillsPreflightResult> {
      ...
      try {
        bundle = await downloadAndExtractSkillBundle(downloadUrl);
        return analyzeExtractedSkillBundle(payload.sourceId, slug, bundle.skillRoot);
      } finally {
        cleanupExtractedSkillBundle(bundle);
      }
    },

    async installMarketplaceSkill(payload: SkillsMarketplaceInstallPayload): Promise<SkillsMarketplaceInstallResponse> {
      const sourceId = payload.sourceId;
      const slug = sanitizeSkillSlug(payload.slug);
      const replaceExisting = payload.replaceExisting === true;
      const summary = await loadSummary(true);
      const workspaceDir = summary.workspaceDir || path.join(config.openclawRoot, DEFAULT_WORKSPACE_DIRNAME);
      ...
      try {
        const cliResult = await runMarketplaceCliInstall(sourceId, slug, workspaceDir);
        method = cliResult.method;
        output = cliResult.output;
      } catch (cliError) {
        const downloadUrl = sourceId === 'skillhub-tencent'
          ? `https://lightmake.site/api/v1/download?slug=${encodeURIComponent(slug)}`
          : `https://clawhub.ai/api/v1/download?slug=${encodeURIComponent(slug)}`;
        ...
        if (sourceId === 'clawhub') {
          note = 'ClawHub 官方源直连安装完成；若后续遇到限流，建议切换到腾讯镜像。';
        } else if (cliError instanceof Error && cliError.message !== 'CLI not available') {
    ...
    async updateInstalledSkill(payload: SkillsMaintenancePayload): Promise<SkillsMaintenanceResponse> {
      const slug = sanitizeSkillSlug(payload.slug);
    ...
    async uninstallSkill(payload: SkillsMaintenancePayload): Promise<SkillsMaintenanceResponse> {
      const slug = sanitizeSkillSlug(payload.slug);
      const skill = await getSkillOrThrow(slug);
      ...
      const removedPaths = [skill.paths.workspacePath, skill.paths.managedPath].filter((item): item is string => Boolean(item));
