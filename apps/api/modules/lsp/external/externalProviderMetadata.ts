...
const PROVIDER_METADATA: Record<string, ProviderMetadataSeed> = {
  ...
  eslint: {
    ...
    license: "MIT",
    optional: false,
    auditSummary: "ESLint provider is accepted as diagnostics-only behind a project config gate; workingDirectory is derived server-side from the nearest root-guarded activation marker, while fix/format/code actions remain disabled/deferred.",
    notes: [
      "Started only through server-side allowlisted process.execPath + require.resolve command.",
      "M11-V exposes ESLint diagnostics/status only for JS/TS documents when the current file has a root-guarded nearest ESLint activation marker.",
      "Frontend cannot provide command, args, env, cwd, ESLint options, runtime, nodePath, execArgv, or workingDirectories.",
