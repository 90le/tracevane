...
import { ensureDir, listDirectories, readJsonFile, readOpenClawConfig, writeJsonFile } from '../../core/state.js';
...
function resolveCronStorePath(config: StudioServerConfig, openclawConfig: Record<string, any>): string {
  ...
  return configured.startsWith('~')
    ? path.join(config.openclawRoot.replace(/\/?\.openclaw$/, ''), configured.slice(2))
    : configured;
}
...
function parseSchedule(rawSchedule: unknown): CronScheduleSummary {
  if (rawSchedule && typeof rawSchedule === 'object') {
    ...
    const kind = normalizeString(schedule.kind, schedule.everyMs ? 'every' : schedule.at ? 'at' : 'cron') as CronScheduleSummary['kind'];
    const summary: CronScheduleSummary = {
      kind,
      label: '',
      expr: normalizeString(schedule.expr),
      everyMs: Number.isFinite(Number(schedule.everyMs)) ? Number(schedule.everyMs) : null,
      at: normalizeDate(schedule.atMs) || normalizeDate(schedule.at),
      timezone: normalizeString(schedule.tz) || null,
      staggerMs: Number.isFinite(Number(schedule.staggerMs)) ? Number(schedule.staggerMs) : null,
    };
...
function parsePayload(rawPayload: unknown): CronPayloadSummary {
  ...
  const kind = normalizeString(payload.kind, payload.systemEvent ? 'systemEvent' : 'agentTurn') as CronPayloadSummary['kind'];
  return {
    kind,
    message: normalizeString(payload.message),
    systemEvent: normalizeString(payload.systemEvent),
    thinking: normalizeString(payload.thinking),
    timeoutSeconds: Number.isFinite(Number(payload.timeoutSeconds)) ? Number(payload.timeoutSeconds) : null,
...
function readAgentOptions(openclawConfig: Record<string, any>): CronAgentOption[] {
  ...
  return Array.isArray(openclawConfig.agents?.list)
    ? openclawConfig.agents.list
      ...
      .sort((left: CronAgentOption, right: CronAgentOption) => left.id.localeCompare(right.id))
    : [];
}
...
function buildCronTargets(config: StudioServerConfig, openclawConfig: Record<string, any>): ParsedCronTarget {
  ...
  const channels = openclawConfig.channels && typeof openclawConfig.channels === 'object'
    ? openclawConfig.channels as Record<string, any>
    : {};
  for (const [channelType, channelConfig] of Object.entries(channels)) {
    const accounts = channelConfig && typeof channelConfig === 'object' && channelConfig.accounts && typeof channelConfig.accounts === 'object'
  ...
  for (const agentId of listDirectories(path.join(config.openclawRoot, 'agents'))) {
    const store = readJsonFile<Record<string, any>>(path.join(config.openclawRoot, 'agents', agentId, 'sessions', 'sessions.json'), {});
    for (const [routeKey, session] of Object.entries(store)) {
      ...
      const labelParts = [
        ...
        normalizeDate(record.updatedAtMs || record.updatedAt) ? new Date(Number(record.updatedAtMs || record.updatedAt)).toLocaleString() : '',
      ].filter(Boolean);
      sessionTargets.push({
        key: routeKey,
  ...
  sessionTargets.sort((left, right) => (right.updatedAt || '').localeCompare(left.updatedAt || ''));
  deliveryTargets.sort((left, right) => left.label.localeCompare(right.label));
  return { deliveryTargets, sessionTargets };
}

function resolveFailureAlert(rawFailureAlert: unknown): CronFailureAlertSummary {
  ...
  const failureAlert = rawFailureAlert && typeof rawFailureAlert === 'object' ? rawFailureAlert as Record<string, any> : {};
  return {
    enabled: true,
    mode: normalizeString(failureAlert.mode, 'announce') === 'webhook' ? 'webhook' : 'announce',
    ...
    after: Number.isFinite(Number(failureAlert.after)) ? Number(failureAlert.after) : null,
    cooldownMs: Number.isFinite(Number(failureAlert.cooldownMs)) ? Number(failureAlert.cooldownMs) : null,
  };
}

function resolveFailureDestination(rawFailureDestination: unknown): CronFailureDestinationSummary {
  ...
  return {
    enabled: true,
    mode: normalizeString(failureDestination.mode, 'announce') === 'webhook' ? 'webhook' : 'announce',
...
function resolveSessionTarget(rawSessionTarget: unknown, sessionTargets: CronSessionOption[]): {
  ...
  const ref = value.startsWith('session:') ? value.slice('session:'.length) : value;
...
function resolveDelivery(rawDelivery: unknown, deliveryTargets: CronTargetOption[]): CronDeliverySummary {
  const delivery = rawDelivery && typeof rawDelivery === 'object' ? rawDelivery as Record<string, any> : {};
  const normalizedMode = normalizeString(delivery.mode, 'silent');
  const mode = normalizedMode === 'announce' || normalizedMode === 'webhook' ? normalizedMode : 'silent';
  if (mode === 'silent') {
    return {
      mode,
      targetType: '',
  ...
  const targetRef = normalizeString(delivery.targetRef || delivery.bindingRef || delivery.sessionKey || delivery.to);
  const targetType = mode === 'webhook'
    ? 'webhook'
    : normalizeString(delivery.targetType, targetRef.startsWith('session:') ? 'directSession' : 'channelBinding') as CronDeliverySummary['targetType'];
  const target = deliveryTargets.find((item) => item.ref === targetRef);
  return {
    mode,
    targetType,
    targetRef,
    label: mode === 'webhook' ? targetRef || 'webhook' : target?.label || targetRef || 'announce',
    bestEffort: normalizeBoolean(delivery.bestEffort, true),
    failureDestination: resolveFailureDestination(delivery.failureDestination),
  };
}
...
function resolveState(rawState: unknown): CronStateSummary {
  const state = rawState && typeof rawState === 'object' ? rawState as Record<string, any> : {};
  return {
    lastStatus: normalizeString(state.lastStatus || state.lastRunStatus) || null,
...
function mapJob(
  ...
  const sessionTarget = resolveSessionTarget(rawJob.sessionTarget, sessionTargets);
  ...
  return {
    ...
    deleteAfterRun: normalizeBoolean(rawJob.deleteAfterRun, schedule.kind === 'at'),
...
function readRunHistory(config: StudioServerConfig, jobId: string, limit = 20): CronRunSummary[] {
  ...
  try {
    ...
    return lines
      ...
      .map((entry) => ({
        ...
        summaryPreview: normalizeString(entry.summary || entry.stdout).slice(0, 220),
        ...
        totalTokens: Number.isFinite(Number(entry.usage?.total_tokens || entry.usage?.total)) ? Number(entry.usage?.total_tokens || entry.usage?.total) : null,
      }))
      .reverse();
  } catch {
    return [];
...
function readLiveCronStatus(): CronSchedulerSummary['live'] {
  try {
    ...
    const message = error instanceof Error ? error.message : 'cron status unavailable';
    return {
      source: 'derived',
      jobs: null,
      nextWakeAt: null,
...
function buildSchedulerSummary(config: StudioServerConfig, openclawConfig: Record<string, any>, storePath: string): CronSchedulerSummary {
  const runLog = openclawConfig.cron?.runLog && typeof openclawConfig.cron.runLog === 'object'
    ? openclawConfig.cron.runLog as Record<string, any>
    : {};
  const cronConfig = openclawConfig.cron && typeof openclawConfig.cron === 'object'
  ...
  return {
    ...
    storePath,
    maxConcurrentRuns: Number.isFinite(Number(cronConfig.maxConcurrentRuns)) ? Number(cronConfig.maxConcurrentRuns) : null,
    runLogDir: resolveRunLogDir(config),
    sessionRetention: normalizeString(cronConfig.sessionRetention, '24h'),
    runLogMaxBytes: Number.isFinite(Number(runLog.maxBytes)) ? Number(runLog.maxBytes) : null,
    runLogKeepLines: Number.isFinite(Number(runLog.keepLines)) ? Number(runLog.keepLines) : null,
    failureWebhook: normalizeString(cronConfig.webhook),
    failureWebhookTokenConfigured: Boolean(normalizeString(cronConfig.webhookToken)),
    defaultFailureAlert: resolveFailureAlert(cronConfig.failureAlert),
    defaultFailureDestination: resolveFailureDestination(cronConfig.failureDestination),
    live,
  };
}
...
function buildSummary(config: StudioServerConfig): CronSummaryPayload {
  ...
  const jobs = Array.isArray(store.jobs)
    ? store.jobs
      .map((job) => mapJob(job, targets.deliveryTargets, targets.sessionTargets))
...
function getRawJob(config: StudioServerConfig, jobId: string): { openclawConfig: Record<string, any>; storePath: string; store: CronStoreShape; job: Record<string, any>; index: number } {
  const openclawConfig = readOpenClawConfig(config);
  const storePath = resolveCronStorePath(config, openclawConfig);
  const store = readCronStore(storePath);
  ...
  if (index === -1) {
    throw new CronServiceError(404, 'cron_job_not_found', `Cron job '${jobId}' not found`);
  }
  return { openclawConfig, storePath, store, job: jobs[index], index };
}

function buildDetail(config: StudioServerConfig, jobId: string): CronDetailPayload {
  const openclawConfig = readOpenClawConfig(config);
  const storePath = resolveCronStorePath(config, openclawConfig);
  const scheduler = buildSchedulerSummary(config, openclawConfig, storePath);
  const targets = buildCronTargets(config, openclawConfig);
...
function parseAtInput(value: string): string {
  ...
  if (!raw) throw new CronServiceError(400, 'invalid_schedule', 'One-shot schedule requires a date/time');
  const timestamp = Date.parse(raw);
  if (Number.isNaN(timestamp)) throw new CronServiceError(400, 'invalid_schedule', 'Invalid one-shot date/time');
  return new Date(timestamp).toISOString();
}

function normalizeInput(input: CronJobInput, config: StudioServerConfig, openclawConfig: Record<string, any>, existing?: Record<string, any>): Record<string, any> {
  const name = normalizeString(input.name);
  if (!name) throw new CronServiceError(400, 'invalid_name', 'Cron job name is required');

  const agentOptions = readAgentOptions(openclawConfig);
  const agentId = normalizeString(input.agentId, existing ? normalizeString(existing.agentId, 'main') : 'main');
  if (!agentOptions.some((agent) => agent.id === agentId)) {
    throw new CronServiceError(400, 'invalid_agent', `Agent '${agentId}' not found`);
  }

  const scheduleKind = normalizeString(input.scheduleKind, 'cron') as CronJobInput['scheduleKind'];
  const schedule: Record<string, any> = { kind: scheduleKind };

  if (scheduleKind === 'cron') {
    const expr = normalizeString(input.cronExpr);
    if (!expr) throw new CronServiceError(400, 'invalid_schedule', 'Cron expression is required');
    schedule.expr = expr;
    if (normalizeString(input.timezone)) schedule.tz = normalizeString(input.timezone);
  } else if (scheduleKind === 'every') {
    const everyMs = parseDurationMs(normalizeString(input.every));
    if (!everyMs) throw new CronServiceError(400, 'invalid_schedule', 'Recurring duration is required, e.g. 10m or 1h');
  ...
  const payloadKind = normalizeString(input.payloadKind, 'agentTurn') as CronPayloadSummary['kind'];
  const payload: Record<string, any> = { kind: payloadKind };
  if (payloadKind === 'systemEvent') {
    const systemEvent = normalizeString(input.systemEvent);
    if (!systemEvent) throw new CronServiceError(400, 'invalid_payload', 'System event text is required');
    payload.systemEvent = systemEvent;
  } else {
    const message = normalizeString(input.message);
    if (!message) throw new CronServiceError(400, 'invalid_payload', 'Agent message is required');
    payload.message = message;
  }
  if (normalizeString(input.thinking)) payload.thinking = normalizeString(input.thinking);
  if (Number.isFinite(Number(input.timeoutSeconds))) payload.timeoutSeconds = Number(input.timeoutSeconds);
  if (normalizeString(input.model)) payload.model = normalizeString(input.model);
  if (input.lightContext === true) payload.lightContext = true;
  if (input.expectFinal === true) payload.expectFinal = true;

  const sessionTargetMode = normalizeString(input.sessionTargetMode, 'isolated') as CronJobInput['sessionTargetMode'];
  ...
  if (sessionTargetMode === 'main') {
    ...
    if (!ref) throw new CronServiceError(400, 'invalid_session_target', 'Existing session target requires a session key');
    sessionTarget = `session:${ref}`;
  }

  const deliveryMode = normalizeString(input.deliveryMode, 'silent') as CronJobInput['deliveryMode'];
  const delivery: Record<string, any> = { mode: deliveryMode === 'announce' || deliveryMode === 'webhook' ? deliveryMode : 'silent' };
  if (delivery.mode === 'announce') {
    const targetType = normalizeString(input.deliveryTargetType, 'channelBinding');
    const targetRef = normalizeString(input.deliveryTargetRef);
    if (!targetRef) throw new CronServiceError(400, 'invalid_delivery_target', 'Delivery target is required for announce mode');
    ...
    if (!/^https?:\/\//i.test(targetRef)) {
      throw new CronServiceError(400, 'invalid_delivery_target', 'Webhook delivery requires a valid http(s) URL');
    }
  ...
  if (input.failureDestinationEnabled === true) {
    ...
    if (mode !== 'announce' && mode !== 'webhook') {
      throw new CronServiceError(400, 'invalid_failure_destination', 'Failure destination mode must be announce or webhook');
    }
    failureDestination = {
      mode,
    };
    if (normalizeString(input.failureDestinationChannel)) failureDestination.channel = normalizeString(input.failureDestinationChannel);
    if (normalizeString(input.failureDestinationAccountId)) failureDestination.accountId = normalizeString(input.failureDestinationAccountId);
    if (normalizeString(input.failureDestinationTo)) failureDestination.to = normalizeString(input.failureDestinationTo);
    if (mode === 'webhook' && !/^https?:\/\//i.test(normalizeString(input.failureDestinationTo))) {
      throw new CronServiceError(400, 'invalid_failure_destination', 'Webhook failure destination requires a valid http(s) URL');
    }
  }
  ...
  if (input.failureAlertEnabled === true) {
    ...
    if (mode !== 'announce' && mode !== 'webhook') {
      throw new CronServiceError(400, 'invalid_failure_alert', 'Failure alert mode must be announce or webhook');
    }
    failureAlert.mode = mode;
    if (normalizeString(input.failureAlertChannel)) failureAlert.channel = normalizeString(input.failureAlertChannel);
    if (normalizeString(input.failureAlertAccountId)) failureAlert.accountId = normalizeString(input.failureAlertAccountId);
    if (normalizeString(input.failureAlertTo)) failureAlert.to = normalizeString(input.failureAlertTo);
    if (Number.isFinite(Number(input.failureAlertAfter)) && Number(input.failureAlertAfter) > 0) {
      failureAlert.after = Number(input.failureAlertAfter);
    }
    const cooldownMs = parseDurationMs(normalizeString(input.failureAlertCooldown));
    if (cooldownMs) failureAlert.cooldownMs = cooldownMs;
    if (mode === 'webhook' && !/^https?:\/\//i.test(normalizeString(input.failureAlertTo))) {
      throw new CronServiceError(400, 'invalid_failure_alert', 'Webhook failure alerts require a valid http(s) URL');
    }
  ...
  return {
    ...
    state: existing?.state && typeof existing.state === 'object'
      ? existing.state
      : { consecutiveErrors: 0, lastStatus: 'never-run' },
  };
}

async function runCronJobByCli(jobId: string): Promise<{ success: boolean; output: string }> {
  try {
  ...
  return {
    ...
    getDetail(jobId: string): CronDetailPayload | null {
      try {
        ...
        if (error instanceof CronServiceError && error.statusCode === 404) return null;
        throw error;
      }
    },
    ...
    updateJob(jobId: string, input: CronJobInput): CronMutationResponse {
      const { openclawConfig, storePath, store, job, index } = getRawJob(config, jobId);
      const jobs = Array.isArray(store.jobs) ? store.jobs : [];
    ...
    toggleJob(jobId: string, enabled: boolean): CronMutationResponse {
      ...
      return {
        checkedAt: new Date().toISOString(),
        success: true,
        message: enabled ? `Cron job '${jobId}' enabled` : `Cron job '${jobId}' disabled`,
        summary: buildSummary(config),
        detail: buildDetail(config, jobId),
      };
    },
    ...
    async runJob(jobId: string): Promise<CronRunResponse> {
      ...
      if (!result.success) {
        throw new CronServiceError(502, 'cron_run_failed', result.output || `Failed to run cron job '${jobId}'`);
      }
