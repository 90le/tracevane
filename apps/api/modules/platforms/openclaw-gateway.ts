...
import { requestGateway } from "./openclaw-gateway-request.js";
...
function normalizeOpenClawGatewayParams(
  value: unknown,
...
function requireGatewayTargetKey(
  ...
  if (!key) {
    throw new OpenClawGatewayServiceError(
      ...
      buildOpenClawGatewayError(
        "invalid_request",
        `OpenClaw gateway method '${method}' requires a session key`,
      ),
    );
  }
  return key;
...
export async function requestOpenClawGateway(
  ...
  switch (method) {
    case "models.list":
      return await requestGateway<Record<string, unknown>>(
        config,
        "models.list",
        {},
      );
    ...
    default:
      throw new OpenClawGatewayServiceError(
        ...
        buildOpenClawGatewayError(
          "invalid_request",
          `Unsupported OpenClaw gateway method '${method || "<empty>"}'`,
        ),
      );
  }
}
