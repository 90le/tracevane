...
function buildStudioOutboundFilePolicy(): string {
  return [
    ...
    "```",
    "For Feishu document, drive, permission, or wiki work, use the Studio-owned `studio-feishu-actions` block when the active channel skill context lists that capability; Studio executes read-only actions through the current Feishu binding and asks the user before enabled mutations.",
    "Do not claim Feishu document, drive, permission, or wiki mutation succeeded unless Studio returns an actual action result; use files or Feishu Markdown messages as a fallback for unsupported content mutations.",
