...
/**
 * `GitPanel` — the source-control surface shown in the IDE side panel when
 ...
 */
export function GitPanel({ rootId, onOpenDiff }: GitPanelProps) {
  ...
  const handleStage = (file: string) => {
    stage.mutate(
      { rootId, paths: [file] },
      {
        onError: (err) => toast.error("暂存失败", { description: err.message }),
      },
    );
  };
  ...
  const handleUnstage = (file: string) => {
    unstage.mutate(
      { rootId, paths: [file] },
      {
        onError: (err) => toast.error("取消暂存失败", { description: err.message }),
      },
    );
  };
  ...
  const handleCheckout = (target: string) => {
    ...
    checkout.mutate(
      { rootId, target },
      {
        onError: (err) => toast.error("切换分支失败", { description: err.message }),
      },
    );
  };
...
// ---------------------------------------------------------------------------
// changeTone — short label + tone for a change kind (IDE-style M/A/D/U codes)
// ---------------------------------------------------------------------------
