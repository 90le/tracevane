...
interface ReplaceUndoPackage {
  ...
  query: string;
  replacement: string;
  items: Array<{ path: string; previousContent: string; replacedContent: string }>;
}

export function WorkspaceSearchPanel({ rootId, onOpenFile }: WorkspaceSearchPanelProps) {
  ...
  const [replacePreview, setReplacePreview] = React.useState<ReplacePreviewItem[] | null>(null);
  const [replaceSelection, setReplaceSelection] = React.useState<Set<string>>(() => new Set());
  const [undoPackage, setUndoPackage] = React.useState<ReplaceUndoPackage | null>(null);
  ...
  const replaceTargets = results.filter((result) => result.kind === "file" && result.textLike);
  ...
  const prepareReplacePreview = React.useCallback(async () => {
    ...
    try {
      for (const result of replaceTargets) {
        try {
          ...
          previewItems.push({
            ...
            diffLines: createReplaceDiffLines(content, query, replaceWith, replaceOptions),
          });
        } catch {
          failedReads += 1;
        }
      ...
      if (failedReads) toast.error("替换预览部分失败", { description: `${failedReads} 个文件无法读取，已跳过。` });
      if (!previewItems.length) toast.info("没有可替换内容", { description: "搜索结果中未找到精确匹配的可写文本。" });
    } finally {
  ...
  const applyReplacePreview = React.useCallback(async () => {
    ...
    try {
      for (const item of selectedReplaceItems) {
        try {
          await writeFileContent({ rootId, path: item.path, content: item.nextContent });
          undoItems.push({ path: item.path, previousContent: item.content, replacedContent: item.nextContent });
          changed += 1;
        } catch {
          failed += 1;
        }
      ...
      if (failed) toast.error("批量替换部分失败", { description: `已更新 ${changed} 个文件，${failed} 个失败。` });
      else toast.success("批量替换完成", { description: `已更新 ${changed} 个文件。` });
    } finally {
      setReplaceBusy(false);
    }
  ...
  const selectAllReplacePreviewItems = React.useCallback((selected: boolean) => {
    setReplaceSelection(selected ? new Set((replacePreview ?? []).map((item) => item.path)) : new Set());
  }, [replacePreview]);
    ...
    try {
      for (const item of undoPackage.items) {
        try {
          ...
          await writeFileContent({ rootId, path: item.path, content: item.previousContent });
          restored += 1;
        } catch {
          failed += 1;
        }
      ...
      await queryClient.invalidateQueries({ queryKey: filesKeys.all });
      if (failed) toast.error("撤销部分失败", { description: `已恢复 ${restored} 个文件，${failed} 个文件已变化或写入失败。` });
      else toast.success("已撤销上次批量替换", { description: `已恢复 ${restored} 个文件。` });
    ...
    <section className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] bg-panel">
      <header className="grid gap-2 border-b border-line px-3 py-2">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
          ...
          <span>结果上限 {searchLimit} 条由后端保护；批量替换前必须预览并可逐文件排除。</span>
          <label className="inline-flex items-center gap-1.5">
            <input
              type="checkbox"
          ...
          {indexStats?.used ? (
            ...
            <span className="rounded-full bg-panel-2 px-2 py-0.5 text-subtle">索引未命中，已扫描补全</span>
          ) : null}
        </div>
        <div className="grid gap-2 rounded border border-line bg-panel-2 p-2">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <span className="text-subtle">当前结果内批量替换；支持大小写、正则、预览确认和撤销。</span>
          </div>
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
            ...
            >
              {replaceBusy ? "分析中..." : `预览替换 ${replaceTargets.length} 个文件`}
            </Button>
          </div>
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
            <span>替换沿用当前搜索模式：{searchCaseSensitive ? "区分大小写" : "不区分大小写"}{searchRegex ? " · 正则替换" : ""}；只写入本次预览勾选的文本文件。</span>
          </div>
        </div>
        {undoPackage ? (
          <ReplaceUndoStrip
      ...
      <div className="min-h-0 overflow-auto p-2">
        {!query ? (
          <EmptyState title="输入关键词搜索" description="搜索当前 workspace root 的文件名和内容。" icon={<Search />} />
        ) : search.isLoading ? (
          ...
          <ErrorState title="搜索失败" description={search.error?.message ?? search.data?.error} />
        ) : results.length === 0 ? (
          <EmptyState title="无匹配结果" description={query} icon={<Search />} />
        ) : (
          <div className="grid gap-1">
            {searchTruncated ? (
              <div className="rounded border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning">
                搜索已达到 {searchLimit} 条结果上限；请收窄关键词，避免在大型 workspace 中一次性渲染过多结果。
              </div>
            ) : null}
            {results.map((result) => (
              <SearchResultRow
    ...
    <div className="grid gap-2 rounded border border-warning/30 bg-warning/10 p-2 text-xs">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-muted">
          {undoPackage.items.length} 个文件 · {new Date(undoPackage.createdAt).toLocaleTimeString()} ·
          <span className="ml-1 font-mono">“{undoPackage.query}” → “{undoPackage.replacement}”</span>
        </span>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="ghost" size="sm" onClick={onDismiss} disabled={busy}>丢弃撤销包</Button>
      </div>
    </div>
  );
}
...
function SearchResultRow({
  ...
  return (
    <button
      ...
      onClick={() => onOpenFile(result.path, { initialSearch: { query, caseSensitive, regex } })}
      className="grid w-full grid-cols-[18px_minmax(0,1fr)] gap-2 rounded-sm px-2 py-1.5 text-left hover:bg-panel-2 disabled:cursor-not-allowed disabled:opacity-60 focus-visible:shadow-[var(--ring)]"
      title={result.path}
    >
      {result.kind === "directory" ? <Folder className="mt-0.5 size-4 text-primary" /> : <File className="mt-0.5 size-4 text-muted" />}
      <span className="min-w-0">
        <span className="block truncate text-sm text-ink-strong">{result.name}</span>
        <span className="block truncate text-2xs text-subtle">{result.directoryPath || "/"}</span>
        {result.snippet ? (
          <span className="mt-0.5 block truncate text-xs text-muted">
            <HighlightedText text={result.snippet} query={query} caseSensitive={caseSensitive} regex={regex} />
          </span>
        ) : null}
      </span>
    </button>
...
function HighlightedText({
  ...
  return (
    <>
      {text.slice(0, index)}
      <mark className="rounded bg-warning/20 px-0.5 text-ink-strong">{text.slice(index, index + (match?.length ?? query.length))}</mark>
...
function findHighlightMatch(
  ...
  try {
    const match = new RegExp(query, options.caseSensitive ? "" : "i").exec(text);
    return match ? { index: match.index, length: Math.max(1, match[0]?.length ?? 0) } : null;
  } catch {
    return null;
  }
}
...
function ReplacePreviewDialog({
  ...
  const selectedMatches = selectedItems.reduce((sum, item) => sum + item.matches, 0);
  const allSelected = Boolean(items?.length && selectedItems.length === items.length);
  return (
    <Dialog open={items !== null} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="w-[min(760px,94vw)]">
        ...
        <DialogBody className="grid gap-3 text-sm">
          <div className="rounded border border-line bg-panel-2 p-3 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              <div className="font-medium text-ink-strong">将把当前搜索结果中的精确匹配替换后再写回文件。</div>
            ...
            <div className="mt-1 text-muted">
              ...
              <span> · {replaceOptions.caseSensitive ? "区分大小写" : "不区分大小写"}</span>
              {replaceOptions.regex ? <span> · 正则</span> : null}
              <span> · 已选 {selectedItems.length}/{items?.length ?? 0} 个文件 · {selectedMatches}/{totalMatches} 处匹配</span>
            </div>
          </div>
          <div className="max-h-[48vh] overflow-auto rounded border border-line">
            {!items?.length ? (
              <div className="p-4 text-center text-muted">没有可应用的替换。</div>
            ) : (
                ...
                {items.map((item) => (
                  <article key={item.path} className="grid gap-2 p-3 text-xs">
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="min-w-0 flex-1 truncate font-medium text-ink-strong" title={item.path}>{item.path}</span>
                      <span className="rounded-full bg-primary-soft px-2 py-0.5 text-primary">{item.matches} 处</span>
                    </div>
                    <ReplaceDiffPreview lines={item.diffLines} query={query} replaceWith={replaceWith} options={replaceOptions} />
                  </article>
                ))}
              </div>
            )}
        ...
        <DialogFooter>
          ...
          <Button variant="primary" onClick={onApply} disabled={busy || selectedItems.length === 0}>
            {busy ? "写入中..." : "确认全部替换"}
          </Button>
        </DialogFooter>
      </DialogContent>
