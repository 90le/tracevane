    ...
    <div className="grid gap-4">
      <Panel>
        <PanelHead
          ...
          action={<ToneBadge tone={installedAgents >= 3 && gatewayReady ? "ok" : "warn"}>{installedAgents}/3 CLI 就绪</ToneBadge>}
        />
        <div className="grid gap-0.5 p-1">
          {terminalStatus.isLoading || runs.isLoading || gateway.isLoading ? (
            ...
            <ErrorState
              title="CLI 工作台状态不可用"
              description={terminalStatus.error?.message || runs.error?.message || gateway.error?.message}
            />
          ) : (
            <>
              <Row
                icon={<Activity />}
                iconClass={toneIconClass(failed > 0 ? "bad" : running > 0 ? "ok" : "info")}
                title={failed > 0 ? `${failed} 个 Agent Run 需要处理` : `${running} 个 Agent Run 正在运行`}
                subtitle={`全部 ${totals?.total ?? 0} · 终端 ${totals?.terminal ?? 0} · IM ${totals?.imChannel ?? 0} · 对话 ${totals?.chat ?? 0}`}
                trailing={<Button variant="ghost" size="sm" onClick={() => goToView("runs")}>运行台<ArrowRight /></Button>}
              />
              <Row
                icon={<Terminal />}
              ...
              <Row
                ...
                title={gatewayReady ? "模型网关可用于 CLI 路由" : "模型网关需要检查"}
                subtitle={health ? `${health.okProviders} 正常 · ${health.degradedProviders} 降级 · ${health.openCircuits} 熔断` : "Provider 状态未返回"}
                trailing={<Button variant="ghost" size="sm" onClick={() => (window.location.hash = "#/model-gateway")}>网关<ArrowRight /></Button>}
              />
            </>
          )}
        </div>
      ...
      <Panel>
        <PanelHead title="职责边界" sub="减少重复页面：CLI 只做运行管理，配置回所属域。" />
        <div className="grid gap-2 p-4 text-sm text-muted">
          <p><strong className="text-ink-strong">CLI Agents：</strong>启动命令、CLI readiness、Agent Runs、Agent 终端会话 stop/delete、证据跳转。</p>
          <p><strong className="text-ink-strong">Model Gateway：</strong>Provider、模型、协议、上下文、路由、客户端接入和用量口径。</p>
          <p><strong className="text-ink-strong">IM Channels：</strong>IM 账号、平台凭据、绑定路由、队列并发、投递和会话驱动。</p>
        </div>
      </Panel>
    </div>
  );
