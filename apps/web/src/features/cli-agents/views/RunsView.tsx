...
function targetHref(run: AgentRuntimeRunSummary): string {
  ...
  return "#/im-channels?view=sessions";
}
...
function visibleRows(
  rows: AgentRuntimeRunSummary[],
  filter: RunFilter,
  query: string,
): AgentRuntimeRunSummary[] {
  return rows.filter(
    (run) => rowMatchesFilter(run, filter) && rowMatchesSearch(run, query),
  );
}
...
function metadataText(run: AgentRuntimeRunSummary): string {
  ...
  const origin =
    run.originId && !refs.includes(run.originId) ? [run.originId] : [];
  return [...origin, ...refs].slice(0, 2).join(" · ");
}

/**
 * Unified Agent Run view. This is intentionally a projection, not a fourth
 * owning runtime: Workspace terminal sessions and IM agent sessions keep
...
export function RunsView(_props: CliAgentsViewProps) {
  ...
  const [stopTarget, setStopTarget] =
    React.useState<AgentRuntimeRunSummary | null>(null);
    ...
    <div className="grid gap-4">
      <Panel>
        <PanelHead
          ...
          sub="统一展示终端来源与 IM 渠道 Agent 运行状态；只对 Agent 终端会话提供安全 stop/delete。"
          action={
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="p-4">
          {runs.isLoading ? (
            ...
            <EmptyState
              title="暂无 Agent Run"
              description="启动 CLI 终端或触发 IM 任务后会出现在这里。"
            />
          ) : filteredRows.length === 0 ? (
            <EmptyState
              title="当前筛选无结果"
              description="调整搜索关键词、切换筛选或刷新后再看。"
            />
          ) : (
            <div
              className="overflow-hidden rounded-md border border-line bg-panel shadow-sm"
              role="table"
                ...
                {filteredRows.map((run) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`mt-0.5 grid size-8 shrink-0 place-items-center rounded-md ${toneIconClass(tone)}`}
                        >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 flex flex-wrap gap-1.5 text-sm text-muted">
                            <span>
                              {run.sourceLabel || SOURCE_LABEL[run.source]}
                            </span>
      ...
      <Panel>
        <PanelHead
          ...
          action={
            <ToneBadge tone={actionRows.length > 0 ? "warn" : "ok"}>
              {actionRows.length} 项
            </ToneBadge>
          }
        />
          ...
          <div className="divide-y divide-line">
            {actionRows.map((run) => {
              ...
              return (
                ...
                >
                  <span
                    className={`grid size-8 shrink-0 place-items-center rounded-md ${toneIconClass(tone)}`}
                  >
                    {sourceIcon(run.source)}
                  </span>
                  <span className="min-w-0 flex-1 basis-[180px]">
                    ...
                    <span className="block truncate text-xs text-muted">
                      {run.actionLabel} ·{" "}
                      {run.lastErrorSummary || run.actionReason}
                    </span>
                  </span>
      ...
      >
        <DialogContent>
          ...
          <DialogBody>
            将请求后端结束会话{" "}
            <span className="font-mono text-ink">{stopTarget?.originId}</span>。
            这只适用于 CLI Agents 识别出的 Codex / Claude Code / OpenCode
            终端会话。
          </DialogBody>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                stopTarget && endSession.mutate({ sid: stopTarget.originId })
              }
            >
              确认停止
            </Button>
      ...
      >
        <DialogContent>
          ...
          <DialogBody>
            将删除已结束/失败的持久化记录{" "}
            <span className="font-mono text-ink">{deleteTarget?.originId}</span>
            。 活跃会话不会被删除，需先停止。
          </DialogBody>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                deleteTarget && deleteSession.mutate(deleteTarget.originId)
              }
            >
              确认删除
            </Button>
