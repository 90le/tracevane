...
import type { FilesTransferConflictPolicy, FilesTrashItem } from "../../../../../types/files";
...
export function TrashManager({
  ...
  const trash = useFilesTrashQuery(rootId);
  const restoreMutation = useRestoreFilesTrashMutation();
  const purgeMutation = usePurgeFilesTrashMutation();
  const [selectedPaths, setSelectedPaths] = React.useState<Set<string>>(() => new Set());
  const [conflictPolicy, setConflictPolicy] = React.useState<FilesTransferConflictPolicy>("rename");
  const items = trash.data?.items ?? [];
  const selectedItems = React.useMemo(() => items.filter((item) => selectedPaths.has(item.trashPath)), [items, selectedPaths]);

  React.useEffect(() => {
    setSelectedPaths((prev) => new Set([...prev].filter((path) => items.some((item) => item.trashPath === path))));
  }, [items]);
  ...
  const restoreItem = React.useCallback(async (item: FilesTrashItem) => {
    try {
      const result = await restoreMutation.mutateAsync({ rootId, trashPath: item.trashPath, conflictPolicy });
      onRecord?.(createOperationRecord({
        title: "恢复回收站项目",
        status: "success",
        itemCount: 1,
        successCount: 1,
      ...
      if (result.affectedPaths[1]) onRevealPath?.(result.affectedPaths[1]);
      setSelectedPaths((prev) => {
        const next = new Set(prev);
  ...
  }, [conflictPolicy, onRecord, onRevealPath, restoreMutation, rootId]);

  const purgeItems = React.useCallback(async (targetItems: FilesTrashItem[]) => {
    if (!targetItems.length) return;
    try {
      const result = await purgeMutation.mutateAsync({ rootId, trashPaths: targetItems.map((item) => item.trashPath) });
      onRecord?.(createOperationRecord({
        title: "永久清理回收站",
        status: "success",
      ...
      toast.success("已永久删除", { description: `${targetItems.length} 个回收站项目已清理。` });
      setSelectedPaths((prev) => {
        const next = new Set(prev);
      ...
      toast.error("清理失败", { description: error instanceof Error ? error.message : String(error) });
    }
  }, [onRecord, purgeMutation, rootId]);

  return (
    <section className="grid gap-3 rounded-lg border border-line bg-panel p-4 shadow-sm" data-file-manager-trash-manager>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <label className="flex items-center gap-1 rounded border border-line bg-panel-2 px-2 py-1">
            ...
            <select
              ...
              onChange={(event) => setConflictPolicy(event.target.value as FilesTransferConflictPolicy)}
          ...
          <Button variant="outline" size="sm" onClick={() => void trash.refetch()} disabled={trash.isFetching}>刷新</Button>
          <Button variant="danger" size="sm" onClick={() => void purgeItems(selectedItems.length ? selectedItems : items)} disabled={!items.length || purgeMutation.isPending}>
            <Trash2 className="size-4" />
            {selectedItems.length ? `永久删除 ${selectedItems.length} 项` : "清空回收站"}
          </Button>
        </div>
      </header>

      {trash.isLoading ? (
        <div className="rounded border border-line bg-panel-2 px-3 py-6 text-center text-sm text-muted">正在读取回收站…</div>
      ) : trash.error ? (
        <div className="rounded border border-danger/30 bg-danger/5 px-3 py-2 text-sm text-danger">{trash.error.message}</div>
      ) : !items.length ? (
