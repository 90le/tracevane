...
import { BoundaryBadge, DetailRail, EvidenceRow, ReadOnlyStrip, ResponsiveTable, StatusPill, WorkbenchToolbar, boolText, fmtDate } from "../components";
...
export function ServicesPage() {
  ...
  return <div className="grid gap-[18px]">
    ...
    <section className="rounded-md border border-line bg-panel shadow-sm"><WorkbenchToolbar title="Service console" description="Gateway、daemon、supervisor 与 service 快照。"><Button variant="outline" size="sm" asChild><Link to="/platforms/openclaw/guard"><Activity />进入守护</Link></Button><BoundaryBadge /></WorkbenchToolbar><div className="grid gap-[18px] p-3 lg:grid-cols-[minmax(0,1fr)_360px]"><div className="grid gap-3"><div className="grid gap-3 md:grid-cols-4"><StatTile label="gateway" value={<StatusPill tone={health.data?.gateway === "online" ? "ok" : "warn"}>{health.data?.gateway ?? "unknown"}</StatusPill>} sub={`port ${health.data?.gatewayPort ?? "—"}`} /><StatTile label="service" value={health.data?.serviceState ?? "—"} sub={health.data?.serviceSubState ?? "—"} /><StatTile label="daemon" value={recovery.data?.daemon.pid ?? "—"} sub={recovery.data?.daemon.version ?? "—"} /><StatTile label="supervisor" value={service?.supervisor ?? "—"} sub={service?.serviceName ?? "—"} /></div><ResponsiveTable columns={["service", "state", "detail", "owner"]} rows={rows.map((row) => <tr key={row.key}><td className="max-w-[280px] truncate px-4 py-3 font-medium text-ink-strong">{row.name}</td><td className="px-4 py-3"><StatusPill tone={row.state === "online" || row.state === "active" ? "ok" : "warn"}>{row.state}</StatusPill></td><td className="max-w-[260px] truncate px-4 py-3 text-muted">{row.detail}</td><td className="max-w-[220px] truncate px-4 py-3 text-muted">{row.owner}</td></tr>)} empty="无服务证据" /></div><DetailRail title="Daemon service" subtitle="systemd / launchd / scheduled-task"><EvidenceRow label="installed" value={boolText(service?.installed)} /><EvidenceRow label="active" value={service?.activeState ?? "—"} /><EvidenceRow label="enabled" value={service?.enabledState ?? "—"} /><EvidenceRow label="config path" value={service?.configPath ?? "—"} /><EvidenceRow label="last checked" value={fmtDate(service?.lastCheckedAt)} /></DetailRail></div></section>
  </div>;
}
