...
const CONFIG_SECTIONS: Array<{ id: ConfigSection; title: string; desc: string }> = [
  ...
  { id: "models", title: "模型", desc: "主模型与 fallback" },
  ...
  { id: "advanced", title: "高级证据", desc: "MCP / Commands / Provider" },
];
...
function renderSection(section: ConfigSection, draft: ConfigDraft, setField: (key: keyof ConfigDraft) => (value: string) => void, setBool: (key: keyof ConfigDraft) => (value: boolean) => void, data: ConfigSummaryPayload | undefined) {
  if (section === "defaults") return <Panel><PanelHead title="基础设置" sub="OpenClaw 默认工作目录、并发和运行超时。" /><FieldGrid><TextField label="Workspace" value={draft.workspace} onChange={setField("workspace")} helper="默认启动目录" /><TextField label="Repo root" value={draft.repoRoot} onChange={setField("repoRoot")} helper="默认仓库根目录" /><TextField label="最大并发" type="number" value={draft.maxConcurrent} onChange={setField("maxConcurrent")} /><TextField label="运行超时秒" type="number" value={draft.timeoutSeconds} onChange={setField("timeoutSeconds")} /><TextField label="Subagent 并发" type="number" value={draft.subagentMaxConcurrent} onChange={setField("subagentMaxConcurrent")} /><TextField label="Context tokens" type="number" value={draft.contextTokens} onChange={setField("contextTokens")} helper="留空交给 OpenClaw 默认处理" /></FieldGrid></Panel>;
  if (section === "models") return <Panel><PanelHead title="模型设置" sub="主模型、fallback 和特殊任务模型。" /><FieldGrid><TextField label="默认模型" value={draft.model} onChange={setField("model")} /><TextField label="Subagent 模型" value={draft.subagentModel} onChange={setField("subagentModel")} /><TextField label="图片模型" value={draft.imageModel} onChange={setField("imageModel")} /><TextField label="PDF 模型" value={draft.pdfModel} onChange={setField("pdfModel")} /><TextField label="模型 fallback" value={draft.modelFallback} onChange={setField("modelFallback")} multiline helper="每行一个，或逗号分隔" /></FieldGrid><div className="border-t border-line px-4 py-3 text-xs text-muted">Provider 和密钥仍由模型网关/Provider 页管理；这里只写 OpenClaw 默认引用。</div></Panel>;
  if (section === "runtime") return <Panel><PanelHead title="运行策略" sub="思考等级、压缩、流式和输入策略。" /><FieldGrid><TextField label="Thinking" value={draft.thinking} onChange={setField("thinking")} /><TextField label="Verbose" value={draft.verbose} onChange={setField("verbose")} /><TextField label="Typing mode" value={draft.typingMode} onChange={setField("typingMode")} /><TextField label="Block streaming" value={draft.blockStreaming} onChange={setField("blockStreaming")} /><SelectField label="Compaction mode" value={draft.compactionMode} onChange={setField("compactionMode")} options={["safeguard", "auto", "off"]} /><TextField label="Compaction model" value={draft.compactionModel} onChange={setField("compactionModel")} /><TextField label="Reserve tokens floor" type="number" value={draft.reserveTokensFloor} onChange={setField("reserveTokensFloor")} /><TextField label="Memory flush soft threshold" type="number" value={draft.memoryFlushSoftThresholdTokens} onChange={setField("memoryFlushSoftThresholdTokens")} /><ToggleField label="Memory flush" checked={draft.memoryFlushEnabled} onChange={setBool("memoryFlushEnabled")} /></FieldGrid></Panel>;
  if (section === "security") return <Panel><PanelHead title="安全与工具" sub="沙箱、工具执行和审批默认策略。" /><FieldGrid><SelectField label="Sandbox mode" value={draft.sandboxMode} onChange={setField("sandboxMode")} options={["off", "workspace-write", "read-only"]} /><SelectField label="Workspace access" value={draft.sandboxWorkspaceAccess} onChange={setField("sandboxWorkspaceAccess")} options={["rw", "ro"]} /><TextField label="Sandbox scope" value={draft.sandboxScope} onChange={setField("sandboxScope")} /><TextField label="Session tools visibility" value={draft.sandboxSessionToolsVisibility} onChange={setField("sandboxSessionToolsVisibility")} /><TextField label="Sandbox prune idle hours" type="number" value={draft.sandboxPruneIdleHours} onChange={setField("sandboxPruneIdleHours")} /><TextField label="Sandbox prune max age days" type="number" value={draft.sandboxPruneMaxAgeDays} onChange={setField("sandboxPruneMaxAgeDays")} /><TextField label="Tools profile" value={draft.toolsProfile} onChange={setField("toolsProfile")} /><TextField label="Exec host" value={draft.execHost} onChange={setField("execHost")} /><TextField label="Exec mode" value={draft.execMode} onChange={setField("execMode")} helper="设置后会覆盖 ask/security" /><TextField label="Exec ask" value={draft.execAsk} onChange={setField("execAsk")} /><TextField label="Exec security" value={draft.execSecurity} onChange={setField("execSecurity")} /><TextField label="Exec timeout sec" type="number" value={draft.execTimeoutSec} onChange={setField("execTimeoutSec")} /><TextField label="Approval security" value={draft.approvalSecurity} onChange={setField("approvalSecurity")} /><TextField label="Approval ask" value={draft.approvalAsk} onChange={setField("approvalAsk")} /><TextField label="Approval ask fallback" value={draft.approvalAskFallback} onChange={setField("approvalAskFallback")} /><ToggleField label="Tools elevated enabled" checked={draft.toolsElevatedEnabled} onChange={setBool("toolsElevatedEnabled")} /><ToggleField label="FS workspace only" checked={draft.fsWorkspaceOnly} onChange={setBool("fsWorkspaceOnly")} /><ToggleField label="Approval auto allow skills" checked={draft.approvalAutoAllowSkills} onChange={setBool("approvalAutoAllowSkills")} /></FieldGrid></Panel>;
  if (section === "gateway") return <Panel><PanelHead title="网关与控制台" sub="OpenClaw 网关监听、认证、rate limit 和 Control UI。" /><FieldGrid><TextField label="Port" type="number" value={draft.gatewayPort} onChange={setField("gatewayPort")} /><TextField label="Mode" value={draft.gatewayMode} onChange={setField("gatewayMode")} /><TextField label="Bind" value={draft.gatewayBind} onChange={setField("gatewayBind")} /><TextField label="Custom bind host" value={draft.gatewayCustomBindHost} onChange={setField("gatewayCustomBindHost")} /><TextField label="Auth mode" value={draft.gatewayAuthMode} onChange={setField("gatewayAuthMode")} /><TextField label="Rate limit attempts" type="number" value={draft.gatewayRateLimitMaxAttempts} onChange={setField("gatewayRateLimitMaxAttempts")} /><TextField label="Rate limit window ms" type="number" value={draft.gatewayRateLimitWindowMs} onChange={setField("gatewayRateLimitWindowMs")} /><TextField label="Rate limit lockout ms" type="number" value={draft.gatewayRateLimitLockoutMs} onChange={setField("gatewayRateLimitLockoutMs")} /><TextField label="Control UI base path" value={draft.controlUiBasePath} onChange={setField("controlUiBasePath")} /><TextField label="Allowed origins" value={draft.controlUiAllowedOrigins} onChange={setField("controlUiAllowedOrigins")} multiline helper="每行一个 origin" /><ToggleField label="Allow Tailscale" checked={draft.gatewayAllowTailscale} onChange={setBool("gatewayAllowTailscale")} /><ToggleField label="Rate limit exempt loopback" checked={draft.gatewayRateLimitExemptLoopback} onChange={setBool("gatewayRateLimitExemptLoopback")} /><ToggleField label="Control UI enabled" checked={draft.controlUiEnabled} onChange={setBool("controlUiEnabled")} /><ToggleField label="Allow insecure auth" checked={draft.controlUiAllowInsecureAuth} onChange={setBool("controlUiAllowInsecureAuth")} /></FieldGrid></Panel>;
  if (section === "messages") return <Panel><PanelHead title="会话与消息" sub="DM scope、thread binding、ack 和队列策略。" /><FieldGrid><TextField label="DM scope" value={draft.sessionDmScope} onChange={setField("sessionDmScope")} /><TextField label="Thread idle hours" type="number" value={draft.threadBindingsIdleHours} onChange={setField("threadBindingsIdleHours")} /><TextField label="Thread max age hours" type="number" value={draft.threadBindingsMaxAgeHours} onChange={setField("threadBindingsMaxAgeHours")} /><TextField label="Response prefix" value={draft.responsePrefix} onChange={setField("responsePrefix")} /><TextField label="Ack reaction" value={draft.ackReaction} onChange={setField("ackReaction")} /><TextField label="Ack reaction scope" value={draft.ackReactionScope} onChange={setField("ackReactionScope")} /><TextField label="Queue mode" value={draft.queueMode} onChange={setField("queueMode")} /><TextField label="Queue debounce ms" type="number" value={draft.queueDebounceMs} onChange={setField("queueDebounceMs")} /><TextField label="Queue cap" type="number" value={draft.queueCap} onChange={setField("queueCap")} /><TextField label="Queue drop" value={draft.queueDrop} onChange={setField("queueDrop")} /><ToggleField label="Thread bindings enabled" checked={draft.threadBindingsEnabled} onChange={setBool("threadBindingsEnabled")} /><ToggleField label="Remove ack after reply" checked={draft.removeAckAfterReply} onChange={setBool("removeAckAfterReply")} /></FieldGrid></Panel>;
  return <Panel><PanelHead title="高级证据" sub="这些区域目前展示证据，不直接提供无保护编辑。" /><ResponsiveTable columns={["区域", "证据"]} rows={[<SelectableRow key="providers" id="providers" selected={false} onSelect={() => undefined}><td className="px-4 py-3 font-medium text-ink-strong">Providers</td><td className="px-4 py-3"><JsonSnippet value={data?.providers ?? []} /></td></SelectableRow>, <SelectableRow key="mcp" id="mcp" selected={false} onSelect={() => undefined}><td className="px-4 py-3 font-medium text-ink-strong">MCP servers</td><td className="px-4 py-3"><JsonSnippet value={data?.mcp?.servers ?? {}} /></td></SelectableRow>, <SelectableRow key="commands" id="commands" selected={false} onSelect={() => undefined}><td className="px-4 py-3 font-medium text-ink-strong">Commands</td><td className="px-4 py-3"><JsonSnippet value={data?.commands ?? {}} /></td></SelectableRow>]} empty="无高级证据" /></Panel>;
}
...
export function ConfigPage() {
  ...
  if (config.isLoading || diagnostics.isLoading) return <div className="grid gap-[18px]" role="status" aria-busy="true"><Skeleton className="h-[118px] w-full" /><Skeleton className="h-[420px] w-full" /></div>;
  ...
  return <div className="grid gap-[18px]">
    <ReadOnlyStrip>配置页拆分为基础、模型、策略、安全、网关、会话消息和高级证据子页面；可编辑字段统一走 PATCH /api/config，MCP、Commands、密钥和 Provider 仍保留只读证据。</ReadOnlyStrip>
    <Panel>
      <WorkbenchToolbar title="OpenClaw 配置" description="分组设置工作台，避免一个巨型表单和无保护覆盖。"><RefreshButton loading={config.isFetching || diagnostics.isFetching} onClick={() => { void config.refetch(); void diagnostics.refetch(); }} /><BoundaryBadge /><Badge variant={dirty ? "warn" : "mute"}>{dirty ? "有未保存修改" : `checked ${fmtDate(data?.checkedAt)}`}</Badge></WorkbenchToolbar>
      <SectionNav active={activeSection} onChange={setActiveSection} />
      <div className="grid gap-3 p-3 md:grid-cols-4"><StatTile label="当前子页" value={activeMeta.title} sub={activeMeta.desc} /><StatTile label="默认模型" value={data?.defaults.model ?? "—"} sub="OpenClaw default" /><StatTile label="网关端口" value={data?.gateway.port ?? "—"} sub={`${data?.gateway.mode ?? "—"} / ${data?.gateway.bind ?? "—"}`} /><StatTile label="安全" value={data?.sandbox.mode ?? "—"} sub={data?.tools.execSecurity ?? "—"} /></div>
    </Panel>
