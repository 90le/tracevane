...
import { Activity, AlertTriangle, CheckCircle2, ExternalLink, ListChecks, Wrench } from "lucide-react";
...
import { BoundaryBadge, boolText, DetailRail, EvidenceRow, Panel, PanelHead, RefreshButton, ResponsiveTable, SelectableRow, StatusPill, useSelectedKey } from "../components";
...
/** Severity order for the actionable list: danger items first. */
...
export function DiagnosticsPage() {
  ...
  const visibleChecks = [...(problemChecks.length ? problemChecks : checks)].sort((a, b) => severityRank(a.level) - severityRank(b.level));
  ...
  return (
    <div className="grid gap-[18px]">
      <PageHeader
        className="px-0"
        title="诊断"
        description="system diagnostics、doctor、bootstrap 与设备信任摘要。诊断页只定位问题，修复进入守护页。"
        meta={<BoundaryBadge />}
        actions={<>
      ...
      <Panel>
        <PanelHead
          title="诊断检查"
          sub="按严重程度排序：错误在前，警告其次；每项都带修复或证据入口。"
          action={problemChecks.length ? <Badge variant="warn">{problemChecks.length} 项需关注</Badge> : <Badge variant="ok">全部正常</Badge>}
        />
              ...
              rows={visibleChecks.map((check) => (
                <SelectableRow key={check.id} id={check.id} selected={selectedCheckId === check.id} onSelect={setSelectedCheckId}>
                  ...
                  <td className="px-4 py-3 text-right">
                    {check.level === "ok" ? (
                      <Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); setSelectedCheckId(check.id); }}>详情</Button>
                    ) : (
                      <Button variant={check.fixable ? "outline" : "ghost"} size="sm" asChild onClick={(event) => event.stopPropagation()}>
                        <Link to="/platforms/openclaw/guard">{check.fixable ? <><Wrench />去修复</> : "查看证据"}</Link>
                      </Button>
                    )}
                  </td>
                </SelectableRow>
      ...
      <Panel>
        <PanelHead title="命令证据" sub="命令证据按需加载，避免进入诊断页时被 doctor/status 慢命令阻塞。" action={<Badge variant="mute">{includeCommands ? "已加载" : "未加载"}</Badge>} />
        <ResponsiveTable
          columns={["命令", "结果", "耗时"]}
