...
import { ArrowLeft, Bot, ChevronRight, Pencil, Plus, Trash2 } from "lucide-react";
...
const emptyBot: BotDraft = { id: "default", enabled: true, dmPolicy: "", groupPolicy: "", streaming: "", responsePrefix: "", fieldValues: {} };
const policyOptions = [{ value: "", label: "继承频道" }, { value: "open", label: "开放：允许所有匹配会话" }, { value: "allowlist", label: "白名单：只允许配置对象" }, { value: "disabled", label: "关闭：不处理此类消息" }];
const streamOptions = [{ value: "", label: "继承频道" }, { value: "off", label: "关闭流式" }, { value: "partial", label: "部分流式：边生成边更新" }, { value: "final", label: "仅最终消息" }];
const groupNames: Record<string, string> = { credentials: "凭据", connection: "连接", files: "文件路径", identity: "身份", behavior: "行为", media: "媒体" };
const fieldLabels: Record<string, string> = {
  token: "访问令牌", botToken: "Bot Token", appToken: "App Token", appId: "App ID / 应用 ID", appSecret: "App Secret / 应用密钥", clientSecret: "Client Secret", signingSecret: "签名密钥", verificationToken: "校验 Token", encryptKey: "加密 Key", serviceAccount: "服务账号 JSON", channelAccessToken: "Channel Access Token", channelSecret: "Channel Secret", accessToken: "Access Token", refreshToken: "Refresh Token", privateKey: "私钥", password: "密码",
  apiUrl: "API 地址", wsUrl: "WebSocket 地址", cdnUrl: "CDN 地址", baseUrl: "Base URL", serverUrl: "服务地址", webhookPath: "Webhook 路径", domain: "租户域名", homeserver: "Homeserver", host: "主机", port: "端口", tls: "启用 TLS", proxy: "代理",
  botUid: "Bot UID", userId: "用户 ID", username: "用户名", nick: "昵称", realname: "真实名称", defaultTo: "默认目标", channels: "频道列表", profile: "配置档", account: "账号", cliPath: "CLI 路径", authDir: "认证目录",
  historyLimit: "历史消息条数", dmHistoryLimit: "私聊历史条数", textChunkLimit: "文本分片长度", maxLinesPerMessage: "单条最大行数", mediaMaxMb: "媒体大小上限 MB", pollIntervalMs: "轮询间隔 ms", heartbeatIntervalMs: "心跳间隔 ms", requireMention: "需要 @ 才响应", ignoreMentionAll: "忽略 @所有人", sendReadReceipts: "发送已读回执", responsePrefix: "回复前缀", messagePrefix: "消息前缀",
};
const optionLabels: Record<string, string> = { "": "继承/未设置", off: "关闭", on: "开启", true: "开启", false: "关闭", open: "开放", allowlist: "白名单", disabled: "关闭", partial: "部分流式", final: "仅最终消息", length: "按长度分片", newline: "按换行分片", always: "总是", mentions: "仅被 @ 时", never: "从不", minimal: "最小", extensive: "完整", online: "在线", idle: "空闲", dnd: "勿扰", invisible: "隐身" };
function labelForField(field: ChannelFieldDescriptor): string { return fieldLabels[field.key] ?? field.label; }
function labelForOption(value: string, label: string): string { return optionLabels[value] ?? label; }
...
function stringFromValue(value: unknown): string { return Array.isArray(value) ? value.join("\n") : value == null ? "" : String(value); }
function draftFromBot(account?: ChannelAccountSummary, catalog?: ChannelCatalogEntry): BotDraft { const fields = [...(catalog?.accountFields ?? []), ...(catalog?.credentialFields ?? [])]; return { id: account?.id ?? "default", enabled: account?.enabled ?? true, dmPolicy: account?.dmPolicy ?? "", groupPolicy: account?.groupPolicy ?? "", streaming: account?.streaming ?? "", responsePrefix: account?.responsePrefix ?? "", fieldValues: Object.fromEntries(fields.map((field) => [field.key, stringFromValue(account?.fieldValues[field.key])])) }; }
function normalizeFieldValue(field: ChannelFieldDescriptor, value: string): unknown { if (!value.trim()) return undefined; if (field.input === "boolean") return value === "true"; if (field.input === "number") return Number(value); if (field.input === "stringList") return value.split(/\n|,/).map((item) => item.trim()).filter(Boolean); return value; }
...
function DynamicField({ field, value, onChange }: { field: ChannelFieldDescriptor; value: string; onChange: (value: string) => void }) { const label = labelForField(field); if (field.input === "boolean") return <SelectInput label={label} value={value} onChange={onChange} options={[{ value: "", label: "继承/未设置" }, { value: "true", label: "开启" }, { value: "false", label: "关闭" }]} />; if (field.input === "select") return <SelectInput label={label} value={value} onChange={onChange} options={(field.options ?? []).map((o) => ({ value: o.value, label: labelForOption(o.value, o.label) }))} />; if (field.input === "textarea" || field.input === "stringList") return <label className="grid gap-1.5 sm:col-span-2"><span className="text-xs font-medium text-muted">{label}</span><textarea value={value} placeholder={field.placeholder} rows={3} onChange={(event) => onChange(event.target.value)} className="min-w-0 resize-y rounded-sm border border-line bg-panel-2 px-3 py-2 text-sm text-ink-strong outline-none focus:shadow-[var(--ring)]" />{field.helpText ? <span className="text-xs text-subtle">{field.helpText}</span> : null}</label>; return <TextInput label={label} type={field.input === "number" ? "number" : field.secret ? "password" : "text"} value={value} placeholder={field.placeholder} onChange={onChange} />; }
function FieldGroups({ fields, draft, setDraft }: { fields: ChannelFieldDescriptor[]; draft: BotDraft; setDraft: React.Dispatch<React.SetStateAction<BotDraft>> }) { const groups = new Map<string, ChannelFieldDescriptor[]>(); for (const field of fields) { const group = field.group ?? (field.secret ? "credentials" : "behavior"); groups.set(group, [...(groups.get(group) ?? []), field]); } return <>{[...groups.entries()].map(([group, groupFields]) => <div key={group} className="grid gap-3"><PanelHead title={groupNames[group] ?? group} sub="字段来自 OpenClaw channel catalog / manifest；不同频道显示不同字段。" /><div className="grid gap-3 sm:grid-cols-2">{groupFields.map((field) => <DynamicField key={field.key} field={field} value={draft.fieldValues[field.key] ?? ""} onChange={(value) => setDraft((prev) => ({ ...prev, fieldValues: { ...prev.fieldValues, [field.key]: value } }))} />)}</div></div>)}</>; }
...
export function ChannelsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<
    ...
    | { kind: "bot"; channelType: string; bot: ChannelAccountSummary }
    | null
  >(null);
  ...
  const active = list.find((channel) => channel.type === activeType) ?? null;
  const selected = active ?? filtered.find((channel) => channel.type === selectedKey) ?? filtered[0] ?? list[0];
  const selectedCatalog = channels.data?.catalog.find((entry) => entry.type === selected?.type);
  ...
  const openChannelEditor = (mode: "edit" | "new", channel?: ChannelSummary) => { setChannelMode(mode); setChannelDraft(mode === "new" ? { ...emptyChannel, type: channels.data?.catalog[0]?.type ?? "" } : draftFromChannel(channel)); setChannelEditorOpen(true); };
  const openBotEditor = (mode: "edit" | "new", bot?: ChannelAccountSummary) => { if (!selected) return toast.error("请先进入 Channel"); setBotMode(mode); setBotDraft(mode === "new" ? draftFromBot(undefined, selectedCatalog) : draftFromBot(bot, selectedCatalog)); setBotEditorOpen(true); };
  const saveChannel = () => { if (!channelDraft.type.trim()) return toast.error("Channel 类型不能为空"); if (channelMode === "new") createChannel.mutate({ type: channelDraft.type.trim(), enabled: channelDraft.enabled }, { onSuccess: () => { updateChannel.mutate({ type: channelDraft.type.trim(), payload: channelPayload() }); toast.success("OpenClaw Channel 已新增"); setSelectedKey(channelDraft.type.trim()); setActiveType(channelDraft.type.trim()); setChannelEditorOpen(false); }, onError: (error) => toast.error("新增失败", { description: error.message }) }); else updateChannel.mutate({ type: channelDraft.type, payload: channelPayload() }, { onSuccess: () => { toast.success("OpenClaw Channel 已保存"); setChannelEditorOpen(false); }, onError: (error) => toast.error("保存失败", { description: error.message }) }); };
  ...
  const removeChannel = (channel: ChannelSummary) => setDeleteTarget({ kind: "channel", channel });
  const removeBot = (bot: ChannelAccountSummary) => {
    if (!selected) return;
    setDeleteTarget({ kind: "bot", channelType: selected.type, bot });
  };
  const catalogOptions = (channels.data?.catalog ?? []).map((entry) => ({ value: entry.type, label: `${entry.icon} ${entry.label} (${entry.type})` }));
  const botFields = [...(selectedCatalog?.accountFields ?? []), ...(selectedCatalog?.credentialFields ?? [])];
