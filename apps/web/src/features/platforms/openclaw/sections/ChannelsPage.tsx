...
export function ChannelsPage() {
  ...
  return <div className="grid gap-[18px]">
    ...
    <section className="rounded-md border border-line bg-panel shadow-sm"><WorkbenchToolbar title="Channel catalog" description="平台侧 channel/account/profile 证据。"><SearchBox value={query} onChange={setQuery} placeholder="搜索 channel" /><BoundaryBadge /></WorkbenchToolbar><div className="grid gap-[18px] p-3 lg:grid-cols-[minmax(0,1fr)_360px]"><div className="grid gap-3"><div className="grid gap-3 md:grid-cols-4"><StatTile label="channels" value={channels.data?.counts.channels ?? 0} sub="OpenClaw native" /><StatTile label="accounts" value={channels.data?.counts.accounts ?? 0} sub="channel accounts" /><StatTile label="profiles" value={channels.data?.counts.profiles ?? 0} sub="routing profiles" /><StatTile label="pairing" value={channels.data?.counts.pairingPending ?? 0} sub="pending" /></div><ResponsiveTable columns={["channel", "accounts", "bindings", "dm policy"]} rows={filtered.map((channel) => <tr key={channel.type}><td className="max-w-[260px] truncate px-4 py-3"><div className="font-medium text-ink-strong">{channel.type}</div><div className="text-xs text-muted">OpenClaw native</div></td><td className="px-4 py-3 text-muted">{channel.accountCount}</td><td className="px-4 py-3 text-muted">{channel.bindingCount}</td><td className="px-4 py-3"><StatusPill tone={channel.enabled ? "ok" : "warn"}>{channel.dmPolicy ?? (channel.enabled ? "enabled" : "disabled")}</StatusPill></td></tr>)} empty="无匹配 Channel" /></div><DetailRail title={selected?.type ?? "未选择 Channel"} subtitle="OpenClaw platform-side evidence"><EvidenceRow label="enabled" value={selected ? String(selected.enabled) : "—"} /><EvidenceRow label="accounts" value={selected?.accountCount ?? "—"} /><EvidenceRow label="bindings" value={selected?.bindingCount ?? "—"} /><EvidenceRow label="dm policy" value={selected?.dmPolicy ?? "—"} /></DetailRail></div></section>
