...
function formatAgentOptionLabel(agent: { id: string; name?: string; model?: string }): string {
  const name = agent.name?.trim();
  const model = agent.model?.trim();
  const identity = name && name !== agent.id ? `${name}（${agent.id}）` : agent.id;
...
export function BindingsPage() {
  ...
  const agentOptions = channels.data?.agents ?? [];
  const agentSelectOptions = agentOptions.map((agent) => ({ value: agent.id, label: formatAgentOptionLabel(agent) }));
