...
export function AgentsPage() {
  ...
  return <div className="grid gap-[18px]">
    <section className="rounded-md border border-line bg-panel shadow-sm">
      <WorkbenchToolbar title="OpenClaw 原生 Agent" description="OpenClaw agents.list 的原生定义。列表只负责浏览和选择；编辑在右侧抽屉中完成，长列表不会把表单挤到页面底部。"><SearchBox value={query} onChange={setQuery} placeholder="搜索 OpenClaw agent / model" /><Button size="sm" onClick={() => openEditor("new")}><Plus className="size-4" />新增 Agent</Button><Button variant="outline" size="sm" onClick={openBatchEditor} disabled={selectedIds.size === 0}>批量编辑 {selectedIds.size || ""}</Button><RefreshButton loading={agents.isFetching} onClick={() => { void agents.refetch(); }} /><Badge variant="info">可编辑</Badge></WorkbenchToolbar>
      <div className="grid gap-3 p-3"><div className="grid gap-3 md:grid-cols-5"><StatTile label="原生 Agent" value={agents.data?.count ?? 0} sub={`默认 ${agents.data?.defaultAgentId ?? "—"}`} /><StatTile label="可用模型" value={agents.data?.availableModels.length ?? 0} sub="OpenClaw 模型清单" /><StatTile label="已启用" value={list.filter((agent) => agent.enabled).length} sub="OpenClaw agents.list" /><StatTile label="当前选择" value={selected?.id ?? "—"} sub="点击行选中，按钮编辑" /><StatTile label="批量选择" value={selectedIds.size} sub="只修改勾选字段" /></div><div className="flex flex-wrap items-center gap-2 rounded-sm border border-line bg-panel-2 px-3 py-2 text-sm text-muted"><Button variant="ghost" size="sm" onClick={toggleFilteredSelection}>{allFilteredSelected ? "取消选择当前列表" : "选择当前列表"}</Button><span>已选择 {selectedIds.size} 个 Agent；批量编辑只会提交你在抽屉里勾选的字段。</span></div><ResponsiveTable columns={["选择", "OpenClaw agent", "默认模型", "定义状态", "操作"]} rows={filtered.map((agent) => <SelectableRow key={agent.id} id={agent.id} selected={selectedKey === agent.id} onSelect={setSelectedKey}><td className="px-4 py-3"><input type="checkbox" checked={selectedIds.has(agent.id)} onChange={(event) => toggleSelectedId(agent.id, event.target.checked)} onClick={(event) => event.stopPropagation()} aria-label={`选择 ${agent.id}`} className="h-4 w-4 accent-primary" /></td><td className="max-w-[320px] truncate px-4 py-3"><div className="font-medium text-ink-strong">{agent.name}</div><div className="truncate text-xs text-muted">{agent.id}</div></td><td className="max-w-[220px] truncate px-4 py-3 text-muted">{agent.model || "—"}</td><td className="px-4 py-3"><StatusPill tone={agent.enabled ? "ok" : "warn"}>{agent.enabled ? "已启用" : "已停用"}</StatusPill></td><td className="px-4 py-3"><div className="flex gap-2"><Button variant="outline" size="sm" onClick={(event) => { event.stopPropagation(); openEditor("edit", agent); }}><Pencil className="size-4" />编辑</Button><Button variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); remove(agent); }}><Trash2 className="size-4" />删除</Button></div></td></SelectableRow>)} empty="无匹配 Agent" /></div>
    </section>
